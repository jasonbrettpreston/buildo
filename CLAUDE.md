...
5. **Automated Gate (Spec 124 R-AG, 2026-09-14):** Husky pre-commit runs the cheap gates + `npm run typecheck && npm run lint && npx vitest related <staged files>`; pre-push runs the FULL `npm run test` (`VITEST_MIN_FORKS=1 VITEST_MAX_FORKS=2` — the variables vitest's forks pool actually reads, both needed on a 12-CPU host or vitest throws a min/max conflict; `VITEST_MAX_WORKERS` is ignored; a 4-fork run was OS-killed for memory). Live-DB tests run under `npm run test:db`. Composition is locked by `src/tests/hooks-composition.infra.test.ts`. ESLint enforces `no-empty` and bans `process.exit()` in `src/`.
