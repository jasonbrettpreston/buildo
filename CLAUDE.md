...
5. **Automated Gate (Spec 124 R-AG, 2026-09-14):** Husky pre-commit runs the cheap gates + `npm run typecheck && npm run lint && npx vitest related <staged files>`; pre-push runs the FULL `npm run test` (`VITEST_MIN_FORKS=1 VITEST_MAX_FORKS=1` — the variables vitest's forks pool actually reads, both needed on a 12-CPU host or vitest throws a min/max conflict; `VITEST_MAX_WORKERS` is ignored; a 4-fork run was OS-killed for memory and 2 forks hit the `onTaskUpdate` worker timeout in step-conformance, so the cap is ONE fork — `.husky/pre-push` is the source of truth and `step-validate.mjs` parses its value). Live-DB tests run under `npm run test:db`. Composition is locked by `src/tests/hooks-composition.infra.test.ts`. ESLint enforces `no-empty` and bans `process.exit()` in `src/`.
...
**Code Reviewer** (`code-reviewer-grounded`): Missing telemetry/logging · Type safety and `any` usage · Naming and pattern consistency · Dead code introduced.

**Observability** (`observability-reviewer`): Audit-row completeness vs spec · verdict cascade row-derived, never a parallel-boolean (`hasFails ? …`) · §11 counter scoping (primary-entity only) · producer/consumer `records_meta` contracts · Spec 48 §3.6/§3.7 + Spec 79 C1–C12 / risk-class tripwires.
...
**Trigger — Reality-Check:** runs in **pipeline WF1 / WF2 / WF3 that add or change any enriched/derived field** (Spec 08 fires it by SUBJECT MATTER, not table — the old "parcel field" wording had a documented miss on record) — plan-altitude on `.cursor/active_task.md`, output-altitude on the re-run data. Skip for pure plumbing (chain wiring, manifest, non-data refactors).

**Cross-read Adversary** (`general-purpose`, main tree — Spec 08 §10 roster, §11.2 trigger): fires after ANY fold, before implementation — checks the folded decisions PAIRWISE for collisions, walks every checklist line for staleness, verifies the tests still cohere as a suite. Paired with a grounder that RE-EXECUTES every claim in the fold.

**Idempotency Lens** (`general-purpose`, main tree — Spec 08 §10 roster): fires on any plan/diff touching a destructive write, backfill, transaction boundary, re-runnable script, or upsert. Hunts the replay/transaction/collision class: same-input-twice state, what-runs-outside-the-txn, run-2-vs-run-1's-artifacts, and fold-decision collisions. Provenance: F18, the backup-atomicity race, the stuck-interim-accretion collision — each unseen by every other seat.

**Grounded Verification Protocol (Spec 08 §11, Backend/Pipeline-calibrated):** no unexecuted executable claim in a plan or fold · fold-validation (grounder + Cross-read Adversary) after every fold · locks proven both directions before red is claimed · tests live IN the plan and the plan panel reviews them · CLI models are whole-file audit generators, never diff reviewers, always grounder-adjudicated. Frontend nuance: §11.5-FE — execute the domain's executable surface; never demand queries of a design judgment.
...
**Two altitudes per WF (Spec 08 §6.4 governs — both mandatory, no longer "on request").** Every WF1/WF2/WF3 runs a PLAN roster (agents read `.cursor/active_task.md` + the live tree/DB, BEFORE code) and an OUTPUT roster (agents read the diff + re-run values). They are complementary — the reality-grounders (Reality-Check, Schema-Fidelity, Ground-truth, Integration) earn their keep at BOTH (RC caught a $105.24M gut-line at PLAN altitude). **Integration is a standing PLAN member for Backend/Pipeline + Admin WF1/WF2** (verifies the plan's wiring / schema-seeding / migration assumptions against reality). **WF3 now has a LEAN plan roster** — 2–3 premise-verifiers (Reality-Check or Ground-truth + Integration + one DeepSeek lens) that confirm the fix's premise before code (the eager-fix antibody). DeepSeek runs as a LENS SET (spec / security / idempotency / error-paths), never one generic pass, at both altitudes — each pass adjudicated by a grounder (§9 CLI blind spots).
