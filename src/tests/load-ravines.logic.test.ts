...
describe('staleness force_run (A-3, RAVINE_FORCE_RELOAD)', () => {
  ...
  it('projects override.accept_anomaly[] to the ctx.overrides keys the compute reads', () => {
    ...
    // dry_run: false — LW-D15 (pilot 4 WF3-D): resolveOverrides now also returns dry_run
    // (ctx.overrides.dry_run exposed); load_ravines declares override.dry_run "none" too.
    expect(off).toEqual({ accept_feature_count_drift: false, accept_mass_delete: false, force_run: false, force_full: false, dry_run: false });
