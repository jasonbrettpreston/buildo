...
describe('write-class-disposition — WD-1 both-directions lock (Spec 122 §1.4/§1.10/§8.2, Spec 124 §5 R-X)', () => {
  ...
  it('assertion (2b) RED: flipping on_contention.self_skip to retire is caught even though checkNoBannedClassDeclared (the class-only check) would stay GREEN — proves the on_contention half is not decorative', () => {
    ...
    // the on_contention-aware checker catches every one of the 15 live descriptors
    ...
    // 2026-09-16, + enrich_ravines, batch-2 row 2.1 commit 1, 2026-09-18 — every one
    // of those still pending at the moment it was counted but its descriptor already
    // on disk, which is exactly why this count is DESCRIPTOR-scoped and not
    ...
    expect(contentionFindings.length).toBe(18); // batch-2 row 2.4 — compute_parcel_cost_estimates descriptor-scoped, +1
