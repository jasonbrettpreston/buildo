...
export const STEP_DESCRIPTIONS: Record<string, StepDescription> = {
  ...
  address_points:       { summary: 'Loads Toronto address point reference data for geocoding', fields: ['address_point_id', 'latitude', 'longitude'], table: 'address_points' },
  parcels:              { summary: 'Loads Toronto property parcel boundaries and lot dimensions', fields: ['parcel_id', 'lot_size_sqm', 'frontage_m', 'depth_m', 'geom'], table: 'parcels' },
  massing:              { summary: 'Loads 3D building massing models from City shapefile', fields: ['source_id', 'footprint_area_sqm', 'max_height_m', 'estimated_stories'], table: 'building_footprints' },
  neighbourhoods:       { summary: 'Loads neighbourhood boundary polygons and demographic data', fields: ['neighbourhood_id', 'name', 'avg_household_income', 'geom'], table: 'neighbourhoods' },
  ...
  link_massing:         { summary: 'Links permits to 3D building footprints via parcel intersection', fields: ['parcel_id', 'building_id', 'match_type', 'confidence'], table: 'parcel_buildings' },
  ...
  link_wsib:            { summary: 'Matches extracted entities against WSIB registry by name', fields: ['is_wsib_registered', 'linked_entity_id', 'match_confidence'], table: 'entities' },
  ...
  create_pre_permits:   { summary: 'Creates placeholder permit records from eligible CoA applications', fields: ['application_number', 'decision', 'ward'], table: 'coa_applications' },
  ...
  // Classify
  classify_scope_class: { summary: 'Classifies permits into project types (residential/commercial/mixed)', fields: ['project_type', 'scope_classified_at'], table: 'permits' },
  ...
  // Quality (CQA)
  assert_schema:        { summary: 'Validates upstream CKAN/CSV column headers before ingestion', fields: ['checks_passed', 'checks_failed', 'errors'], table: 'pipeline_runs' },
  assert_data_bounds:   { summary: 'Post-ingestion SQL checks for cost outliers, null rates, referential integrity', fields: ['checks_passed', 'checks_failed', 'checks_warned'], table: 'pipeline_runs' },
  // Deep Scrapes (coming soon)
  inspections:          { summary: 'Scrapes permit inspection stages from City Application Status portal', fields: ['stage_name', 'inspection_date', 'status'], table: 'permit_inspections' },
