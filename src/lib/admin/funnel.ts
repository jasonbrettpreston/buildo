...
export function computeRowData(
  ...
  if (lastUpdated) {
    ...
    // produce 0 records when no new upstream data was loaded (incremental runs)
      ...
      const LOADER_SLUGS = ['permits', 'coa', 'address_points', 'parcels', 'massing', 'neighbourhoods', 'load_wsib', 'load_zoning'];
...
export const STEP_DESCRIPTIONS: Record<string, StepDescription> = {
  ...
  load_heritage:        { summary: 'Loads Toronto Heritage Register (Part IV/V address points) + Heritage Conservation Districts (polygons) from two City shapefiles (Ontario Heritage Act)', table: 'heritage_properties' },
  load_centreline:      { summary: 'Loads the Toronto Centreline street-network segments from the City shapefile (Spec 62 §8c); drives corner/through-lot detection', table: 'toronto_centreline' },
  ...
  enrich_centreline:    { summary: 'Spatially joins parcels against the centreline (Spec 62 §8d); writes is_corner_lot / is_through_lot / primary_frontage_street_name + lineage onto parcels', table: 'parcels' },
  ...
  classify_coa_trades:  { summary: 'Consumes scope_tags via TAG_TRADE_MATRIX to write lead_trades rows + optional realtor append per Spec 42 §6.8', table: 'lead_trades' },
  compute_coa_cost_estimates: { summary: 'Geometric-only cost estimator for CoA leads — joins parcel/building/trade context through the Brain to write coa_applications cost cols + lead_id-keyed cost_estimates rows', table: 'cost_estimates' },
  ...
  dispatch_notifications: { summary: 'Gated push-notification dispatcher (P25): drains the notification queue, sends via Expo behind a once-per-(user,lead,type,Toronto-day) ledger, records tickets/receipts, prunes dead tokens. Inert until notifications_dispatch_enabled=1.', table: 'notification_dispatches' },
...
export const PIPELINE_TABLE_MAP: Record<string, string> = {
  ...
  load_ravines: 'ravines', load_heritage: 'heritage_properties', load_centreline: 'toronto_centreline',
  enrich_parcels: 'parcels', enrich_ravines: 'parcels', enrich_heritage: 'parcels', enrich_centreline: 'parcels', enrich_permits: 'permits', enrich_coa_zoning: 'coa_applications',
