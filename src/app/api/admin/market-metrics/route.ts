...
// --- reference month ---
// Use current month if it has data, otherwise fall back to the most recent month with permits.
// This prevents showing all-zeros on the 1st of the month before new data arrives.

async function getReferenceMonth(): Promise<string> {
  const rows = await query<{ month: string }>(
    `SELECT date_trunc('month', MAX(issued_date))::date::text AS month FROM permits`
  );
...
async function fetchKpi(refMonth: string) {
...
async function fetchTrades(refMonth: string) {
  // Hybrid: permit_type direct mapping for dedicated types + ALL classified trades
  // for building permits (no is_active filter — market metrics shows total pipeline)
  const rows = await query<{
    name: string;
    slug: string;
    color: string;
...
async function fetchResidentialVsCommercial() {
  // Fetch 24 months so we can pair each recent month with its year-ago counterpart
  const rows = await query<{
    month: string;
    residential: string;
    commercial: string;
  ...
  // Return only the recent 12 months, each with its YoY counterpart
...
async function fetchScopeTagsSegmented(refMonth: string) {
...
async function fetchNeighbourhoods() {
  ...
  // Group by tier, take top 5 per tier, skip unknown
  ...
  // Return in order: high → middle → low, including empty tiers
