...
- [~] 1.4 **PARTIAL — scoped down to pass 5 ONLY, per coordinator instruction (commit 3).** `startStallTicker(pool, pipelineRunId, intervalMs, getRowsProcessed)` is a genuine `setInterval` started before `enrichOptimalConfig`'s for-await loop and cleared in a `finally` — same "second connection while the main client is blocked" mechanism this row specifies. **NOT landed**: the ticker does NOT cover `defer_scope`/pass1-4 (those remain uninstrumented — computeDeferScope never runs on the cloud sources chain anyway, STEP 0 H6 ruling), and it does not stamp a `current_pass` value (pass 5 is the only pass it watches, so the field would be constant). Both-directions lock landed for the reduced scope: a stalled mock stream proves the ticker fires after 2 no-progress ticks; a progressing mock stream proves it does not false-positive. **Remaining scope filed for pilot 9 (ENRICHER conversion) charter**, per STEP 2.5.
- [~] 1.5 **PARTIAL — landed as `records_meta.stall_diagnostic` (not `stall_probe`), single backend, no INFO/WARN counter pair (commit 3).** `captureStallDiagnostic(pool, pipelineRunId)` runs the H5/H3 `pg_stat_activity` query (`pid`, `state`, `wait_event_type`, `wait_event`, `query_start` — no `query` text column at all, so there is no 200-char-truncation need: nothing query-text-shaped is captured, which is a stricter bound than this row asked for) on a fresh `pool.query` connection and writes the single top row into `records_meta.stall_diagnostic` (Spec 48 §3.10, extended). **NOT landed**: top-5 backends (only the oldest `query_start` match is captured); the paired `enrich_parcels_stall_probes`/`enrich_parcels_stall_probe_errors` INFO/WARN audit-row counters (this is a diagnostic write, not yet a `checks[]`/`verdictCascade` audit row — filed for pilot 9). Fires once per contiguous stall episode (re-arms when progress resumes), never spamming one query per tick.
