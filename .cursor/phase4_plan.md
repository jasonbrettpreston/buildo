...
> **BALLOT RULINGS RECORDED (operator, 2026-07-21).** #1 OD-A = **code alias** (RESOLVED, shipped in `client.ts`). #2 session-pooler pin = **YES** (env change, operator-applied). #3 DECODO_ secret names = **CONFIRMED renamed**. #4 CI-root-cred blast radius **ACCEPTED** + PITR **OFF** (posture, accepted). #5 `disable_signup=true` **ACCEPTED** (dashboard, operator). #6 OD-D merge timing, #7 OD-C `PG_POOL_MAX`, #8 OD-F `.vercelignore` scope = **still open** (low-stakes, operator-timed).

1. **OD-A — connection-string gap (blocks first deploy). ✅ RESOLVED = CODE ALIAS.** `src/lib/db/client.ts` now resolves `process.env.POSTGRES_URL ?? process.env.DATABASE_URL` (integration injects `POSTGRES_URL`, not `DATABASE_URL`; local dev + pipeline scripts keep `DATABASE_URL`, un-aliased). Shipped as F1a code; regression-locked in `src/tests/db-client.logic.test.ts`. (P4-G7; Spec 113 §3.)
2. **Session-pooler URL pin (fold #14). ✅ YES.** Pin `SUPABASE_DATABASE_URL` to the **session-pooler `:5432` form**, NOT the IPv6-only direct host `db.<ref>.supabase.co` (which intermittently `ENOENT`'d live). Applies to the `DATABASE_URL=` override for `migrate.js`, to `restore-db.js`'s `--target=cloud`, AND to the `SUPABASE_DATABASE_URL` GitHub secret. **This is an env/secret change the operator applies — NOT a code change.**
3. **DECODO proxy GH-secret names (fold #9). ✅ CONFIRMED renamed** to `DECODO_PROXY_HOST` / `DECODO_PROXY_PORT` / `DECODO_PROXY_USER` / `DECODO_PROXY_PASS` — the workflows read `secrets.DECODO_PROXY_*` (`chain-deep-scrapes.yml:179-182`), **not** `PROXY_*`.
4. **CI root-credential blast radius + PITR-off re-confirm (fold #10). ✅ ACCEPTED.** Spec 113 §8.2-option-3 risk re-confirmed now that F0 loads real production data (the `SUPABASE_DATABASE_URL` GitHub secret is the `rolbypassrls` `postgres` role; poisoned workflow dep = total prod compromise + backup tampering). **PITR OFF (D9) — accepted posture.** Both are operator/posture rulings, no code.
5. **Cloud signup posture (fold #8). ✅ ACCEPTED — `disable_signup=true`** on the cloud project until launch (dashboard, operator), to prevent admin-email squatting and shrink pre-launch surface.
6. **OD-D — migration-branch → `main` merge timing. ⬜ OPEN.** Merging to `main` is required before 4.3 cron `workflow_dispatch`/`schedule` (P4-G9) AND it **arms the 3 pg_cron jobs' code path on `main`** — the operator picks when.
7. **OD-C — `PG_POOL_MAX` Vercel value. ⬜ OPEN.** Recommend **1–2** at launch, tune at 4.4. (P4-G7)
8. **OD-F — `.vercelignore` scope beyond `docs/`. ⬜ OPEN.** Recommend +`mobile/`, +`.cursor/`.

_Resolved & removed this fold:_ **OD-B** (G10 gate-(f) re-pin — empirically moot, zero drift confirmed; see CG1), **OD-E** (transfer size — measured/estimated, see CG2). **PITR** posture is not re-opened as a standalone item; it rides ballot #4 (ACCEPTED).
