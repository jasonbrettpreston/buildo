...
async function main() {
  ...
  L.push('# P16 — Trade-Attachment Evaluation (P14 ground truth + P16 lean-complement gate)');
  L.push('');
  L.push('_Generated by `scripts/analysis/p14-trade-attachment-evaluation.js` (READ-ONLY; the script name keeps its P14 birth-prefix — P16 extended it with scenario 6 + the DEV/HOLD-OUT gate). Spec 80 §5.C._');
  L.push('');
  ...
  // (P16 addendum nit: the delta is computed from the ROUNDED display values so the printed
  // arithmetic closes — e.g. 16.6 − 6.0 = 10.6, not the unrounded 10.65+.)
  const meanDeltaDisplay = (Number(whole.pre.meanAttached.toFixed(1)) - Number(whole.evid.meanAttached.toFixed(1))).toFixed(1);
  L.push(`**Demoted-bundle recall value** (P14-B item 3): pre-P13-3 recall ${(whole.pre.recall * 100).toFixed(1)}% − evidence-only recall ${(whole.evid.recall * 100).toFixed(1)}% = **${demotedRecall.toFixed(1)} pts** of recall the demoted bundle tier was contributing — at the cost of ${whole.pre.meanAttached.toFixed(1)}−${whole.evid.meanAttached.toFixed(1)} = ${meanDeltaDisplay} extra attached trades/permit and prec(insp) ${(whole.pre.precInsp * 100).toFixed(1)}% vs ${(whole.evid.precInsp * 100).toFixed(1)}%.`);
  L.push('');

  L.push('## Scenario scores — per stratum (recall / prec(insp))');
  L.push('');
  L.push('| Stratum (n) | (1) pre-P13-3 | (2) evidence | (3) scope-union | (5a) ceil-evid | (6) P16 lean ∪ evid |');
  L.push('|---|---|---|---|---|---|');
  for (const [k, v] of [...stratCounts.entries()].sort((a, b) => b[1] - a[1])) {
    ...
    L.push(`| ${k} (${v}) | ${cell('pre')} | ${cell('evid')} | ${cell('scope')} | ${cell('ceilEvid')} | ${cell('lean6')} |`);
  }
  L.push('');
  L.push('_Per-stratum scenario-6 numbers are WHOLE-corpus (dev+hold combined): the hold-out half of any single stratum is too small (n≈8-39) to stand alone — the gate reads the pooled hold-out only._');
  L.push('');
  ...
  L.push('');
  L.push('_The split is SEEDED + DETERMINISTIC: within each stratum, permits are ordered by md5(permit_num) and alternated DEV/HOLD — byte-reproducible on any machine, no RNG state._');
  L.push('');
  L.push('**Calibration provenance (stated honestly):** the complement was calibrated on a WHOLE-CORPUS per-inspectable-trade TP/FP diagnostic, then scored on the held-out half — the drop signal (hvac 0 TP/69 FP, plumbing 0/38, demolition 0/61, drain-plumbing 3/48) was separately verified to replicate at ≤9% precision on DEV alone, so the leakage is immaterial; do NOT read the hold-out as a virgin sample.');
  L.push('');
