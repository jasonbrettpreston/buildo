...
pipeline.run('classify-inspection-status', async (pool) => {
  ...
  const stalledResult = await pool.query(
    `UPDATE permits p
     ...
     FROM (
       SELECT pi.permit_num, MAX(pi.inspection_date) AS last_activity
       ...
       HAVING MAX(pi.inspection_date) < NOW() - INTERVAL '${STALE_MONTHS} months'
     ) stale
  ...
  // (scraper may have re-scraped and found recent inspection dates)
  ...
  pipeline.emitMeta(
    { permits: ['permit_num', 'enriched_status'], permit_inspections: ['permit_num', 'inspection_date'] },
