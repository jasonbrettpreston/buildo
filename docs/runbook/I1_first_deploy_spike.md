...
- `scripts/classify-lifecycle-phase.js` — CoA-side ACTIVE: emits a ledger row when `coa_applications.matched_status` changes from the prior classifier run. **Permit-side ACTIVE as of Phase I.1.1b (commit `73b257b`):** `classifyLifecyclePhase()` extended to return `matchedStatus` per Spec 84 §3.7 18-rule contract; dirty SELECT predicate adds `OR matched_rule IS NULL` so existing classified permits backfill on first I.1.1b run.
...
| `classify-lifecycle-phase.js` (permit-side) | classify-lifecycle-phase.js | **ACTIVE as of Phase I.1.1b (commit `73b257b`).** **Day 1 spike: 230K-245K rows** (per Spec 84 §2.5.a row counts: ~247K permits with non-null status — only rule 1 status-null and a tiny `'Notice Sent'` catchall slice are excluded; everything else emits a ledger row). Day 2+: converges to ~500–2000 rows/day. The `permitFirstDeployGrace` flag softens `permit_unmapped_status_count` from WARN to INFO during the first 7 days. | ~500–2000 rows/day after Day-7 convergence |
