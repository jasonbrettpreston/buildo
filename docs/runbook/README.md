...
| `scripts/analysis/enrich-heritage-cohort-differential.js` | Batch-2 row 2.2 §4.5 re-runnable committed-perturbation differential (output-panel O2). Reads the committed 494-row cohort (`docs/reports/golden/enrich_heritage/differential/cohort.json`), perturbs it, runs the REAL `scripts/enrich-heritage.js`, asserts `records_updated=494` + the whole-table projected hash returns to baseline, and restores-on-failure unconditionally. LOCAL DEV DB ONLY (`PG_HOST=127.0.0.1 PG_PORT=54322 ...`) — never point at a shared/cloud target. |
| `scripts/analysis/compute-parcel-cost-cohort-differential.js` | Batch-2 row 2.4 §4.5/R-AS re-runnable committed-perturbation differential. Reads the committed 1,006-row cohort (`docs/reports/golden/compute_parcel_cost_estimates/differential/cohort.json`; ≥100/zone × 7 zones + all `cost_fb_total>15M` + all `cost_addition_total>50M` + 200 empty-menu sample + top-10 `cost_gut_total`, plus a 50-row non-residential negative control), perturbs it, runs the REAL step (`--step-script=<abs path>`, default this tree's `scripts/compute-parcel-cost-estimates.js`; pass the LEGACY pre-conversion script's path for the legacy-side arm), asserts `records_updated>=1006` + the 16-column projected hash returns to baseline, and restores-on-failure unconditionally. LOCAL DEV DB ONLY (`PG_HOST=127.0.0.1 PG_PORT=54322 ...`) — never point at a shared/cloud target. |
| `scripts/analysis/compute-parcel-cost-keyset-diff.mjs` | Batch-2 row 2.4 output-panel O3 mechanical PRE-vs-POST key-set diff — `records_meta` top-level keys AND `audit_table.rows[].metric` keys, against the committed goldens (`docs/reports/golden/compute_parcel_cost_estimates/{pre,post}/<arm>.json`). `--arm=sources\|standalone` (default `sources`). Read-only, no DB connection; the "account for every diff" tool behind assessment §6.5's key-set-diff confirmation. Not a generic tool — the golden paths and field names are specific to this step. |
