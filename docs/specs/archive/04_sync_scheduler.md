...
- **Current State (as of 2026-03-04):** Cloud Scheduler is NOT deployed. Pipeline execution can be triggered manually via admin dashboard "Run" / "Run All" buttons (`POST /api/admin/pipelines/{slug}`), or automatically via the local cron worker (`npm run local-cron`). The local worker uses `node-cron` with `America/Toronto` timezone to trigger chains on schedule: permits daily 6 AM, CoA daily 7 AM, sources quarterly. It includes a concurrency guard (checks `pipeline_runs` for running chains) and graceful shutdown. The `syncTrigger` Cloud Function exists in `functions/src/index.ts` as an HTTP endpoint but is not wired to Cloud Scheduler.
