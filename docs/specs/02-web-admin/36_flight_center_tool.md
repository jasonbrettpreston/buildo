...
- **Denormalization rationale:** `permit_num`/`revision_num`/`coa_application_number` are stored ALONGSIDE the derived `lead_key` (which encodes the same identity) so the flight-list JOINs to `permits`/`coa_applications` need no key-parsing, and the XOR CHECK can enforce the per-type shape at the DB layer. Drift between the components and the key is prevented by construction: rows are write-once (`ON CONFLICT DO NOTHING`, no UPDATE path) and the key is produced exclusively by the single test-locked `buildLeadKey` builder from those same components — the consistency mechanism is the builder, not a DB trigger.
...
- **`GET /api/admin/leads/watchlist`** — the flight list. `admin_watchlist` JOIN `permits`/`coa_applications` LEFT JOIN `trade_forecasts` (UNION ALL of a permit arm and a coa arm, each producing the same column interface). Forecast join is ON `tf.lead_id = admin_watchlist.lead_key` (the ONE uniform key that works for permit AND coa rows), through `permit_trades` scoped `is_active = true` so demoted/inactive trades never supply the project's expected start. Project-level expected-start = the EARLIEST `tf.predicted_start` row across the lead's active-trade forecast rows (one `ORDER BY predicted_start ASC LIMIT 1` LATERAL per arm); `opportunity_score` comes **FROM that same earliest-start row** (same-row semantics — see §4.5). `temporal_group` computed server-side via the shared `computeTemporalGroup` through a thin aggregation wrapper (see §4). Server-side paginated: `LIMIT 50` + offset; total count in `meta`.
- **`POST /api/admin/leads/watchlist`** — bulk save. Body `{ items: Array<{ lead_type, permit_num/revision_num | coa_application_number, lead_key?, address? }> }`. Per-item `safeParse` (one bad item must not reject the batch); valid items → multi-row `INSERT ... ON CONFLICT (admin_uid, lead_key) DO NOTHING`; response `{added, skipped_existing, failed:[{index, reason}]}`. **Conflict rule:** the stored `lead_key` is ALWAYS recomputed server-side via `buildLeadKey` from the component parts; when an item carries BOTH a `lead_key` and component parts, the recomputed key must agree with the client's — a mismatch fails THAT ITEM (the per-item `failed[]` shape, not a batch 400), and the client key is never trusted as the stored key.
- **`DELETE /api/admin/leads/watchlist`** — bulk delete. Body `{ ids: number[] }` → deduped post-Zod (the `.max(1000)` cap applies to the raw array) → `DELETE WHERE admin_uid = $1 AND id = ANY($2)`; `deleted` derives from the RETURNING rowCount (real rows only).
- **`GET /api/admin/leads/watchlist/search?q=`** — address → permit/coa resolution. `q` min-length ≥2 is enforced at the Zod layer (400 below it — kills the single-char trgm-scan DoS vector). Ports the `permits` street-substring logic from the consumer `leads/search` route behind `verifyAdminAuth`, ADD a coa arm (`coa_applications.address ILIKE`). Returns enough to save (`lead_type, lead_key, permit_num, revision_num, coa_application_number, address, lifecycle_phase`).
...
- `verifyAdminAuth` is the FIRST line of every route (Spec 33 §8, Spec 35 §5.1).
- **CSRF (reference, not re-implementation):** the `Origin` check on mutations is enforced INSIDE the shared guard — `src/lib/auth/verify-admin.ts:62-74` short-circuits state-mutating methods (`MUTATING_METHODS`, `:38`) through `isOriginAllowed()` (`:151-157`, DEFAULT-DENY on a missing/unlisted `Origin` vs `ADMIN_ALLOWED_ORIGINS`) BEFORE any auth-mode check. The full CSRF suite pins this at `src/tests/verify-admin.logic.test.ts:193-270`. Watchlist routes carry NO Origin code of their own (the guard covers all current and future admin routes); at the route boundary a cross-origin mutation surfaces as **401** (the guard returns null) — **403** is reserved for the `admin_key` session-write rejection below.
...
- **Core Logic:**
  ...
  4. **Detail drill-down = the inspect endpoint + a flight-semantics header.** The drawer reuses `GET /api/admin/leads/inspect/:id` (the ~70-field / 8-panel all-info surface; the ≥0.85 identity-link floor is preserved — a sub-0.85 CoA link stays gated to null so the inspector never shows the WRONG permit). ABOVE the panels: a prominent header with the DELAYED badge + EXPECTED START DATE + `temporal_group` chip; the 8 diagnostic panels render below (secondary). The header takes `temporal_group`/aggregated `predicted_start`/p25/p75 from the already-fetched watchlist row as `initialData` (instant render), then RECONCILES from the `useLeadInspect` forecast/lifecycle panels once they resolve — header and body converge on the same snapshot. **Reconciliation authority ([PF14]):** once the `useLeadInspect` query resolves, ITS result is AUTHORITATIVE for the header — stalled from the lifecycle panel, expected-start/window/score re-aggregated from the forecast panel; the cached watchlist row is display-only `initialData` until then and never overrides a resolved inspect payload (implementation: `FlightSemanticsHeader` in `src/components/admin/FlightCenterTool.tsx` — every derived field reads `inspect ? <inspect-derived> : <row fallback>`).
  5. **DELAYED / EXPECTED-START / SCORE derivation.** There is NO stored `delayed` column. DELAYED = `lifecycle_stalled` (present on BOTH `permits` and `coa_applications`, so the badge is uniform across lead types) OR a past-due aggregated `predicted_start`. EXPECTED START = the project-level `predicted_start` = the EARLIEST-start row across the lead's ACTIVE-trade forecast rows (earliest FUTURE-or-past-due; "when does ANYTHING start"), with that row's p25/p75 as the window. **`opportunity_score` = the score FROM that same earliest-start forecast row (same-row semantics — a SEMANTIC CHANGE from the earlier `MAX(opportunity_score)`-across-rows shape; the UI reads it as "score at next start"). Response-contract note: start, window and score are always mutually consistent — they describe ONE forecast row, never a composite of two. Accepted caveat: a lead whose only active forecasts lack `predicted_start` reads a null score (such forecasts are non-actionable anyway).** `temporal_group` server-side via the shared `computeTemporalGroup`: the admin aggregation WRAPPER checks `lifecycle_stalled → action_required` FIRST, then delegates to the untouched shared function (the wrapper corrects the shared function's null-score `on_the_horizon`-before-stalled ordering for the new tool WITHOUT forking or editing the shared function — the consumer flight-board keeps its WF3-#13 demotion semantics).
