...
- **Why the middleware cannot be that gate — the MECHANISM, measured 2026-09-15 (WF3 SEC-1).** This section previously said only "middleware is bypassable", and the hypothesised bypasses (matcher gaps, direct invocation, misclassification) are all **REFUTED**: `config.matcher` covers every `/api/admin/*` path, and `classifyRoute` returns `'admin'` on a `startsWith('/api/admin/')` prefix with a fail-closed default. The real bypass is narrower and worse — **the admin/API arm performs no authorization at all, only a presence check**: `if (!hasValidSession && !request.headers.get('x-admin-key')) return 401; return response;`. `hasSupabaseSessionCookie` passes on ANY non-empty `sb-*-auth-token` cookie, and the `x-admin-key` arm is presence-only **by design** (the P1-F4 break-glass TRANSPORT decision, 2026-07-19; the secret compare belongs to `verify-admin.ts` mode 2 and must NOT be re-added to the edge). So `curl -X PUT -H 'x-admin-key: anything' …/api/admin/control-panel/configs` reached the handler. Measured: **11 of 29 admin route files had no guard**, including `stats` — which `src/app/dashboard/page.tsx:14`, an `'authenticated'`-class page for any signed-in consumer, fetches from the browser. Both directions are now locked in `src/tests/api.infra.test.ts` + `src/tests/admin-route-guard.infra.test.ts`; `00_engineering_standards.md` §4.1 is amended to match.
...
**The `admin_key`/`dev_bypass` uids are NOT personal identities.** Any admin feature that writes **per-admin-scoped rows** (a personal watchlist, a per-admin draft, a saved view) MUST gate its mutations on `authMethod === 'session'` and return **403** on `admin_key` (CI/pipeline scripts have no personal state); `dev_bypass` writes are tolerated as dev-local artifacts under the `'dev-user'` sentinel. This is the **Spec 36 PF1 pattern** and is now the house precedent for per-admin-scoped writes (`src/app/api/admin/leads/watchlist/route.ts`). Read-only endpoints accept all three auth methods.

**AMENDED 2026-09-15 (WF3 SEC-1) — the rule widens from per-admin-scoped rows to EVERY AUDITED admin mutation, and `dev_bypass` is no longer tolerated on those.** `admin_audit_log.admin_uid` is **UUID NOT NULL**, while BOTH sentinels (`'admin-key'`, `'dev-user'`) are non-UUID strings. Once a mutation writes its audit row inside its own transaction (Spec 128 R-12), a sentinel-authenticated call can only raise `22P02 invalid input syntax for type uuid` and roll back as a 500. The gate is therefore `authMethod !== 'session'` → **403 `SESSION_REQUIRED`**, applied to all 13 mutating exports closed by that WF3 — a refused mutation is better than an unattributable one, and a named 403 is better than a rolled-back 500. The prior narrower `authMethod === 'admin_key'` shape on `leads/watchlist` (which tolerated `dev_bypass`) is a **knowingly retired fence**, recorded on its test. **LOCAL DEV — the operator cost is PAID, not absorbed (Integration fold, same day).** Refusing both sentinels left local dev with a dead admin write surface: `isDevMode()` returned `'dev-user'`, all 13 mutations 403'd, and no admin button could be exercised locally. **Ruled:** in dev mode ONLY, `DEV_ADMIN_UID` resolves the bypass to a real UUID returned with **`session` semantics**, so mutations run and their audit rows carry a genuine, attributable `admin_uid`. Dev keeps its mutations, under a KNOWN AUDITED IDENTITY rather than a shared sentinel.
> **Measured precondition — stronger than "seed a `profiles` row".** `admin_audit_log.admin_uid` is UUID NOT NULL **with `fk_admin_audit_log_admin → auth.users(id) ON DELETE RESTRICT`** (`migrations/229_uid_uuid_fk_conversion.sql:104-106`; the SEC-1 plan's §0.4 measured the type and MISSED the FK), and `profiles.id` FKs to `auth.users(id)` as well (`migrations/226:25`). So `DEV_ADMIN_UID` must be the id of a REAL local `auth.users` row with `profiles.is_admin = true` — precisely what `scripts/bootstrap-first-admin.js` provisions against the local stack. A made-up UUID does not half-work: the audit INSERT raises `23503` inside the mutation transaction and the mutation rolls back. Loud, not silent.
> **Production is unreachable by construction:** `isDevMode()` already requires BOTH `DEV_MODE === 'true'` AND `NODE_ENV !== 'production'` (Spec 13 §4a two-flag defense), and the dev-admin branch re-checks `NODE_ENV` independently. Unset or non-UUID `DEV_ADMIN_UID` falls back to the original `'dev-user'` sentinel — byte-identical to the pre-fold behaviour, so the feature changes nothing by merely existing. Locked both directions in `src/tests/dev-admin-identity.logic.test.ts`.

The alternatives were weighed and rejected: a reserved sentinel UUID would make two different actors indistinguishable in the compliance log, and widening `admin_uid` to TEXT would drop the constraint that makes the log joinable to `profiles`. When emitting per-admin telemetry, hash the uid (`sha256(uid).slice(0,16)`, §11 + Spec 35 §7.3) — never the raw uid to PostHog.
...
- **Refusal envelopes — `unauthorized()` / `sessionRequired(ctx, route)`** (`src/lib/admin/admin-responses.ts`). The 401 (§8, no/failed admin credential) and the 403 (§8.1, a shared non-session sentinel) live in ONE module, not copy-pasted per route — WF3 SEC-1 introduced twelve copies and they were consolidated the same day. The **403 LOGS** (`logWarn` with `{ authMethod, route }`); the 401 deliberately does not, because `verifyAdminAuth` already records the failure reason and a second line would double-count every unauthenticated probe. A 403 here is a real operator or CI job being turned away mid-task whose only symptom is a button that does nothing — it was silent at nine call sites until this module existed.
