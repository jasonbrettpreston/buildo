...
- **Cost Coverage (Section 2)** shows TWO percentages:
  ...
  - **CoA cost coverage (WF2 P5, 2026-07-06):** `coa_cost_coverage_pct` = priced CoAs (`coa_applications.cost_source='archetype_parcel'`) / total CoAs — written to `pipeline_runs` by `refresh-snapshot.js` (see Spec 83 §Acceptance re-derivation). Live baseline: corpus **58.4%** (19,449/33,280); the feed-relevant OPEN subset (geocoded + non-terminal `lifecycle_group ∈ {C1,C2,C3}`) is **49.5%** (1,585/3,200). Alongside it the **servable-CoA funnel** rows (total → geo+non-terminal → +cost → +fresh-forecast → +score; P0 baseline 33,280→3,200→1,585→1,421) are emitted as INFO audit rows — they inform, they do NOT gate the traffic light yet.
...
| Lifecycle | step 21 (`classify_lifecycle_phase`) + step 23 (`compute_phase_calibration`) | `phase` + `phase_name` (friendly name from Spec 84 §3), `stalled`, `classified_at`, `phase_started_at`, `current_phase_days_in`, `predicted_remaining_days`, `predicted_completion_at`, **`timeline[]`** — unified completed/current/upcoming entries with cohort percentile comparison (median/p25/p75/sample_size from `phase_stay_calibration`). See Spec 84 §5 (Inspector Lifecycle Timeline) for the entry shape. **WF1 #B 2026-05-09 expansion** — closes Spec 84 bug 84-W4 by giving the ledger a downstream consumer. |
...
> **Phase F.4 (DELIVERED 2026-05-17 commit `9fec4df`):** Cycle 8 amendment implemented per Spec 42 §6.11 Phase F. CoA Classification panel renders for `lead_id LIKE 'coa:%'` (primary) OR permit lead with `linked_coa_application_number` (cross-stream). 12 sub-sections including full 110-position lifecycle scrubber (bundled JSON catalog from `universal_stream_catalog`), cross-stream timeline merge via 3-arm UNION ALL (active lead + all permit revisions + linked CoA), decision-evolution timeline. Admin canonical lead_id formats: `permit:NUM:REV` (LPAD'd revision) and `coa:APP-NUM`. Cost source: `coa_applications.{estimated_cost, cost_source, modeled_gfa_sqm}` direct columns (mig 133 — geometric-only per Spec 83 §3.A; no `cost_estimates` JOIN). Lifecycle catalog icon columns: `group_icon`/`block_icon`/`stage_icon` (VARCHAR(8) per mig 128 storing emoji codepoints). When `coa_applications` row is missing for a primary `coa:` lead_id, backend returns 200 + `coa: null` + source stub with `address.full: ''`; UI renders `<ClassifierPendingBanner>`. When permit has `linked_coa_application_number` referencing a missing CoA, UI renders `<OrphanLinkedCoaBanner>` + data_quality Sentry breadcrumb. Cycle 7 thin-shape pass-through to `/api/leads/detail/:id` is RETAINED for mobile contract (Spec 91 §4.3.1) — NOT used by admin inspector.
...
| `description` | `coa_applications.description` (verbatim CKAN free-text via `load-coa.js trimToNull(raw.DESCRIPTION)`) | **WF3 #14 Pass-2.5 Finding I (shipped 2026-05-22):** rendered between `project_type` and `scope_tags` so the operator can substantiate the auto-classified scope_tags + project_type without leaving the panel. Custom multi-line-safe rendering element (NOT stock `<Field>`): bordered light-bg container with `whitespace-pre-wrap break-words max-h-32 overflow-y-auto` — preserves newlines, handles long unbreakable strings, bounded vertical height keeps panel layout stable on 2KB+ descriptions. JSX `{value ?? '—'}` expression auto-escapes — XSS-safe by construction; regression-locked by `<script>`-payload UI test. `data-testid="coa-panel-section-description"`. |
| `structure_type` | `coa_applications.structure_type` (dwelling-use archetype keyword-classified from `description` by `classify-coa-scope.js`; corrected 2026-06-20 — not denormalized from `parcel_buildings`) | Plain text |
...
| `estimated_cost` (archetype-based) | `coa_applications.{estimated_cost, cost_source, modeled_gfa_sqm}` direct columns (mig 133 — F.4 v4.1 fold; NO `cost_estimates` JOIN) | Total in dollars + a provenance badge (**WF2 §3-ARCHETYPE, 2026-07-06**): the live CoA cost path is the archetype ladder, so `cost_source='archetype_parcel'` (T2, the dominant case) / `archetype_rate` (T3, never fires on CoA) render an **"archetype"** badge; `'geometric'` renders a "geometric" badge (legacy rows only — retired for new rows, mig 209); `'none'` renders no cost. Only an UNEXPECTED value (outside `{archetype_parcel, archetype_rate, geometric, none}`) surfaces the yellow ⚠ "Unexpected cost_source: <value>" anomaly badge. Matches `CoaClassificationPanel.tsx` `COA_EXPECTED_COST_SOURCES` / `COST_SOURCE_BADGE`. No permit-side Liar's-Gate panel (CoA has no applicant declaration to gate). **Semantic pin (WF2 P12-C4):** an `archetype_parcel` cost + its trade bundle is the **opportunity menu** for the parcel — the modeled cost/trades of the maximal build the lot could support — NOT the applicant's declared CoA scope. The >$10M tail (≈64 rows, ~1,350 m² modeled GFA @ ~$8.7k/sqm) is the oversized-opt-envelope artifact, not a real application cost. |
| `modeled_gfa_sqm` | `coa_applications.modeled_gfa_sqm` | sqm + sqft conversion |
| `lifecycle_seq` + group/block/stage | `coa_applications.lifecycle_seq` JOIN `universal_stream_catalog` on `usc.seq = ca.lifecycle_seq` (F.4 v4.1: `usc.seq` is the PK column per mig 128) | Renders Group/Block/Stage labels with their `group_color` / `block_color` / `stage_color` hex + `group_icon`/`block_icon`/`stage_icon` columns (VARCHAR(8) per mig 128 — store emoji codepoints) per Spec 84 §2.5.h Color & Icon Strategy. Granular timeline scrubber with all 110 seq positions; current position highlighted via `aria-current="step"`; bid_value rendered as 0-1 progressbar with `role="progressbar"`/`aria-valuenow`. 110-row catalog bundled as `src/lib/admin/universal-stream-catalog.json` (generated by `scripts/generate-stream-catalog-json.js`); static import — no fetch at runtime. **WF3 #15 Pass-2.5 Finding G fold (2026-05-22):** the timeline reflects what `lifecycle_status_history` actually contains for the lead — entry count is data-driven, not classifier-truncated. See Spec 84 §2.5.h.10 for empirical patterns by permit category (non-construction permits + sub-permits typically show fewer entries; ALT permits should show full P1-P20 path when they reach inspection). |
| `bid_value` | `coa_applications.bid_value` / `permits.bid_value` (mig 132/133 — snapshot column populated by `classify-lifecycle-phase.js:1419` at classification time; stale until the classifier re-runs if the underlying catalog is reseeded) | **WF3 #15 Pass-2.5 Finding H fold (2026-05-22):** per-row 0-1 weighting from the Universal Stream catalog (see **the Universal Stream table in Spec 84 §2.5.h.2** for the authoritative per-row values). **NOT dollars, NOT probability, NOT percentage** — a relative bid-moment strength. **Faithful catalog band values:** `1.0` = Application Acceptable (optimal); `0.9` = pre-issuance / intake / Plan Review (strong); `0.8` = CoA decision approved (Approved / Approved with Conditions / Final & Binding / Conditional Consent — strong post-CoA bid); `0.7` = Postponed (moderate in-flight pause); `0.6` = Permit Issued (bid window still open; GC mobilizing); `0.5` = CoA Complete / Closed (moderate trailing bid); `0.2–0.4` = Notice / On Hold / Deferred / Examiner's Notice / Response Received band (weak — most rows 0.2 per the seed catalog; the 0.2–0.4 range is per Spec 84 §2.5.h.9 line 948 + §2.5.h.2 column description at line 748; opportunity engine excludes the `≤ 0.2` subset per the `no bid checkmark` rule); `0.1` / `0.05` = TLAB / OMB appeal (very weak; outcomes unpredictable); `0.0` = Refused / Withdrawn / Cancelled / terminal-dead. **UI emphasis behavior:** high-value moments (1.0 / 0.9 / 0.8) get prominent placement; low-value moments (0.2 / 0.1 / 0.05) still render but visually de-emphasized. The opportunity engine's `≤ 0.2 → exclude` rule (Spec 84 §2.5.h.9) is a separate engine-routing decision distinct from UI emphasis. |
| Linked permit (if `coa_applications.linked_permit_num IS NOT NULL` **AND `linked_confidence >= 0.85`**) | JOIN `permits` ON `permits.permit_num = coa_applications.linked_permit_num` | Permit summary chip + "Jump to Permit Inspector" link. When the inspector is loaded for the CoA `lead_id`, the linked permit shows as a sibling; vice versa. **WF2 P12-B1:** the linked-permit surface + cross-stream timeline apply the **`>= 0.85` identity floor** (`COA_IDENTITY_LINK_MIN_CONFIDENCE`) — a sub-0.85 geo/flagged link would surface the WRONG property (see Spec 60 §Link-CoA consumer contract) |
| Cross-stream timeline | `lifecycle_status_history WHERE lead_id IN ('coa:<num>', 'permit:<linked>:<rev>') ORDER BY COALESCE(event_date, (transitioned_at AT TIME ZONE 'UTC')::date) ASC, CASE WHEN event_date IS NOT NULL THEN 0 ELSE 1 END ASC, transitioned_at ASC, id ASC` | Single chronological merge of CoA + permit status history. Per Spec 42 §6.6.B Example 2 "Cross-stream timeline JOIN" — shows full project journey from CoA Application Received through Permit Closed across both entities. **WF3 Pass-2.5 Finding C Phase 5 (shipped 2026-05-22):** Inspector renders `event_date` directly when set (YYYY-MM-DD from CKAN source); falls back to date portion of `transitioned_at` with a 'detected' badge when `event_date IS NULL` (operator-visible amber badge, `data-testid="cross-stream-detected-badge"`). Cross-row ORDER BY is **TZ-deterministic** via DATE-only comparison (`AT TIME ZONE 'UTC'` anchors the transitioned_at→DATE cast independently of session timezone). On the same effective date, rows with real `event_date` sort BEFORE detected-only rows (the `CASE WHEN` tie-break). Phase 1 (mig 160) added the column; Phases 2-3 populate from `load-permits` / `load-coa` for milestone statuses (Specs 50 §3 / 51 §3); Phase 4 (mig 161) locks classifier-derived rows to NULL; Phase 5 wires the Inspector. |
