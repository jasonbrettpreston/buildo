...
| Cross-stream timeline | `lifecycle_status_history WHERE lead_id IN ('coa:<num>', 'permit:<linked>:<rev>') ORDER BY COALESCE(event_date::timestamptz, transitioned_at)` | Single chronological merge of CoA + permit status history. Per Spec 42 §6.6.B Example 2 "Cross-stream timeline JOIN" — shows full project journey from CoA Application Received through Permit Closed across both entities. **WF3 Pass-2.5 Finding C Phase 5 (planned, 2026-05-21):** Inspector renders `COALESCE(event_date, transitioned_at)` per row with a 'detected' badge when `event_date IS NULL` (operator-visible signal that this row's date reflects when the pipeline observed the change, not when the source's status actually changed). Cross-row ordering uses the same COALESCE so milestone rows (with real event dates from CKAN) sort relative to non-milestone rows (observed timestamps). Phase 1 (mig 160) added the column; Phases 2-4 populate it from writers; Phase 5 lands the Inspector rendering. |
