...
| `estimated_cost` (geometric only) | `coa_applications.{estimated_cost, cost_source, modeled_gfa_sqm}` direct columns (mig 133 — F.4 v4.1 fold; NO `cost_estimates` JOIN) | Total in dollars + `cost_source='geometric'` badge (no permit-side Liar's-Gate panel — geometric-only per Spec 83 §3.A). When `cost_source !== 'geometric'`, render yellow warning badge "Unexpected cost_source: <value>" |
