...
| `liar_gate_threshold` | Cost | The ratio window (e.g., 0.25) before city cost data is discarded as a lie. |
...
| `lifecycle_band_<phase>_min/_max` (×36) | Lifecycle CQA | **WF2 2026-05-07 (mig 119):** Per-phase row-count tolerance bands consumed by `assert-lifecycle-phase-distribution.js` (via `PHASE_TO_LOGIC_VAR_SUFFIX` map). Widen/tighten without a redeploy when CKAN deltas shift the snapshot. See Spec 84 §3.8. Legacy gate; `lifecycle_seq_band_*` are the authoritative per-seq bands (see below). |
...
| `lifecycle_seq_band_<N>_min/_max` (×220 keys, N=1–110) | Lifecycle CQA | **WF1 #coa-pipeline-parity-phase-a 2026-05-13 (mig 148):** Per-seq row-count tolerance bands — one pair per Universal Stream row (Spec 84 §2.5.h.2). **Authoritative gate keys** consumed by `assert-lifecycle-phase-distribution.js` (dynamic pattern `/^lifecycle_seq_band_(\d+)_(min\|max)$/`). Seq-level not block-level: outcome-diverse blocks (e.g., B2.C contains both seq 13 "Refused" DENIED and seq 14 "Final and Binding" APPROVED) cannot be safely aggregated. Legacy P-code bands retained alongside these for operator backward-compat (both consumed by `assert-lifecycle-phase-distribution.js`). |
| `lifecycle_band_seq_<seq>_sample_size_threshold` (×110) | Lifecycle CQA | **WF1 2026-05-13:** Per-seq tuning curve selector for the distribution gate. Determines which band-width tier applies per seq based on historical sample size: ≥1,000 rows → tight bands (median ±10–20%); 100–999 → moderate (±30%); 10–99 → loose floor+ceiling (`0 ≤ count ≤ max(20, recent_avg × 3)`); <10 (single-row outliers — 12 such seqs in v10 CSV) → INFO-only, no FAIL/WARN trigger. Each key stores the chosen tier (`'tight' | 'moderate' | 'loose' | 'info_only'`). Auto-derived from production-shape data during Phase E recalibration. |
| `lifecycle_status_history_retention_days` | Lifecycle ledger | **WF1 2026-05-13:** Days to retain rows in `lifecycle_status_history` (default 1825 = 5 years). Long retention supports forecast cohort segmentation by traversal pattern across the median 1,078-day CoA→permit lag. Hard floor: must be ≥ 730 to support typical CoA cohort learning. |
...
| **15** | `compute-cost-estimates` | Fetches `base_rate_sqft` + `structure_complexity_factor`. Applies `liar_gate_threshold`, `commercial_shell_multiplier`, `placeholder_cost_threshold`, and `income_premium_tiers`. |
...
**Objective:** Build the UI for the universal `logic_variables` (15 at Phase 3 authoring time; 451 as of WF2 "ADMIN-1 ratchet to zero", 2026-09-09; **CORRECTED 2026-09-21 (batch-2 row 2.4 cutover, R-AN's spirit)** — the live count is carried by the generated `docs/reference/logic-variables-registry.md` and is not retyped here or above; the 19 `compute_parcel_cost_*` keys this cutover adds join the EXISTING **Cost Tuning**, **Source Ingestion** and **Data Quality Thresholds** admin groups — no new group is created (FOLD-I7) — see §1 note). **WF3 S0.1 (2026-09-21):** `max_build_lot_min_sqm` / `max_build_lot_max_sqm` (the promoted `LOT_MIN_SQM`/`LOT_MAX_SQM` literals, Spec 65 §4 MB-2) join the EXISTING **Spatial & Massing** group — no new group, same FOLD-I7 pattern; regenerate via `npm run logic-var-groups` + `npm run logic-vars-docs`. **WF3 S0.3 (2026-09-21):** `parcel_sanity_onlot_share_rd_min` / `parcel_sanity_onlot_share_attached_min` (the `existing_structure_onlot_share_low` validate_only invariant's zone-aware floors, Spec 43 step #25) originally joined **Spatial & Massing** — **RETIRED at the slice-0 output-panel peel (2026-09-22, finding O1):** the invariant's `sql` never actually consumed either value (a `source:"invariant"` entry's `sql` runs verbatim, no runtime config-substitution seam exists for it, unlike `kind:"distribution"`'s `ctx.config` thread), so the two declared variables were dead — removed rather than kept declared-but-unused; 0.90/0.50 now live as documented literals in the invariant's own SQL. **WF3 S0.2 (2026-09-21/22):** `product_scope_max_existing_gfa_sqm` (the residual product-scope cut, Spec 88 §2.1, Spec 43 step #23) joins **Spatial & Massing** — no new group, the 19th `compute_parcel_cost_*`-adjacent key this WF3 programme adds (the FOLD-I7 pattern continues).
