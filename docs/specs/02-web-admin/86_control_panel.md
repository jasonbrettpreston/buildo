...
| `lifecycle_band_block_<block_id>_min/_max` (~15 pairs) | Lifecycle CQA | **WF1 #coa-pipeline-parity-phase-a 2026-05-13:** Block-level row-count tolerance bands replacing the legacy `lifecycle_band_p{N}_min/_max` (P-code-keyed) bands. Per the granular-first move (Spec 84 §3 Phase-Code Namespace Deprecation), the distribution gate validates `lifecycle_block` counts (~15 keys: B1.A through B15.H) against these new bands. CoA blocks (B1–B4), Permit blocks (B5–B9, B14–B15), and Inspection blocks (B10–B13) get their own namespace — no per-stream conflation. Legacy P-code bands retained through Phase H for backward compat. |
| `lifecycle_band_seq_<seq>_min/_max` (×110, optional) | Lifecycle CQA | **WF1 2026-05-13:** Per-seq diagnostic bands (1 per Universal Stream row). Optional — not consumed by `assert-lifecycle-phase-distribution.js` by default; available for operator-driven diagnostic queries against `lifecycle_status_history`. Higher granularity than block-level but noisier. |
