...
| `lifecycle_band_seq_<seq>_min/_max` (×110 pairs) | Lifecycle CQA | **WF1 #coa-pipeline-parity-phase-a 2026-05-13 (reframed to seq-level same day):** Per-seq row-count tolerance bands — one pair per Universal Stream row (Spec 84 §2.5.h.2). **Authoritative gate keys** consumed by `assert-lifecycle-phase-distribution.js`, replacing the legacy `lifecycle_band_p{N}_min/_max` (P-code-keyed) bands. Seq-level not block-level: outcome-diverse blocks (e.g., B2.C contains both seq 13 "Refused" DENIED and seq 14 "Final and Binding" APPROVED) cannot be safely aggregated. Tuning is sample-size-aware via `lifecycle_band_seq_<seq>_sample_size_threshold` (see below). Legacy P-code bands retained through Phase H for backward compat with `compute-trade-forecasts.js` P-code routing. |
