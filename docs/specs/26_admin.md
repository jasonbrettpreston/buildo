...
- **Core Logic:**
  ...
  - 30 pipeline slugs supported: 26 individual + 4 chain orchestrators (`chain_sources`, `chain_permits`, `chain_coa`, `chain_entities`). Chains use 1-hour timeout; individual pipelines use 10-minute timeout. See `scripts/run-chain.js`.
  - **4-Pillar Architecture:** Chains are grouped by dependency hierarchy: (1) Source Data Updates (foundation, quarterly), (2) Permits Pipeline (fast daily ingestion), (3) CoA Pipeline (fast daily ingestion), (4) Corporate Entities Pipeline (slow daily enrichment via web scraping). Enrichment scripts (`enrich_wsib_builders`, `enrich_named_builders`) are isolated in the entities chain to prevent slow external API calls from blocking core ingestion.
