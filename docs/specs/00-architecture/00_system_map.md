...
| 115 | `00-architecture/115_scheduling.md` | Pipeline Scheduling (GitHub Actions + pg_cron) | `scripts/check-chain-running.js`, `scripts/check-chain-verdict.js`, `scripts/lib/chain-concurrency.js`, +8 more | `src/tests/apply-migrations-workflow.infra.test.ts`, `src/tests/db/237_scrape_outcome_prune.db.test.ts`, `src/tests/chain.logic.test.ts` | ACTIVE |
...
| # | Spec File | Feature | Implementation | Tests | Status |
|---|-----------|---------|---------------|-------|--------|
| 101 | `01-pipeline/101_notification_dispatch.md` | 101 Notification Dispatch Engine | `scripts/lib/notification-types.js`, `scripts/classify-lifecycle-phase.js`, `scripts/update-tracked-projects.js`, +1 more | `src/tests/pipeline-advisory-lock.infra.test.ts` | Done |
...
| 119 | `01-pipeline/119_backend_verification_doctrine.md` | Backend Verification Doctrine | `scripts/*.js` | `src/tests/*.{logic,infra,db}.test.ts` | ACTIVE (authored 2026-08-15, operator-directed: *"Spec 118 is deep-scrapes-focused; generalize for the backend — what is the approach getting us to good code, what is the process, and how do we improve the process."*). |
...
| 122 | `01-pipeline/122_pipeline_step_optimization.md` | Pipeline Step Optimization (in-place standardization) | — | `src/tests/golden-fingerprint.infra.test.ts`, `src/tests/step-conformance.infra.test.ts`, `src/tests/pipeline-advisory-lock.infra.test.ts` | UNRATIFIED DRAFT · **Scope:** the `sources` chain's 27 steps first, then the estate's 64 |
...
| 123 | `01-pipeline/123_step_opt_assessment_validation.md` | Step Optimization: Assessment & Validation | — | — | UNRATIFIED DRAFT · **Scope:** the procedure applied to each of the 27 `sources` steps, then the estate's 64 |
| 124 | `01-pipeline/124_step_standard_policy.md` | Step Standard Policy (the McDonald's standard) | — | — | RATIFIED · **Scope:** every step in the pipeline estate, present and future — not scoped to the |
...
| 44 | `01-pipeline/44_chain_deep_scrapes.md` | Deep Scrapes (AIC Inspection Portal) | `scripts/manifest.json`, `scripts/aic-scraper-nodriver.py`, `scripts/aic-orchestrator.py`, +8 more | `src/tests/db/backfill-smeared-enriched-status.db.test.ts` | Done |
...
| 66 | `01-pipeline/66_enrich_permits.md` | Enrich: Permit + CoA Zoning (Spec 58 WF3) — **v1.0** | `src/components/FreshnessTimeline.tsx`, `scripts/enrich-permits.js` | `src/tests/zoning-permits.logic.test.ts`, `src/tests/permits-coa-zoning-columns.regression.test.ts`, `src/tests/db/enrich-permits.db.test.ts`, +4 more | Done |
| 78 | `01-pipeline/78_optimal_lot_configuration.md` | Optimal Lot Configuration | `scripts/load-permits.js`, `scripts/compute-build-norms.js` | `src/tests/optimal-config.logic.test.ts` | Built — Phases 0-3 + 4A + 4D shipped/PUSHED; only forecast/cost reconciliation moved to Spec 88. |
...
| 88 | `01-pipeline/88_parcel_cost_model.md` | Parcel Renovation Cost Model | `scripts/lib/parcel-cost.js`, `scripts/seeds/logic_variables.json` | — | ACTIVE (P1 shipped / DB-populated — committed `8a3a364` + pushed + dev DB populated, 2026-06-30). Design-of-record: `docs/reports/wf1-parcel-renovation-cost-model.md`. |

## Web Admin

| # | Spec File | Feature | Implementation | Tests | Status |
|---|-----------|---------|---------------|-------|--------|
| 102 | `02-web-admin/102_admin_notifications_tool.md` | 102 Admin Notifications Tool | `src/app/admin/notifications/page.tsx`, `src/app/api/admin/notifications/route.ts`, `src/app/admin/page.tsx` | `src/tests/admin-notifications.infra.test.ts` | Done |
...
| 36 | `02-web-admin/36_flight_center_tool.md` | Flight Center Tool (Standalone Admin Watchlist) | `scripts/analysis/backfill-admin-watchlist.js`, `src/app/api/admin/leads/watchlist/**`, `src/app/admin/flight-center/**`, +4 more | — | ACTIVE |
| 76 | `02-web-admin/76_lead_feed_health_dashboard.md` | Admin Lead-Feed Tooling (Test Feed + Flight Center + Detail Inspectors) | `src/app/api/admin/leads/test-feed/route.ts`, `src/components/admin/TestFeedTool.tsx`, `src/lib/admin/test-feed-utils.ts`, +16 more | `src/tests/test-feed.infra.test.ts`, `src/tests/test-feed-utils.logic.test.ts`, `src/tests/admin-detail-inspectors.ui.test.tsx`, +2 more | **CLOSED.** A `?user_type=` parameter is NOT a planned product feature. Realtors and tradespeople share Spec 91's feed/flight-board algorithm; differentiation flows through `trade_slug` (a parameter the admin Test Feed Tool already exposes), not through a separate `user_type` axis. Manufacturers are not customer-facing feed personas — they're admin-managed B2B accounts per Spec 94 §7. |
