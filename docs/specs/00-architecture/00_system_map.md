...
| 59 | `01-pipeline/59_source_ravine_protection.md` | Toronto Ravine and Natural Feature Protection (Ingest + Link) | — | `src/tests/load-ravines.logic.test.ts`, `src/tests/load-ravines.infra.test.ts`, `src/tests/db/migration-N-ravines.db.test.ts`, +2 more | Authored (WF1 Genesis — spec-only deliverable; implementation deferred per §8b) |
