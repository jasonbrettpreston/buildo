...
| 122 | `01-pipeline/122_pipeline_step_optimization.md` | Pipeline Step Optimization (in-place standardization) | — | `src/tests/golden-fingerprint.infra.test.ts`, `src/tests/step-conformance.infra.test.ts`, `src/tests/pipeline-advisory-lock.infra.test.ts` | ACTIVE · **Scope:** the `sources` chain's 27 steps first, then the estate's 64 |
