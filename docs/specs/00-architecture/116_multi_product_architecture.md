...
| Shell / runtime | Same Expo tab shell (lead feed + flight-board + flight-job already ship as tabs) | Own navigation, onboarding, session rhythm |
...
* **N2 — Entitlement is per-product from day one.** Replace the single `user_profiles.subscription_status` (Spec 96) with an `entitlements` table keyed `(user_id, product, status, stripe_subscription_id, current_period_end)` — one row per user per product engaged. Existing subscribers backfill to a single `lead_gen` entitlement so no user's access changes at cutover (moot pre-launch at zero users, retained as the cutover rule regardless).
...
| OD3 | Billing SKU shape | **OPEN — decide before wiring the Stripe webhook** | — | Bundle-with-line-items vs independent per-product subscriptions vs discounted all-product tier. Does NOT change the entitlements table; changes only the webhook fan-out (price → product → upsert entitlement). This is the next decision that unblocks webhook implementation. |
