...
8. **Commit fences (G9).** Commits happen ONLY through `git_commit`. Bash argv `git commit …`, `git add …`, `git push`, `git reset`, `git checkout`, `git restore`, `git stash`, `git clean`, `git rebase`, `git merge` are unmatched by the allowlist and therefore blocked. `git_commit` stages **only the enumerated `paths`**, each of which must be the target of a successful `write_file`/`edit_file` record earlier in the run (`PATH_NOT_LEDGERED`) — `git add -A`/`-u`/`.` do not exist as a code path. Any of `--no-verify`, `-n`, `--amend`, `--allow-empty`, `--no-gpg-sign` in `args` ⇒ **the whole call is REFUSED and ledgered** (`FLAG_REFUSED`), never stripped and retried. A single-committer advisory lock (per worktree, see C.6.4: `<ledger_dir>/committer-<hash>.lock`, `O_EXCL`, holds `{pid, run_id, repo_root, ts}`; stale if the pid is dead) must be held for the duration of the commit — contention ⇒ `COMMITTER_BUSY`.
...
1. **Write scope (per task, declared in the brief).** The brief file opens with a front-matter block (`---` … `---`) containing `write_scope:` followed by one `- <glob>` line per entry (globs relative to the repo root; `**` = any depth, `*` = within a segment; parsed by the engine's own minimal parser — no YAML library). `write_file`, `edit_file` and every `git_commit.paths` entry must match at least one scope glob, else `blocked` with code `PATH_OUT_OF_SCOPE`. Reads and `grep_files` are unrestricted by scope (a worker must read its neighbours). Provider `deepseek` with an empty or missing `write_scope` ⇒ downgrades to `claude` per §B (commit 11: `provider_source: 'fallback:no_write_scope'`, see below) rather than refusing to start. `run_start` records `write_scope`.
...
3. **Active claims registry.** `<ledger_dir>/active-claims.json` — a JSON array of `{ claim_id, run_id, pid, repo_root (realpath), branch, write_scope, ts }`, guarded by `<ledger_dir>/active-claims.lock` (`O_EXCL`, held only for the read-modify-write). At `run_start` the engine (a) drops entries whose `pid` is dead (`process.kill(pid, 0)` throws), (b) refuses to start with `CLAIM_CONFLICT` if any live entry has the same `repo_root` and an overlapping scope, or the same `branch` and an overlapping scope; overlap = the literal directory prefix of one glob (everything before its first `*`) is a prefix of the other's, (c) appends its own claim. The claim is removed at `run_end` (any status) and on process exit; a crashed engine's stale claim is reclaimed by (a). This file is separate from the ledger and is written by `scripts/lib/exec-claims.js`, so `exec-ledger.js` stays truncate-free.
