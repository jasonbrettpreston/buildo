...
| **DeepSeek Execution Engine** | `scripts/deepseek-exec.js` — a Node agentic function-calling loop with `read_file`/`grep_files`/`write_file`/`run_bash_command`/`git_commit` | Shell + filesystem + git, under built fences | Cheapest per token; **highest blast radius** | Code generation, file writing/editing, committing — the execution seat | **`live`** — v1 (2026-09-22): proven on the non-golden task class by the pilot record `docs/reports/2026-09-22-sub-eng-1-pilot-record.md` (§6); converted-step compute changes need the orchestrator's golden re-capture landing (v1.1 filed). Build: `.cursor/wf1_deepseek_execution_engine_active_task.md`. |
...
* **STATUS:** `deepseek` is **live as of SUB-ENG-1 v1 (2026-09-22)** for execution steps; the engine (`scripts/deepseek-exec.js`) reads `EXECUTION_PROVIDER` and resolves per §C.5; the fallback to `claude` remains the default and the operational floor.
...
6. **Read-before-write (G3) + staleness (G4).** `write_file`/`edit_file` on an EXISTING file require a `read_file` of that path earlier in the same run (`NOT_READ`), and the file's sha256 + mtime must equal the values captured at that read (`STALE`). Creating a new file needs no prior read. An edit whose `old_string` matches zero or more than one location fails (`NO_MATCH` / `AMBIGUOUS_MATCH`) — never guesses. `old_string` has a schema `minLength` of 1 (Step 9 panel fold, F-DS11 — an empty string has no defined match semantics); the replacement is applied with a FUNCTION replacer (F-DS10 — `String.prototype.replace(searchString, replacementString)` interprets `$&`/`$'`/`` $` ``/`$1`-style patterns in the replacement even for a plain-string search, which would otherwise let a `new_string` containing e.g. `$'` silently splice in unrelated file content instead of being inserted verbatim).
...
8. **Commit fences (G9).** Commits happen ONLY through `git_commit`. Bash argv `git commit …`, `git add …`, `git push`, `git reset`, `git checkout`, `git restore`, `git stash`, `git clean`, `git rebase`, `git merge` are unmatched by the allowlist and therefore blocked. `git_commit` stages **only the enumerated `paths`**, each of which must be the target of a successful `write_file`/`edit_file` record earlier in the run (`PATH_NOT_LEDGERED`) — `git add -A`/`-u`/`.` do not exist as a code path. `args` is **reserved in v1: ANY non-empty value is refused** (`FLAG_REFUSED`), never stripped and retried — a fixed exact-match denylist (the original v1 design) is bypassable via git's own unambiguous long-option abbreviations (`--amen` resolves to `--amend`), so the fix is refusing the whole field rather than enumerating flags. Before `git add`, the index must be empty (`INDEX_DIRTY` if not — refuse, never reset someone else's staged work); after our own `git add`, the staged set must equal `paths` exactly or the add is undone with a targeted `git reset -- <paths>` (`STAGE_MISMATCH`). The commit message's first line must match policy's `commit_message_pattern` BEFORE any git call (`MESSAGE_FORMAT`; the pattern mirrors `scripts/hooks/validate-commit-msg.sh` and is stated in the system prompt). A single-committer advisory lock (per worktree, see C.6.4: `<ledger_dir>/committer-<hash>.lock`, `O_EXCL`, holds `{pid, run_id, repo_root, ts}`; reclaimed only on a confirmed-dead pid, mtime as a fallback ONLY when the pid field is unreadable; released only if its contents still carry our own `run_id`) must be held for the duration of the commit — contention ⇒ `COMMITTER_BUSY`. The child env for `git add`/`git commit` (and the husky hooks they spawn) is scrubbed of every `GIT_*` AND `DEEPSEEK_*` variable (Step 9 panel fold, F-II6 — the four `GIT_AUTHOR_*`/`GIT_COMMITTER_*` vars are then set back explicitly), matching every other git-shelling call in the engine (`grep_files`, `run_bash_command`).
9. **Budgets (G6/G8).** `max_iterations` (default 40), `max_total_tokens` (default 400 000), per-command `timeout_ms` (default 600 000, ceiling 1 200 000), bash output cap 64 KiB per stream, `read_file` cap 256 KiB, `write_file` cap 512 KiB, `run_bash_command`/`git_commit`'s own spawned processes cap captured output at 16 MiB (`OUTPUT_TOO_LARGE` on overflow, Step 9 panel fold F-DS12). Exceeding a budget ends the run with status `budget_exhausted` — evaluated AFTER checking whether the model is genuinely finished (Step 9 panel fold, F-DS15): a turn with NO tool calls is `completed` even if that FINAL turn's own usage happens to push the total over the cap, since there is nothing left to cut off; `budget_exhausted` is reachable only when the model still wants to call a tool and the budget says no. `max_total_tokens` is compared against CUMULATIVE BILLABLE tokens — cache-miss prompt tokens (or, absent a cache signal, the full raw prompt) plus completion tokens per turn, summed — never the raw per-turn prompt total, which double/triple/N-counts a cache hit re-sent unbilled on every later turn of the same conversation (commit 12e); the raw provider fields stay ledgered unchanged alongside it.
10. **Fail closed on a malformed call.** A tool call whose name is unknown, whose arguments do not validate against C.2, or whose JSON does not parse ⇒ an `error` record with code `MALFORMED_TOOL_CALL` and the run aborts with status `aborted`. The engine never improvises a repair. The same fail-closed posture applies one level down, to TOOL_SCHEMAS itself (Step 9 panel fold, F-DS16): an unrecognised JSON Schema `type` in a property definition throws (a schema-authoring bug in the engine's own code, never a silent no-op that would admit any value). A handler that throws an unmapped runtime exception (e.g. an fs TOCTOU race) still produces exactly one structured `tool_call` record — `HANDLER_FAULT` or a mapped fs code (F-DS8) — never an uncaught crash of the engine loop (the dead `NOT_IMPLEMENTED` stub path from Phase 1, superseded once every tool shipped a real handler, was deleted rather than left unreachable, F-CR1).

### C.2 Tool-call contract (v1 — six tools, every one returns `{ ok: true, … }` or `{ ok: false, error: { code, message } }`)

| Tool | Arguments (JSON schema, `additionalProperties: false`) | Success shape | Error codes (beyond C.1's) |
|---|---|---|---|
| `read_file` | `{ path: string, offset?: int ≥ 1, limit?: int ≥ 1, reason: string }` | `{ path, content, sha256, mtime_ms, lines_total, truncated: bool }` — content is the requested window; `sha256`/`mtime_ms` are of the WHOLE file and are what C.1.6 later re-checks | `NOT_FOUND`, `IS_DIRECTORY`, `TOO_LARGE`, `PERMISSION_DENIED` |
...
| `run_bash_command` | `{ argv: string[] (≥ 1), timeout_ms?: int, reason: string }` | `{ exit_code, stdout, stderr, truncated: bool, duration_ms }` — spawned with `shell: false`, cwd = repo root, env = the parent env minus every `GIT_*` and `DEEPSEEK_*` variable | `COMMAND_NOT_ALLOWED`, `FLAG_NOT_ALLOWED`, `TIMEOUT`, `ARGV_UNSAFE_TOKEN` (Windows cmd.exe-route metacharacter guard, §C.4 note) |
| `git_commit` | `{ message: string, paths: string[] (≥ 1), args?: string[], reason: string }` (`args` **reserved in v1 — any non-empty value is refused**, see C.1.8) | `{ sha, files: string[] }` — before `git add`, the index must be EMPTY (`INDEX_DIRTY`); after our own `git add`, the staged set must equal `paths` exactly or the add is undone with a targeted `git reset` (`STAGE_MISMATCH`); the message's first line must match policy's `commit_message_pattern` BEFORE any git call (`MESSAGE_FORMAT`); runs `git add -- <paths>` then `git commit -m <message>` through the husky hooks; the engine appends a trailer line `Executed-By: deepseek-exec <model> run=<run_id>` to `message` | `PATH_NOT_LEDGERED`, `FLAG_REFUSED`, `COMMITTER_BUSY`, `INDEX_DIRTY`, `STAGE_MISMATCH`, `MESSAGE_FORMAT`, `OUTPUT_TOO_LARGE` (a spawned git process's captured output exceeded the 16 MiB buffer cap), `HOOK_FAILED` (the hook's exit code + last 64 KiB of its output are in the record) |

Every handler additionally may report `HANDLER_FAULT` (an unmapped fs/runtime exception the dispatch-level catch-all turned into a structured record rather than an uncaught crash) or one of its mapped forms — `NOT_FOUND` (`ENOENT`), `PERMISSION_DENIED` (`EACCES`/`EPERM`), `IS_DIRECTORY` (`EISDIR`) — for any tool, not only the rows above that already surface them via their own inline checks.
...
| `model_turn` | `iteration`, `usage: { prompt_tokens, completion_tokens, total_tokens, billable_tokens }` (raw provider fields, e.g. `prompt_cache_miss_tokens`, pass through unmodified; `billable_tokens` is this turn's own §C.1.9 billable count), `tool_calls: int`, `finish_reason`, `assistant_text` (redacted, ≤ 4 KiB) |
...
| `kill` | `sentinel_path` |
| `error` | `code`, `message`, `iteration?` — engine-level faults (`MALFORMED_TOOL_CALL`, `PROVIDER_ERROR`, `SCOPE_GLOB_UNANCHORED` (Step 9 panel fold, F-II2 — a refused write_scope glob, no `iteration`), `LEDGER_WRITE_FAILED` is never ledgered — it is the abort) |
| `run_end` | `status: completed\|aborted\|killed\|budget_exhausted\|delegated_to_claude\|hook_failed\|claim_conflict\|scope_invalid` (the last two, §C.6.3/F-II2, are refuse-to-start outcomes recorded alongside a `run_start`, never after a tool has dispatched), `iterations`, `usage_total` (adds `billable_tokens`, the cumulative §C.1.9 figure the budget itself compares against), `tool_calls_total`, `blocked_total`, `commits: string[]`, `duration_ms` |

**`pre` / `post` worktree capture (F10)** — present on EVERY `tool_call` whose tool is `write_file`, `edit_file`, `run_bash_command` or `git_commit`, captured immediately before and after the handler: `{ status_porcelain: string (verbatim `git status --porcelain`, ≤ 16 KiB), status_sha256, diff_sha256 (sha256 of `git diff HEAD`), head_sha }`. Reconciliation rule (exit criterion #2, amended Step 9 panel fold F-II5): a `write_file`/`edit_file` record's pre→post delta names exactly its `path`; a `run_bash_command` record's delta is EMPTY; a `git_commit` record's delta is **EXACTLY** its `paths` (the old "plus `lint-staged` formatting" allowance is REMOVED — measured: `lint-staged` here runs `biome check --no-errors-on-unmatched` with no `--write` flag, diagnostic only, so it produces no formatting delta to allow for; if `lint-staged` is ever changed to write, this allowance must be re-added WITH a lock proving it), and `head_sha` advances. Any other delta is a fence breach.
...
```json
{
  ...
  "bash_allow": [
    ...
    { "argv": ["node", "scripts/analysis/step-validate.mjs"], "flags": ["--step=<slug>", "--all", "--fast", "--staged"] },
    { "argv": ["node", "scripts/analysis/step-churn-complexity.mjs"], "flags": ["--check"] },
    { "argv": ["node", "scripts/analysis/spec-split-check.mjs"], "flags": ["--check"] },
    { "argv": ["node", "scripts/steps/_schema/generate-template-freeze.mjs"], "flags": ["--check"] }
  ],
  ...
  "commit_message_pattern": "^(feat|fix|refactor|test|docs|chore)\\([0-9]{2,3}[a-z]?_[a-z0-9_]+\\)!?: .+",
...
**Per-validator flags (Step 9 panel fold, F-II3 — the old single `<validator>`-placeholder entry gave EVERY validator the SAME flag set, which was wrong for reality: `step-validate.mjs` does not accept `--check` at all (it throws `usage: --step=<slug> | --all | --staged [--write] [--fast]`), and the hooks' own `--staged` flag was missing from policy entirely).** Each of the four validators now has its OWN `bash_allow` entry with a literal path (not the `<validator>` placeholder) and its own real flag set, verified against both the hooks (`.husky/pre-commit`, `.husky/pre-push`) and each script's own `process.argv` parsing: `step-validate.mjs` → `--step=<slug>`, `--all`, `--fast`, `--staged`; the other three (`step-churn-complexity.mjs`, `spec-split-check.mjs`, `generate-template-freeze.mjs`) → `--check` only. `commit_message_pattern` (F-II4) mirrors `scripts/hooks/validate-commit-msg.sh` byte-for-byte and gates `git_commit.message`'s first line before any git call (C.1.8).

Matching rules: literal tokens match exactly, by position; `<path>` is one repo-confined, non-denied path; `<path>...` one or more; `<rev>` matches `^([0-9a-f]{4,40}|HEAD(~[0-9]+)?)$`; `<int>` matches `^[0-9]{1,5}$`; `<range>` matches `^[0-9]+,[0-9]+$`; `<slug>` matches `^[a-z0-9_]+$`. (`<validator>`/`validators` remain supported by the matcher for a future entry that legitimately wants ANY-of-a-set — no current `bash_allow` entry uses the placeholder, since per-validator flags require a literal path per entry.) Any token beginning with `-` that is not in the entry's `flags` ⇒ `FLAG_NOT_ALLOWED` (`--fix`, `--write`, `--output`, `--reporter`, `--outputFile`, `-C`, `--git-dir`, `--work-tree`, `-c` are therefore all rejected without being named — the Cross-read collision "a path-confinement rule and an allowlist entry each safe alone" is closed by positional matching: `git -C ../x status` fails at position 1). Policy is loaded once at `run_start`, its sha256 ledgered; the file is on the C.1.4 denylist, so the model cannot widen it mid-run. **Shape validation (F-DS9):** every key documented in this section and §C.6 is REQUIRED — a load with any missing key throws `POLICY_INVALID` before `run_start` (an engine-level fault, never a silently-disabled fence).
...
* **CLI.** `node scripts/deepseek-exec.js --brief <file> [--repo <path>] [--provider=deepseek|claude] [--model <id>] [--max-iterations <n>] [--max-total-tokens <n>] [--transcript <file>] [--ledger-dir <dir>]`. `--max-total-tokens` (commit 12e) overrides `policy.limits.max_total_tokens` and is compared against cumulative BILLABLE tokens, per C.1.9. `--repo` (F14, commit 11) is the worktree root the engine operates in, defaulting to `process.cwd()`; realpath'd and asserted to contain a `.git`, and every confinement/claims/lock key downstream derives from it — this is what lets one orchestrator shell launch N engines against N worktrees. `--brief` is confined the same way (Step 9 panel fold, F-DS13): resolved via the same repo-root confinement `read_file`/`write_file` use, so an absolute or `../`-laced value that escapes the repo root throws `BRIEF_OUTSIDE_REPO` before the file is ever read — `.cursor/`-rooted briefs (the documented norm) are repo-relative and pass through unaffected. The entry throws on any fault (`--repo`/`--brief` validation, `POLICY_INVALID`, a corrupt claims registry) before any ledger `run_start`; a thin wrapper sets `process.exitCode` (no `process.exit()`). Exit code 0 only for `completed`/`delegated_to_claude`.


### C.6 Multi-worker isolation (F13, ratified by operator 2026-09-22) — write scope, reserved registries, active claims

The engine is designed to run in several git worktrees at once (one engine per worktree, one worktree per task). Git gives filesystem + index isolation for free; the three layers below make the *task* boundary mechanical rather than orchestrator discipline. Evaluation order inside the tool layer: C.1.3 confinement → C.1.4 self-protection → **C.6.2 reserved registries → C.6.1 write scope** → C.1.5 secrets → C.1.6 read-state.

1. **Write scope (per task, declared in the brief).** The brief file opens with a front-matter block (`---` … `---`) containing `write_scope:` followed by one `- <glob>` line per entry (globs relative to the repo root; `**` = any depth, `*` = within a segment; parsed by the engine's own minimal parser — no YAML library). Every declared glob is normalised (Step 9 panel fold, F-DS7: leading `./` stripped, backslashes → forward slashes, repeated slashes collapsed) before use. `write_file`, `edit_file` and every `git_commit.paths` entry must match at least one scope glob, else `blocked` with code `PATH_OUT_OF_SCOPE`. Reads and `grep_files` are unrestricted by scope (a worker must read its neighbours). Provider `deepseek` with an empty or missing `write_scope` ⇒ downgrades to `claude` per §B (commit 11: `provider_source: 'fallback:no_write_scope'`, see below) rather than refusing to start. **Directory-anchored (Step 9 panel fold, F-II2):** every glob's first path segment must be a literal (no leading `*`/`**`) — an unanchored glob has an EMPTY prefix, which `scopesOverlap` (below) would then treat as conflicting with every other run's scope anywhere in the repo. The one exception is the bare, literal `**` — the documented, everywhere-used "the whole repo" scope; its empty prefix genuinely SHOULD overlap everything, for a run that honestly claims the entire tree. An unanchored glob (`*.md`, `**/foo.ts`) is refused before a claim is acquired: ledgered as a `run_start` naming the bad scope + an `error` record (`SCOPE_GLOB_UNANCHORED`) + a `run_end{status: 'scope_invalid'}`, exit code non-zero — a refuse-to-start, not a §B downgrade (the brief's scope is malformed, not "provider unavailable"). `run_start` records `write_scope`.
2. **Reserved registries (orchestrator-only).** `exec-policy.json` gains `registry_reserved`: `scripts/manifest.json`, `scripts/steps/_schema/converted.json`, `scripts/steps/_schema/converted.json.pending`, `scripts/steps/_schema/step-archetype-census.json`, `scripts/steps/_schema/programme-items.json`, `scripts/steps/_schema/template-freeze.json`, `scripts/steps/_schema/schema-baseline.json`, `docs/specs/00-architecture/00_system_map.md` — and `claude_only_globs` — the §B money/auth/PII/migrations downgrade, enforced as `PATH_CLAUDE_ONLY` (F14, commit 11): `migrations/**`, `src/app/api/**/auth/**`, `src/app/api/**/billing/**`, `src/lib/auth/**`, `src/lib/billing/**`, `.github/workflows/**`. Writes ⇒ `PATH_RESERVED` (registry) or `PATH_CLAUDE_ONLY` (claude-only) even when the brief's `write_scope` names them — scope cannot widen past policy; evaluated in the same position, immediately after C.6.2. Consequence for the conversion protocol: the engine ships a step's code and tests; the orchestrator's landing commit performs the registry edits (`converted.json` registration, the `pending` deletion, system-map regeneration). Both lists are data (they may grow), unlike the C.1.4 denylist, which is code.
3. **Active claims registry.** `<ledger_dir>/active-claims.json` — a JSON array of `{ claim_id, run_id, pid, repo_root (realpath), branch, write_scope, ts }`, guarded by `<ledger_dir>/active-claims.lock`. At `run_start` the engine (a) drops entries whose `pid` is dead (`process.kill(pid, 0)` throws `ESRCH`; `EPERM` — the pid exists but signalling it is denied — means ALIVE, Step 9 panel fold F-DS4), (b) refuses to start with `CLAIM_CONFLICT` if any live entry has the same `repo_root` and an overlapping scope, or the same `branch` (non-null on both sides — a `null` branch, e.g. a detached-HEAD checkout, F-DS14, never matches another `null`, so only `repo_root` can conflict two detached runs) and an overlapping scope; overlap = the literal directory prefix of one glob (everything before its first `*`, both globs NORMALISED first — leading `./`, backslashes, repeated slashes — and folded case-insensitively on win32, F-DS7) is a prefix of the other's — this prefix rule over-approximates on same-literal-stem siblings (`src/foo*.ts` vs `src/foo/*.ts` are flagged even though they are disjoint; fail-closed, accepted, not fixed — the actual false-positive class, an unanchored glob's empty prefix, is closed upstream by C.6.1's anchoring requirement instead), (c) appends its own claim. `active-claims.json` is written ATOMICALLY (tmp file + rename, F-DS2); a present-but-unparsable file THROWS `CLAIMS_CORRUPT` (the run refuses to start — mutual exclusion is never silently treated as absent) rather than being read as an empty registry, which stays the correct behaviour ONLY for a genuinely missing (`ENOENT`) file. `active-claims.lock` carries `{pid, ts}` (Step 9 panel fold, F-DS5): contention reclaims by confirmed-dead pid first, mtime staleness ONLY as a fallback when the lock's own pid field is unreadable; release removes the file only if its pid still matches ours. The claim is removed at `run_end` (any status, wrapped in `try/finally` so an escaping throw — e.g. a ledger write failure — still releases it promptly rather than waiting for process exit, F-DS6) and on process exit (defense-in-depth backstop); a crashed engine's stale claim is reclaimed by (a). This file is separate from the ledger and is written by `scripts/lib/exec-claims.js`, so `exec-ledger.js` stays truncate-free.
4. **Committer lock is per worktree.** C.1.8's advisory lock is `<ledger_dir>/committer-<first 12 hex of sha256(realpath(repo_root))>.lock`, so engines in different worktrees commit independently; the one-committer-per-branch rule is carried by the claims registry's `branch` overlap check, not by the lock.

Ledger additions: `run_start.write_scope`, `run_start.claim_id`; `run_end.status` gains `claim_conflict`; block codes `PATH_OUT_OF_SCOPE`, `PATH_RESERVED`, `CLAIM_CONFLICT`, `PATH_CLAUDE_ONLY` (F14, commit 11). **Amended §B, commit 11:** a `deepseek` run with an empty/missing `write_scope` is NO LONGER a `no_write_scope` terminal `run_end.status` (Phase 2 commit 10b's original, non-zero-exit design) — §B forbids a halt, so it is now a `provider_source: 'fallback:no_write_scope'` downgrade to `claude`, landing on the ordinary `delegated_to_claude` path (exit 0), exactly like `engine_unavailable:no_api_key` (no `DEEPSEEK_API_KEY` and no injected/transcript client). `NO_WRITE_SCOPE` as a block CODE no longer exists; the reason lives only in `provider_source`.
...
|------|----------------------|----------------|
| **Code Reviewer** | `code-reviewer-grounded` · worktree | "Correctness, type-safety/no-`any`, missing telemetry/logging, naming/pattern consistency, dead code, error-path/envelope handling, mutation idempotency." |
| **Observability** | `observability-reviewer` · worktree | "Audit-row completeness vs spec; verdict cascade row-derived not parallel-boolean; §11 counter scoping; producer/consumer records_meta contracts (Spec 47/48); no state mutation without an observable trail; no swallowed error." |
...
| **Idempotency Lens** | `general-purpose` · MAIN tree + live code (+read-only DB) · fires on ANY plan/diff touching a destructive write, backfill, transaction boundary, re-runnable script, or upsert (elevated 2026-08-13 from the DeepSeek CLI lens-set to a full agent charter — the CLI form reviews whole files; this charter hunts a CLASS) | "You are the IDEMPOTENCY/COLLISION lens for <target>. This is the lens that caught F18 (a re-runnable script whose fixed-name backup DESTROYS its own prior rollback) and the READ-COMMITTED backup-atomicity race (a row entering the predicate between backup and UPDATE is nulled without being backed up). Hunt that class: (1) same input twice → byte-identical state? name every unconditional stamp/trigger that breaks it; (2) transaction boundaries — what runs OUTSIDE the txn that the txn's correctness depends on? snapshot isolation level vs concurrent writers (name the actual scheduled writers and their times); (3) re-run lifecycle — does run 2 destroy, duplicate, or silently skip run 1's artifacts? dated names, early-idempotent returns, fail-closed collisions; (4) COLLISIONS between the plan's own folded decisions as a SET; (5) every test asserting idempotency — can it distinguish a guard-skip from a genuine no-op write? Findings [BLOCKING]/[SHOULD-FIX]/[NOTE] with file:line." *Provenance: C3 — F18 + the atomicity race (nobody else saw either) · C7 plan — the stuck-interim-accretion collision + Case B vacuity (co-found) · C7 fold — the same-day backup fail-closed deploy gap.* |
| **Cross-read Adversary** | `general-purpose` · MAIN tree · fires after ANY fold (§11.2) — plan folds, panel folds, correction batches | "You are the CROSS-READ ADVERSARY for the fold in <plan section>. The repo's recorded systemic lesson (`tasks/lessons.md:117`): the worst defects are COLLISIONS between corrections adopted separately and never cross-read. Zero deference. (A) Take every decision the fold adopted and check them PAIRWISE — does adopting one invert, weaken, or contradict another? Think mechanically about what each DOES, not what it says. (B) Walk every execution-plan checklist line against the fold — a restated scope claim ('nothing else', 'three sites') dies the moment the fold expanded the edit set; a recorded verification query must be able to reproduce the fold's own findings. (C) Tests as a SUITE: shared fixtures, red-today vs lock-green-both stated per case, and no case asserting something the folded design doesn't produce. (D) List every load-bearing claim NO reviewer executed. Verdict FOLD ACCURATE / FOLD DEFECTS (+items)." *Provenance: invented 2026-08-13; C4 fold — 2 blocking; C7 fold — 7 defects incl. an unwritten SQL expression and a falsified census query.* |

> **SUBSTRATE RULE — a seat's agent type MUST grant the tools its lens requires (ratified 2026-08-16).**
> Every role in this table whose lens says "verify / execute / query / `git blame`" needs **Bash**. The
> three project-defined seats above (`code-reviewer-grounded`, `observability-reviewer`,
> `regression-guardian`, all in `.claude/agents/`) exist precisely because the `feature-dev:*` agent types
> they replaced **have no Bash tool** — they can Read and Grep but cannot run a command. A seat spawned on
> a tool-less substrate does not fail loudly; it silently downgrades to static reasoning and returns
> confident PASS verdicts it never executed, which is the §11.1 violation this spec exists to prevent,
> committed by the reviewers themselves. **Before composing a panel, check each seat's declared `tools:`
> against its lens.** Precedent: the same defect is already recorded for the CLI substrate at
> Operating-Model Compliance ("a tool-less CLI cannot audit decision truth", §5.8) — this generalizes it.
...
## 11. Grounded Verification Protocol — execute claims, validate folds (ratified by operator 2026-08-13)

> **Calibration scope:** this section is EVIDENCE-BASED FOR BACKEND/PIPELINE (DB/SQL/chain) work — the Phase A C1–C7 ledger below is its provenance. The general form that transfers to every domain: **execute what's executable; ground what's groundable; judge the rest.** See §11.5 for the frontend nuance (operator-ruled: this may not be the case for front-end work — do not port the roster mechanics there uncritically).

### 11.1 No unexecuted executable claim
A plan or fold may not contain a claim that COULD be executed but wasn't: a data claim carries its query **run** (host + `current_database()` printed — lessons `:83`); a SQL shape is **parse-tested** against a real DB before any reviewer approves it; a census grep runs at the **full scope its conclusion covers**; a line citation is **read**, not remembered. "Approved on paper" is not a verification state.
*Incidents:* C2's "397 permits" premise — one missing column (`permit_type`) survived four review rounds (2026-08-12) · C7's `$16` CASE — THREE review passes approved SQL that Postgres rejects (`inconsistent types deduced`); Case C2's red caught it by executing (2026-08-13) · C7's Q1 census — the plan's recorded query could not reproduce the plan's own fourth-writer finding (fold-validation #1).

### 11.2 Fold-validation is a STEP, not a courtesy
After folding any review round, and BEFORE implementation: **one grounder re-executes every claim in the fold** + **one cross-read adversary checks the folded decisions PAIRWISE and walks every checklist line for staleness**. Folds are read as a SET — the worst defects are collisions between corrections adopted separately (`tasks/lessons.md:117`). Sub-rules with incident bodies: **a fold must be exercised in every execution mode the file has** (the C4 `!pool` guard redded plain-Husky mode — cross-reading could not catch it, only running could) · **an expanded edit set kills every restated scope claim** ("the three SET terms. Nothing else." survived one fold past its own falsification).
*Yield:* C4 fold-validation — 2 blocking (four-chain watchdog scope; the dropped `PIPELINE_CHAIN` harness line). C7 fold-validation — 10 defects incl. an unwritten fourth SQL expression and a falsified census query. Orchestrator folds averaged ~5 defects each across the session; the validation pass caught every one at the cost of two agents.

### 11.3 Red-first mechanics
Every lock is proven in BOTH directions (fails on the old shape, passes on the new) before red is claimed — a lock that was never red against the defect it names is not a lock (the 2026-08-13 envelope regex matched nothing; its "red" was the not-found branch). Red must land at the DESIGNED assertion; red-for-the-wrong-reason is a test defect found NOW. Survive-the-fix locks (green-both) are stated as such per case.

### 11.4 Tests live in the plan; the plan panel reviews them
The failing tests — cases, fixtures, red/green expectations, invocation mode — are written INTO the plan and reviewed at PLAN altitude, collapsing the separate test-panel round. *Incident:* C7 Case B's hash-guard vacuity was caught by three blind reviewers BEFORE the test file existed; the authored test then enforced the corrected design.

### 11.5 Roster consequences (Backend/Pipeline) + the frontend nuance
* **Lean grounder-first PLAN panel (3–4 seats):** Integration (main tree, live DB — the session's highest hit-rate seat: refuted C2's premise, found the live fourth writer, ruled the `--file` harness) · **Idempotency lens** (agent charter — owns replay/transaction/collision classes: F18, the backup-atomicity race, the stuck-interim-accretion collision) · **Regression Guardian, ALWAYS provisioned with shell+git** (fence archaeology: `f238b814`, the `c6bf9f1d` founding-intent ruling, Spec 41's fiction-from-birth, the inverted runSync-history claim) · + Reality-Check when derived values change (subject-matter trigger, §10.3).
* **Cross-read Adversary is a NAMED role** (spawn: *"Hunt collisions between the plan's separately-folded decisions — pairwise, mechanically, with zero deference; walk every checklist line for staleness against the fold; verify the tests still cohere as a suite."*).
* **CLI models (Gemini/DeepSeek raw) are DEMOTED to whole-file audit generators.** Their yield files to `review_followups.md`, never folds into a diff review. Evidence 2026-08-13: two CLI diff reviews produced ZERO diff findings; one produced a critical-sounding verdict phrase that did not reproduce and matched no line finding; DeepSeek re-raised the C1-refuted `PG_HOST` claim verbatim. Their pre-existing-defect inventories remain genuinely valuable (the close-stale safety-guard bundle) — that is their job now. Always grounder-adjudicated (§9).
* **Lean OUTPUT panel: Guardian + one grounded reviewer.** The C7 five-seat output round produced exactly two novel catches, both reachable by that pair.
* **§11.5-FE (operator nuance, 2026-08-13):** for Admin/Expo/frontend work, §11.1 applies to the domain's executable surface — typecheck, UI-regression suites (`*.ui.test.tsx`), render tests, a running app — but visual/UX/interaction-quality claims are NOT executable; reviewer judgment, the non-pipeline 3-reviewer panel, and the Spec 33/99 conventions carry the weight there. Fold-validation is recommended-lean (one validator) for multi-round frontend folds and mandatory only where a fold asserts executable facts. Do not demand queries of a design decision.

### 11.6 Why this is the EFFICIENT shape, not just the safe one
Grounding first means the plan panel argues about a true map instead of discovering the territory mid-round; tests-in-plan merges two panel rounds into one; fold-validation replaces "another full round" (historically low-yield — eight reviewers missed what one cross-read caught) with two targeted agents (which caught everything). C7 ran ~11 agent passes plan-to-ship under the old shape; this protocol's shape is ~7 for the same defect coverage. Reviewer METHOD diversity beats reviewer count.
