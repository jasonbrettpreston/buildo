...
| `run_end` | `status: completed\|aborted\|killed\|budget_exhausted\|delegated_to_claude\|hook_failed\|claim_conflict\|scope_invalid` (the last two, §C.6.3/F-II2, are refuse-to-start outcomes recorded alongside a `run_start`, never after a tool has dispatched), `iterations`, `usage_total`, `tool_calls_total`, `blocked_total`, `commits: string[]`, `duration_ms` |
