...
Ledger additions: `run_start.write_scope`, `run_start.claim_id`; `run_end.status` gains `claim_conflict` and `no_write_scope`; block codes `PATH_OUT_OF_SCOPE`, `PATH_RESERVED`, `CLAIM_CONFLICT`, `NO_WRITE_SCOPE`.
