...
| `'supplier'` | one product-trade slug (their product's trade, e.g. `glazing` for a window manufacturer) | **P24 (2026-07-11).** Architecturally identical to a tradesperson — a single trade, the standard feed. `account_preset='supplier'` is a UX/billing hint only; it never branches the feed. **EXPLICIT-ONLY:** set by admin provisioning or the audited Spec 21 join-editor re-label — never derived from the trade (self-serve derivation stamps `tradesperson`; Spec 21 §4 v2). May hold a multi-trade set via `trade_slugs_override` (the SELECTED-TRADE model — see §2.5.2). |
...
- Determines `restored_status`:
  - `account_preset = 'manufacturer'` → `'admin_managed'` (comp account, independent of Stripe — skips the live read)
  - All others → **the live Stripe status** (WF3 2026-07-14 ruling): because delete schedules PERIOD-END cancel, the sub stays live until period end. If a `stripe_customer_id` is stored, read the customer's subscriptions and restore `deriveEffectiveStripeStatus(subs)` (`'active'` / `'past_due'` / `'expired'`) — a within-period reactivator keeps the access they already paid for. `cancel_at_period_end` is NOT cleared (no un-cancel — access lasts the remaining paid period, then lapses to `'expired'` via the period-end `.deleted` webhook unless they re-subscribe). **LOUD-NON-FATAL:** any Stripe failure/unconfigured, or no stored customer id → fall back to `'expired'` (never blocks reactivation; a later webhook/reconcile corrects it). Prior `trial` does not resume.
- Sets `account_deleted_at = NULL`, `subscription_status = restored_status`, **`last_stripe_event_at = NOW()`**, and **`stripe_cancel_failed_at = NULL`** atomically (WF3 2026-07-14 round-2):
  - `last_stripe_event_at = NOW()` **re-stamps** the out-of-order watermark to the reactivation instant — it is NOT cleared to `NULL`. On the live-restore path the `stripe_customer_id` is UNCHANGED, so the webhook's superseded-sub fence (`$1='active' OR stripe_customer_id IS NOT DISTINCT FROM $2`) passes for every same-customer event and provides no freshness protection; the watermark is then the ONLY guard against a stale/delayed same-customer event downgrading the freshly-restored status. `NULL` would disable that guard (`last_stripe_event_at IS NULL` → always true); `NOW()` keeps it forward-only (the genuine future period-end `.deleted` still applies; stale events are rejected). Regression Guardian, Spec 20 §4.2/§6.
  - `stripe_cancel_failed_at = NULL` retires moot delete-time cancel debt (the user is staying).
- Returns 200 with only the CLIENT_SAFE column list (not `RETURNING *` — P24-24A leak fix). Client receives `subscription_status` = `'active'` / `'past_due'` (live-restore), `'expired'` (no live sub / Stripe fallback), or `'admin_managed'` (manufacturer); the subscription gate handles routing. Response SHAPE is unchanged (existing field, values the client already handles) — non-breaking for the Expo consumer.
- Returns 400 if recovery window has passed (> 30 days). Returns 400 if account is not in deletion state.
- Try-catch + `logError`.
- **Known Failure Mode (Regression Guardian F3, LOW — accepted):** if the delete-time `cancelAllStripeSubscriptions` call had FAILED (Stripe outage → `stripe_cancel_failed_at` was set, `cancel_at_period_end` never scheduled), the sub is fully live with no pending cancellation. Reactivation clears `stripe_cancel_failed_at` and live-restores `'active'` → the account continues as a normal paid subscription with no scheduled cancel. Benign — it favors the user (they reactivated; there is nothing left to retry-cancel against), not a money-loss direction.
