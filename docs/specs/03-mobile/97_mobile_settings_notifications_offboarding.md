...
- On confirm: client calls `POST /api/user-profile/reactivate` (Spec 95 Step 3b — dedicated reactivation endpoint). The server determines `restored_status` and applies atomically:
  ...
  - All other presets → restore to `'expired'` (the original subscription was scheduled to cancel at period end on deletion; reactivation also clears `stripe_cancel_failed_at` + `last_stripe_event_at`, both scoped to that superseded subscription; the user re-subscribes via `buildo.com` with a fresh customer id — Spec 20 §4.2/§6)
    - **⚠️ RULED 2026-07-14, PENDING WF3 (not yet built):** because delete now schedules *period-end* cancel, a within-period reactivation should **restore `'active'`** when the customer still has a live Stripe sub (read live status; `deriveEffectiveStripeStatus`), falling back to `'expired'` only when no live sub remains or Stripe is unreachable. `cancel_at_period_end` stays scheduled (access lasts the remaining paid period, then lapses). This paragraph describes CURRENT behavior (always `'expired'`); update it when the WF3 lands. See `review_followups.md` "New-agent empirical validation" D3.
