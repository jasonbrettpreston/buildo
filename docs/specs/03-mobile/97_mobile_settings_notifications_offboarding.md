...
- On confirm: client calls `POST /api/user-profile/reactivate` (Spec 95 Step 3b — dedicated reactivation endpoint). The server determines `restored_status` and applies atomically:
  ...
  - All other presets → restore **the live Stripe status** (WF3 2026-07-14 ruling): because delete schedules *period-end* cancel, the sub stays live until period end, so a within-period reactivation restores `'active'` / `'past_due'` when the customer still has a live sub (`deriveEffectiveStripeStatus`), falling back to `'expired'` only when no live sub remains, no customer id is stored, or Stripe is unreachable (loud-non-fatal). `cancel_at_period_end` stays scheduled — access lasts the remaining paid period, then lapses to `'expired'` via the period-end `.deleted` webhook unless the user re-subscribes. Reactivation also clears `stripe_cancel_failed_at` + `last_stripe_event_at` (scoped to the superseded subscription; the webhook's superseded-sub fence is the primary guard — Spec 20 §4.2/§6). See Spec 95 §6.4 Step 3b + `review_followups.md` "New-agent empirical validation" D3.
