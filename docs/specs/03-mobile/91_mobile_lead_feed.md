...
> **Wire-up status (2026-05-06; updated 2026-05-11):** the `'realtor'` `trade_slug` is recognized by mobile onboarding (`mobile/src/lib/onboarding/tradeData.ts`, `mobile/app/(onboarding)/profession.tsx`, Spec 94 §3.1 trade list + §4 Path R) and as of **WF3 #realtor-backfill (2026-05-11)** the backend data layer is also wired end-to-end: items 1-4 of §3.5 below have shipped. Item 5 (Maestro flow end-to-end) is queued behind the parked WF1 #C admin inspector UI build.
...
The realtor persona's mobile UX is wired today (Spec 94 §3.1 trade list + §4 Path R; trade picker entry; onboarding path). The backend data layer landed across two phases: items 1-3 in WF2 Cycle 7 (commit `2901fcd`, 2026-05-06), item 4 in WF3 commit `779ec88` (2026-05-09 — 3-axis gate refinement), and item 4's recurring-chain wiring + backfill-script bug fixes in **WF3 #realtor-backfill (2026-05-11)** — the original 2901fcd script was merged but never run end-to-end; three latent bugs (NULL `lead_score` crash, manifest gap, advisory-lock collision) prevented it from producing a single row. The WF3 #realtor-backfill commit fixed all three plus a fourth latent bug (Finding 4: SQL bypassed the 3-axis gate) that only became observable once the crash was fixed.

Item 5 (end-to-end Maestro coverage) is queued behind the parked WF1 #C admin inspector UI build.

1. **`TRADES` array entry.** **SHIPPED 2026-05-06 (commit `2901fcd`).** `src/lib/classification/trades.ts:51` — `{ id: 33, slug: 'realtor', name: 'Real Estate Agent', icon: 'Key', color: '#EC407A', sort_order: 33 }`. Mobile already references `slug='realtor'` in `tradeData.ts:76`.
2. **DB `trades` table seed.** **SHIPPED 2026-05-06 (`migrations/118_realtor_trade.sql`).** Appends the `id=33, slug='realtor'` row to the `trades` table; also seeds the `trade_configurations` calibration row (`bid_phase_cutoff:P1, work_phase_target:P19` — see item 3).
3. **`trade_configurations` calibration row.** **SHIPPED 2026-05-06 (mig 118 docstring records the product decision).** Resolution: option (a) — single `work_phase_target`. Realtors care about two temporal extremes (listing at intake, closing at occupancy); rather than split into two trade slugs or extend the schema, the calibration uses `bid_phase_cutoff:P1` (earliest visibility — listing prospecting begins at permit intake) + `work_phase_target:P19` (latest tracked phase short of terminal P20 — same lifecycle row naturally ages from on_horizon → departing_soon → action_required as the build progresses, covering the closing-prospecting use case via temporal grouping rather than algorithm branching).
4. **`permit_trades` association — every-eligible-active-permit row (option (a)).** **SHIPPED 2026-05-11 (WF3 #realtor-backfill).** Recurring chain step 14 in `chains.permits` via `scripts/backfill-realtor-permit-trades.js`. Idempotent; advisory lock 114; 3-axis gate matching `shouldAppendRealtor` exactly. Row-count cost: ~95K active rows on dev DB (residential-non-commercial-construction subset of the ~219K active total — the 3-axis gate dropped the original "all active" estimate from item 4's pre-WF3-`779ec88` contract by ~57%).
   - **Rejected alternative — SQL bypass:** modifying `getLeadFeed` to short-circuit the `permit_trades` JOIN when `trade_slug='realtor'` would be an algorithm branch. **Explicitly rejected** because it violates §1.2; documented here only so future cycles don't re-discover and re-litigate. The rejection holds regardless of operational cost — if the row-count cost of (a) becomes truly infeasible, the right response is to amend §1.2 (a deliberate architectural change), not to silently break the invariant in `getLeadFeed`.
   - **Latent-bug history (WF3 #realtor-backfill 2026-05-11):** the original `2901fcd` script had four bugs that prevented it from producing any rows — Finding 1: NULL writes to `permit_trades.lead_score` tripped PG 23502; Finding 2: not registered in `scripts/manifest.json`; Finding 3: advisory lock 91 collided with `link-massing.js`; Finding 4: SQL bypassed the 3-axis gate from `shouldAppendRealtor`. All four had the same root cause (merged without an end-to-end run). Lesson captured in `tasks/lessons.md`.
...
> **WF2 #2 amendment (2026-05-08):** the realtor TradeMatch is now also gated on `permit_type_class === 'construction'` per Spec 80 §5. A sign permit, fee deferral, or fire-upgrade permit does NOT generate a listing opportunity, so `appendRealtorMatch` is skipped for those classes. The §1.2 invariant is preserved — the gate branches on `permit_type_class` (DB-derived axis), NOT on `account_preset` (persona axis, explicitly rejected per Spec 95 §2.5.1). Item 4 above (every-active-permit row) still applies, but only for construction-class permits — which is 95.5% of dev DB rows, so the operational cost concern is mostly unchanged.

> **WF3 amendment (2026-05-09):** the realtor gate is now 3 axes within the construction class — `permitClass === 'construction'` AND `permit_type ∈ REALTOR_RELEVANT_TYPES` (5 residential building types) AND `'commercial' ∉ scope_tags`. Trade-only permits (PLB, MS, DSS), demolition (DM), non-residential, and commercially-scoped permits no longer receive realtor. Item 4's row-count estimate updates: only the residential-non-commercial subset of construction permits gets a `(permit_id, 'realtor')` row (~95K of dev DB rows, down from ~219K under the WF2 #2 contract). See Spec 80 §5 "Realtor sub-gating" sub-table for the full contract; live-DB regression-locked by `src/tests/db/realtor-gating.db.test.ts`.
