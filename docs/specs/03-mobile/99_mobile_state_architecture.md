...
>     // No newer write landed — safe to roll back.
>     useFilterStore.getState().setRadiusKm(ctx.previous);
>   }
>   // Otherwise: a newer mutation/hydrate already updated this field.
>   // Last-write-wins; do not clobber.
> },
> ```
>
> **Alternative: per-field version counter.** Each `onMutate` increments a counter associated with the field; `onError` only restores `previous` if the counter at error-time matches the value captured during `onMutate`. More precise (handles the edge case where a concurrent write happens to set the same value as the optimistic — the equality check above misses that case), but more elaborate. Default to re-read-before-rollback; reach for the version counter if the equality miss-case becomes a real bug.
>
> **For deep-equal fields** (e.g., `homeBaseLocation`), the equality check uses `fast-deep-equal/es6` per §6.6 — same comparison used by the optimistic write's `set` bail-out.
>
> **Code-side amendment is filed as a follow-up WF3** in `docs/reports/review_followups.md` — `mobile/src/hooks/usePatchProfile.ts` §9.16 implementation currently uses the naive pattern; updating to re-read-before-rollback is out of scope for this WF2 (doc-only).
...
| 9.24 | ✅ DONE (spec); code-side filed as follow-up WF3 | B3 rollback race amendment per WF2 M1+M2+M3 #7 (DeepSeek). Documents the optimistic-rollback race (concurrent write between `onMutate` and `onError` gets clobbered) + recommended mitigation (re-read-before-rollback / last-write-wins) + alternative (per-field version counter). Cross-references `mobile/src/hooks/usePatchProfile.ts` §9.16 as the canonical B3 site that should adopt the mitigation in a follow-up WF3. | WF2 doc-only (spec); WF3 follow-up (code) | `docs/specs/03-mobile/99_mobile_state_architecture.md` §4 B3 Rules subsection |
