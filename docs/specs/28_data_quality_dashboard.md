...
- **Core Logic:**
  ...
  - Pipeline chain orchestrator (`scripts/run-chain.js`): 3 chains (permits=14 steps, coa=6, sources=14). All three chains bookend with CQA: `assert_schema` (Tier 1) runs first as pre-ingestion gate, `assert_data_bounds` (Tier 2) runs last as post-ingestion validation. Sequential execution with stop-on-failure. API chains use 1-hour timeout.
  - Continuous Quality Assurance (CQA): Tier 1 pre-ingestion schema validation (`scripts/quality/assert-schema.js`) checks CKAN datastore metadata for permits/CoA expected columns, CSV headers for address_points/parcels, GeoJSON property keys for neighbourhoods, and URL accessibility for massing shapefiles — runs before data load to catch upstream drift. Tier 2 post-ingestion data bounds (`scripts/quality/assert-data-bounds.js`) runs SQL checks after all processing: permits (cost outliers, null-rate thresholds, referential audits, duplicate PKs), source tables (address_points row count + duplicate IDs, parcels row count + duplicate IDs + lot size bounds, building_footprints row count + height bounds, neighbourhoods count >= 158 + duplicate IDs). Tier 3 engine health (`scripts/quality/assert-engine-health.js`) queries `pg_stat_user_tables` for all monitored tables and checks: dead tuple ratio > 10% (VACUUM not keeping up), sequential scan ratio > 80% on 10K+ row tables (missing/unused indexes), update ping-pong (cumulative updates > 2x inserts). Snapshots engine stats to `engine_health_snapshots` table (migration 051) for trending. All three tiers log results to `pipeline_runs` table and appear as "Quality" group entries in FreshnessTimeline.
  ...
  - **Telemetry Labelling & Expected Ranges:** Two distinct metric tiles are clearly labelled: (1) **"DB State Changes"** (`TelemetrySection`) shows observed PostgreSQL mutations from `pg_stat_user_tables` (raw SQL operation counts); (2) **"Script Output"** (Last Run tile for non-funnel steps) shows self-reported values from `PIPELINE_SUMMARY` (logical record counts). Each tile displays a descriptor explaining what its numbers represent. Per-step expected behavior ranges (`STEP_EXPECTED_RANGES` in `src/lib/admin/funnel.ts`) provide inline range badges (green checkmark = normal, yellow warning = borderline, red X = anomaly) and a violet behavior note explaining why numbers look the way they do (e.g., neighbourhoods showing ~2054 SQL UPDATEs is expected from Census enrichment). Range status computed via `getRangeStatus(value, [min, max])` with 20% borderline margin.
  - **Raw DB Transparency (T1-T5):** Dashboard renders pipeline state directly from `pipeline_runs` DB values. Status indicator (`getStatusDot()`) maps 1:1 from DB `status` column (running→blue, completed→green, failed→red, skipped/cancelled→gray) — no client-side stale detection. Time-based freshness badges (`getFreshnessBadge()`) are decoupled from status (Fresh/Recent/Aging/Overdue based on hours since `last_run_at`). T1 (row count deltas), T2 (pg_stat ins/upd/del diffs), T4 (NULL fill rates) rendered inline via `TelemetrySection` in `DataFlowTile`. T3 (live table counts from `pg_class.reltuples`) rendered as `LiveStateBar` in chain headers. T5 (historical sparklines from `/api/admin/pipelines/history`) rendered as inline 40×16px SVG in pipeline tile header rows, lazy-loaded on accordion expand and cached in a ref to avoid re-fetching. Telemetry data stored in `records_meta.telemetry` JSONB (migration 041). See Spec 37 §3.7 for SDK telemetry contract.
  - **Pipeline history API:** `GET /api/admin/pipelines/history?slug={slug}&limit={N}` returns last N runs (1-50) for sparkline rendering. Includes `duration_ms` and `status` per run.
...
- `src/components/funnel/FunnelPanels.tsx`
