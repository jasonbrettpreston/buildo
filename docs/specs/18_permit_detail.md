...
- **Core Logic:**
  - Single API call to `GET /api/permits/{id}` returns permit record, trade matches, change history, builder data, parcel data, neighbourhood profile, massing data, and CoA applications (see `src/app/api/permits/[id]/route.ts`).
  - Page renders 8+ sections: Header (address, status badge, lead score), Property Photo (Street View with dev-mode placeholder), Property Details (lot size, frontage, depth from parcel data with irregular-lot detection), Building Massing (footprint, stories, height, coverage from Spec 31), Project Details (all permit fields plus scope tags as badges), Timeline (application/issued/completed dates and construction phase), Builder/Owner (enriched contact info, Google rating, WSIB), Trade Matches (sorted by lead_score DESC with tier, confidence, phase status), Change History (chronological, max 50 entries), and CoA link (hidden if no match).
