...
> **D4 was half spec-staleness and half CODE DEFECT; both halves are now closed.** The spec text ("a live WARN counter") was wrong and was fixed at this reconciliation pass; the counter's *vacuity* — `const … = 0` while `parcelFamilyFromZoning`'s fall-through to `'all'`/`norm_basis:'pre_r2'` silently affected **105,595** R/RA/RAC parcels (measured live) — was **defect `CPCE-D3`**, **✅ CLOSED 2026-09-21** (`docs/reports/defect-ledger.md`, WF3 CPCE-D1/D2/D3 commit 2) — see §2.11. Two further pins, cross-referenced in place for the same reason, are also **✅ CLOSED 2026-09-21**: **`CPCE-D1`** (the undatable-rates-vs-undatable-index asymmetry, §2.9, commit 1) and **`CPCE-D2`** (menus stranded outside the `R%` population by a later re-zoning, §2.1, commit 3).
...
**The population — stated, not implied (D8).** "Residential parcel" is exactly `WHERE zoning_class IS NOT NULL AND upper(zoning_class) LIKE 'R%'`, ordered by `id` (**437,279** parcels, measured 2026-09-21). There is **no incremental mode and no skip path**: every invocation streams that whole population and re-prices it (§2.11); the 16-column `IS DISTINCT FROM` write guard is what keeps a steady-state re-run's write count near zero, not a gate.

**S0.2 — the PRODUCT-scope bound (WF3 existing-structure-area-artifacts, 2026-09-21/22, `tasks/lessons.md:30`, Spec 124 R-⟨next⟩).** "Residential parcel" alone is not the model's own population — a parcel `enrich_parcels`' max-build pass has ALREADY ruled out of the product model (`max_buildable_gfa_sqm IS NULL` — lot outside `[max_build_lot_min_sqm, max_build_lot_max_sqm]`, Spec 65 §4 MB-2) had its reno lines priced anyway: the two passes disagreed on "is this parcel in the model." The FULL in-scope population is now `zoning_class LIKE 'R%' AND max_buildable_gfa_sqm IS NOT NULL AND (cur_floor_gfa_sqm IS NULL OR cur_floor_gfa_sqm <= product_scope_max_existing_gfa_sqm)` — the residual cut (`product_scope_max_existing_gfa_sqm`, default **750 m²**, exclusive) removes the large-existing-structure tail that survives even inside the max-build model (§2.4's own p999=716/max=1,739 distribution). `buildSourceSql`/`buildZoneSql` (both config-driven, not baked literals) carry this bound identically so the stream and its own observability never disagree with each other either. Measured live 2026-09-21/22: **18,529** parcels excluded by the primary (lot-band) cut + **322** by the residual cut = **18,851** total, **$80,430,829,443.26** of `cost_gut_total` removed (of which $77,593,312,011.51 is the primary cut's own share); `cost_gut_total > $50M` **103 → 0**; `> $10M` **2,129 → 0**; surviving max **$8,875,649.17** (well under the pre-fix $20.6M ceiling the primary cut alone would have left).

**`CPCE-D2` CLOSED (2026-09-21), WIDENED by S0.2.** A parcel re-zoned *out* of `R%` after a menu was written used to keep its stale, category-wrong menu forever — `writes[0]`'s own `WHERE` simply stops selecting it, and it silently kept pricing a non-residential parcel with residential archetype rates. Fixed in its own commit (Spec 123 §3.1): a second declared write target (`outputs.writes[1]`, class `set_based_null_retract`) retracts (NULLs) the 16 cost/FSI columns of any parcel outside the population that still carries one, via `ctx.retract(1, [])` (Spec 124 R-AK), called first in the pass, guarded by "carries at least one of the 16 values" (so an already-clean row is never rewritten) and self-extinguishing (`idempotent_rerun:"zero_writes"`). A `recovery.before_image:"generated"` JSONL captures the retracted rows' pre-image before the UPDATE (R-M). S0.2 WIDENED the SAME target's `scope` (OR-extended, R%-guarded so it cannot overlap the original non-R% arm — never a second retraction mechanism, FOLD-I1) to also cover the new product-scope exclusions. `no_cost_outside_population` (a FAIL invariant, evaluated post-retraction) reads 0 on every run including the first, against the WIDENED population definition.
...
- **The absence rule, exactly (D8):** a line is omitted when `area === null || area <= compute_parcel_cost_min_priceable_area_sqm` (seed `0` ⇒ today precisely "NULL or non-positive"), or — defensively, since the rate rows are `NOT NULL`/`CHECK > 0` — when no `archetype_cost_rates` row exists for that line's archetype. A parcel on which *every* line is absent (but the parcel is still IN the product scope, §2.1) stores the bare `{"_schema_version": 1}` sentinel (never NULL, never `{}`) and is counted in `null_geom_basis_count` — **7,674 parcels / 1.755 %** of the population, measured 2026-09-21.
- **S0.2 — a THIRD, distinct state: scope-excluded.** A parcel OUTSIDE §2.1's product-scope bound (not in `R%`, or out of the max-build model, or existing GFA over `product_scope_max_existing_gfa_sqm`) is never streamed at all by `writes[0]` — its `parcel_cost_menu`/16 scalars are left exactly as `writes[1]`'s retraction sets them: **SQL `NULL`, not the `{"_schema_version":1}` sentinel.** This is deliberately a different state from line-absence: "no computable line on an in-model parcel" (sentinel, §2.4 above) and "not a product-model parcel at all" (NULL, this bound) answer different questions and must not be collapsed into one reading — a consumer checking `parcel_cost_menu IS NOT NULL` already distinguishes them correctly without a schema change.
...
- **`cost_rates_stale`** — `MAX(as_of_date)` over `archetype_cost_rates` vs `cost_rates_stale_months`. Value is a severity code: **`0`** fresh (PASS) · **`1`** stale **or undatable** (`MAX(as_of_date) IS NULL`, WARN, never a silent PASS — **CPCE-D1 CLOSED**, symmetric with `cost_index_stale` below) · **`2`** `as_of_date > run_at` (**FAIL**). The human-readable `false` / `true` / `'undatable'` / `'future_dated'` string rides in the row's `detail` (which `verdict.js#checkRow` prefers over the value when rendering). The numeric form is load-bearing: a boolean or a bare string is not `Number.isFinite` and would score "unevaluable" rather than WARN/FAIL.
...
**`CPCE-D1` CLOSED (2026-09-21).** The undatable arm was asymmetric: an undatable *index* WARNed, but an undatable *rate table* (`MAX(as_of_date) IS NULL`) silently PASSed. Fixed in its own commit (Spec 123 §3.1) — both arms now WARN/`'undatable'` identically. Both arms remain unreachable today — the three underlying columns are `NOT NULL` and an empty rate table HALTs earlier — and a live-DB precondition lock reds if a future migration relaxes any of them (at which point both arms are already symmetric; no further fix is owed).

### 2.10 Propagation to permits/coa (Spec 48 → Spec 49)
The `cost_*` + FSI scalars join the dominant-parcel propagation in `enrich-permits.js` via a new **`COST_PROP_COLS`** set (the §4D mechanism — like `OPT_COMP_PROP_COLS`). The `parcel_cost_menu` JSONB stays **parcel-scoped**. Propagation emits **Spec 48** per-column audit rows (enrich-permits step) → **Spec 49** `assert-global-coverage.js` rows on THREE surfaces: `parcels` (GATED ≥85% of residential-with-building — the ~9% building-less are an exclusion *filter*, not a numerator note), `permits` (Step 9b), `coa_applications` (CoA Step 4b — INFO, sparse where parcel-unlinked). Migrations add the `cost_*`/FSI cols to permits + coa (guarded ADD COLUMN, like mig 204). **CPCE-D2 CLOSED:** a parcel's `CPCE-D2` retraction rides this SAME `COST_PROP_COLS` set with no propagator-side change needed — `parcel-cost-cols.js`'s own contract already has the 15 scalars ride the generic `= NULL` orphan-nullify path (`scripts/lib/parcel-cost-cols.js:9-11`), so the next `enrich_permits` run re-selects the (now-NULL) dominant-parcel columns and propagates the NULL onward.
...
**Audit rows — 24 declared checks, all ALWAYS emitted including at value 0**, plus 3 declared row-level assertions (`no_priced_null_lot_parcel`, `new_build_cost_not_gt_coa_cost`, `no_cost_outside_population`) and the SDK-injected `sys_*_duration_ms` (one per invariant) / `sys_velocity_rows_sec` / `sys_duration_ms` (32 rows total on a clean run, measured 2026-09-21).

| Row(s) | Severity | Meaning |
|---|---|---|
| `residential_parcels_examined` | FAIL below `compute_parcel_cost_min_population` | the population actually streamed — **S0.2 (2026-09-21/22):** now the PRODUCT-scope population (§2.1), not the bare R%-zoning population; measured 410,754 of 437,279 R%-zoned parcels post-fix (18,851 excluded) |
...
| `unmapped_residential_family_fallback_count` | WARN, strict `viol == 0` bound, `retighten_when` declared (**CPCE-D3 CLOSED**) | COUNT of residential parcels whose zoning falls through `parcelFamilyFromZoning` to the `'all'` bucket (R/RA/RAC) — a standing, structurally non-zero population until Spec 88 P2's family-aware norm read lands; measured live 2026-09-21: 105,595 of 437,279 |
| `stranded_cost_rows_retracted` | INFO (**CPCE-D2 CLOSED, WIDENED S0.2**) | COUNT of parcels this run's `ctx.retract(1, [])` call NULLed — CPCE-D2's own converging run measured 2; S0.2's own converging run (widened `scope`, the SAME counter per R-AT/FOLD-I1, no parallel counter) retracted 18,851 (measured before/after: $80,430,829,443.26 of `cost_gut_total` alone), 0 in steady state, legitimately non-zero again after a future re-zoning or max-build recompute |
...
| `no_cost_outside_population` (invariant) | FAIL, `viol == 0`, evaluated post-retraction (**CPCE-D2 CLOSED, WIDENED S0.2**) | parcels OUTSIDE the product-scope population (§2.1 — not just non-R%) carrying any cost value — 0 on every run including the first, since the retraction has already run in the same pass |

**Counters that are `records_meta` keys, not audit rows.** The conversion relocated these — same values, different home, no data loss: `null_geom_basis_count`, `area_confidence` (`{high, medium, low}`), `fit_gated_suite_count`, `fit_gated_garage_count`, `cost_rates_age_months`, `cost_index_age_months`, `rates_max_as_of_date` (**this is the correct name — never `as_of_date`**), `cost_escalation_index`, `records_updated`, `records_skipped`, plus the resolved `config` snapshot, the `rates_as_of`/`index_updated_at` stamps and a `cost_by_zone` block (7 named zones + `other`, each with `parcels`/`menus`/`empty_menus`/`p50_cost_fb`/`max_cost_gut`, whose Σ-identity against `residential_parcels_examined` is asserted in code, not merely declared — **S0.2:** `buildZoneSql` now applies the SAME product-scope bound `buildSourceSql` does, or the Σ-identity throws by construction on the very mismatch this bound introduces, measured live 2026-09-22 before the fix: Σ 437,279 !== scanned 410,754). `unmapped_residential_family_fallback_count` and `stranded_cost_rows_retracted` moved OUT of/into this list per the relocation rule (§2.11's rule moves a counter *up* into the audit table, never *out* of `records_meta` — both keys still ride `records_meta` too, now carrying their real values). The legacy's `dry_run` row is retired with the flag; `parcels_with_menu_pct` is superseded by the two coverage checks above.

**`CPCE-D3` CLOSED (2026-09-21).** `unmapped_residential_family_fallback_count` was **structurally `0`** (`const … = 0`) — a vacuous green while `parcelFamilyFromZoning`'s fall-through to `'all'` / `norm_basis: 'pre_r2'` silently affected **105,595** R/RA/RAC parcels (measured live 2026-09-21). Fixed in its own commit (Spec 123 §3.1): the counter now measures the real fall-through, via a Σ-identity coded in `runCostMenuPass` (detached+townhouse+multiplex+unmapped ≡ scanned) that reddens the run on a bucketing defect instead of under-reporting. Per Spec 124 operator adjudication (R-H's own precedent form, link_massing LM-D6/LM-D11): the new `checks[]` entry is WARN with a strict `viol == 0` bound and a declared `retighten_when` — **no new logic variable** (a high-water-mark ceiling was considered and rejected: it would read PASS on the exact population the defect describes). Will retighten to 0 when Spec 88 P2's family-aware norm read lands.
