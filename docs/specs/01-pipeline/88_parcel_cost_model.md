...
**`CPCE-D2` CLOSED (2026-09-21).** A parcel re-zoned *out* of `R%` after a menu was written used to keep its stale, category-wrong menu forever — `writes[0]`'s own `WHERE` simply stops selecting it, and it silently kept pricing a non-residential parcel with residential archetype rates. Fixed in its own commit (Spec 123 §3.1): a second declared write target (`outputs.writes[1]`, class `set_based_null_retract`) retracts (NULLs) the 16 cost/FSI columns of any parcel outside the `R%` population that still carries one, via `ctx.retract(1, [])` (Spec 124 R-AK), called first in the pass, guarded by "carries at least one of the 16 values" (so an already-clean row is never rewritten) and self-extinguishing (`idempotent_rerun:"zero_writes"`). A `recovery.before_image:"generated"` JSONL captures the retracted rows' pre-image before the UPDATE (R-M). Measured live 2026-09-21: 2 parcels (373779 CR, 11891 O) retracted on the converging run; `no_cost_outside_population` (a new FAIL invariant, evaluated post-retraction) reads 0 on every run including the first.
...
| `stranded_cost_rows_retracted` | INFO (**CPCE-D2 CLOSED**) | COUNT of parcels this run's `ctx.retract(1, [])` call NULLed — 2 on the converging run, 0 in steady state, legitimately non-zero again after a future re-zoning |
...
| `no_cost_outside_population` (invariant) | FAIL, `viol == 0`, evaluated post-retraction (**CPCE-D2 CLOSED**) | non-R% parcels carrying any cost value — 0 on every run including the first, since the retraction has already run in the same pass |

**Counters that are `records_meta` keys, not audit rows.** The conversion relocated these — same values, different home, no data loss: `null_geom_basis_count`, `area_confidence` (`{high, medium, low}`), `fit_gated_suite_count`, `fit_gated_garage_count`, `cost_rates_age_months`, `cost_index_age_months`, `rates_max_as_of_date` (**this is the correct name — never `as_of_date`**), `cost_escalation_index`, `records_updated`, `records_skipped`, plus the resolved `config` snapshot, the `rates_as_of`/`index_updated_at` stamps and a `cost_by_zone` block (7 named zones + `other`, each with `parcels`/`menus`/`empty_menus`/`p50_cost_fb`/`max_cost_gut`, whose Σ-identity against `residential_parcels_examined` is asserted in code, not merely declared). `unmapped_residential_family_fallback_count` and `stranded_cost_rows_retracted` moved OUT of/into this list per the relocation rule (§2.11's rule moves a counter *up* into the audit table, never *out* of `records_meta` — both keys still ride `records_meta` too, now carrying their real values). The legacy's `dry_run` row is retired with the flag; `parcels_with_menu_pct` is superseded by the two coverage checks above.
