...
**Audit rows — 23 declared checks, all ALWAYS emitted including at value 0**, plus 2 declared row-level assertions (`no_priced_null_lot_parcel`, `new_build_cost_not_gt_coa_cost`) and the SDK-injected `sys_duration_ms` / `sys_velocity_rows_sec`:
