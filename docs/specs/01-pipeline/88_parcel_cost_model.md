...
**Eight measured disagreements between this spec's §2.11/§2.5/§2.9/§2.8 and the as-shipped step (D1–D8, found grounding this reconciliation — the spec text below is NOT yet corrected for these; recorded here so the disagreement is visible, not silently absorbed):**

1. **D1 (highest):** the entire Phase-B-B3 run-ledger gate / SKIP summary / `COMPUTE_PARCEL_COST_FORCE_FULL` escape hatch is ABSENT from this spec's §2.11 (documented only in Spec 43 Core-Logic item 10); §2.11 never contemplates a `records_total: 0` SKIP run.
2. **D2:** `FSI_MAX_PLAUSIBLE = 99.999` (now `compute_parcel_cost_fsi_max_plausible`), the NULL-and-count disposition, and `fsi_implausible_count` are undocumented in §2.5/§2.11.
3. **D3:** §2.11's audit roster is missing 7 emitted rows (`residential_parcels_examined`, `parcels_with_menu_pct`, `fsi_implausible_count`, `cost_index_stale`, `records_updated`, `records_skipped`, `dry_run`) and misnames `as_of_date` (live: `rates_max_as_of_date`).
4. **D4:** §2.11 implies `unmapped_residential_family_fallback_count` is a live WARN counter; it is `const … = 0` (P1; family mapping is P2 — CPCE-D3).
5. **D5:** neither the empty-rates refuse-to-run (F1) nor the duplicate-archetype throw (F2) is documented in §2.9.
6. **D6:** §2.9 implies a separate future-dated FAIL row; the code folds it tri-state into `cost_rates_stale` (a numeric severity code + a human-readable `detail`).
7. **D7:** §2.8 lists 6 logic vars; the script reads 3 (`reno_kitchen_gfa_pct`/`reno_bath_gfa_pct` belong to `enrich-parcels.js`; `min_comp_count` belongs to Spec 78) — load-bearing, because the descriptor may declare only what this step actually reads.
8. **D8:** the `upper(zoning_class) LIKE 'R%'` population predicate and the `area <= 0 → line absent` rule are unstated in §2.1–§2.5.

Closing D1–D8 in the spec body is out of this cutover's scope (a prose-only pass over §2.1–§2.11, not a behaviour change) — filed as a follow-up in `docs/reports/review_followups.md`.
