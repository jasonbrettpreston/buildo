...
> **P25 AMENDMENT — evaluator/enqueuer truth (this script is NOT the sender).** As of P25 (Spec `101_notification_dispatch.md`) `update-tracked-projects.js` is an **evaluator + ENQUEUER**, not a delivery mechanism. Its `INSERT INTO notifications` rows are queue entries (`is_sent = false`); the **only** component that delivers a push is the gated `dispatch_notifications` chain step, which reads this queue and delivers each eligible row to Expo EXACTLY ONCE per `(user, lead, type, America/Toronto-date)` via the `notification_dispatches` ledger. This script is the **second enqueuer** (the first is `classify-lifecycle-phase.js`, which P25 flipped from a direct sender to an enqueuer of `LIFECYCLE_PHASE_CHANGED` / `LIFECYCLE_STALLED` / `START_DATE_URGENT`). The sticky "memory" columns (`last_notified_urgency` / `last_notified_stalled`) remain THIS script's cross-run dedup for the CLAIMED-status evaluator; they are cleared when the condition clears (stall recovery → `false`; urgency recedes → `null`) so a stuck flag cannot permanently silence a pair (locked by `src/tests/notification-sticky-reset.logic.test.ts`). **Eligibility (saved vs claimed):** this evaluator only fires for CLAIMED-state trackers; mobile savers persist to `lead_views.saved` and are materialized into `tracked_projects` by the Step-0 self-feed (KFM-1). The `dispatch_notifications` sender is INERT until `logic_variables.notifications_dispatch_enabled = 1` (seeded OFF; see the runbook `docs/runbook/notification_dispatch_smoke.md`).
...
**`notifications.permit_num` polymorphism for CoA:** the 3 new notification subtypes (`COA_HEARING_IMMINENT`, `COA_DECISION_RENDERED`, `COA_STALLED`) store the CoA `application_number` in the existing `notifications.permit_num` column (already nullable). Mobile app discriminates via `type LIKE 'COA_%'` prefix check. v4 CRIT-CC: fallback chain `coa_application_number || permit_num || 'unknown-coa'` prevents NULL in the column for malformed lead_id inputs. F.4 (Spec 76 §3.5 Lead Inspector CoA panel) may revisit this with a dedicated `notifications.lead_id` column. **P25 UPDATE:** the dedicated **`notifications.lead_id` column IS now delivered** (mig 218, P25 25A) — the canonical routing key that retires the `application_number`-in-`permit_num` polymorphism. Both enqueuers populate it (`permit:<num>:<rev>` / `coa:<application_number>`); the dispatcher reads it for the ledger key + the deep-link `entity_id`. The CoA subtypes remain FENCED (not dispatched) in v1 (no mobile CoA card yet) — recognised-and-skipped by `scripts/lib/notification-types.js` `FENCED_TYPES_V1`.
...
### KFM-2 — the `notifications` queue had NO sender (2026-07-11) — **RESOLVED (P25, dispatch consolidation)**

> **FIXED 2026-07-11 (P25 25A/25B).** For its entire life this spec's `notifications` queue (mig 010 — `channel` / `is_sent` / `sent_at`, a queue *shape* built for a sender) was **INSERTed into but never read by any delivery mechanism.** The rich Spec 82 taxonomy was enqueued and then stranded: no code path turned an `is_sent = false` row into an Expo push. Meanwhile a SEPARATE, hardened sender lived inside `classify-lifecycle-phase.js` (direct HTTP to Expo, its own 3 types), with a PROVEN double-send (that step runs in BOTH daily chains and `START_DATE_URGENT` had no cross-run memory). **Two disconnected systems.** P25 consolidates them: `classify-lifecycle-phase.js` and THIS script both become ENQUEUERS to the one queue; a single gated `dispatch_notifications` step is the ONLY sender, delivering once per `(user, lead, type, Toronto-date)` via the `notification_dispatches` ledger (mig 218). The double-send is now structurally impossible (the ledger UNIQUE). See `docs/specs/01-pipeline/101_notification_dispatch.md`. **Fenced in v1:** the CoA subtypes (`COA_*`) and `NEW_HIGH_VALUE_LEAD` are recognised-and-skipped (no mobile surface yet). **Residual (pre-existing, NOT P25 — filed):** the Step-4 `lead_analytics` sync INSERT omits the NOT-NULL `lead_id` (mig 141); because Postgres enforces NOT NULL *before* the `ON CONFLICT` redirect, that INSERT throws on any non-empty `tracked_projects` — masked in dev only because the table is empty. Needs a one-line fix (add the `lead_id` projection to both branches) under its own WF3.
