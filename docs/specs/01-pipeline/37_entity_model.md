...
- **Inputs:** Permits (builder_name), CoA applications (applicant), WSIB registry entries. Data migration from existing `builders` table.
  > **IMPLEMENTATION STATUS (verified 2026-07-29):** the CoA input path is NOT YET IMPLEMENTED — entities are extracted solely from `permits.builder_name` (`scripts/extract-builders.js`); `load-coa.js` contains no entity code and CoA currently yields zero entities. CoA extraction is DEFERRED (operator ruling 2026-07-29, reaffirming Spec 42 "SKIP v1"): the live CKAN feeds publish NO applicant names (Active resource lacks CONTACT_NAME entirely; Closed resource carries it blank; cloud DB holds 16 relic rows). A tripwire audit row in `load-coa.js` (`applicant_names_present`, WARN-on-presence) signals the moment names reappear — that is the build trigger for this input path.
- **Core Logic:**
