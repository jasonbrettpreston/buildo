...
> **Note (2026-05-13, chain membership re-derived 2026-09-15):** the note as first written described a 12-step "current state" with a PRE-permit placeholder and no scope/trade/cost classification on CoAs; §6's Phases C–G have since shipped. WF2 SPECTBL-1 re-derived **chain membership, position and step count** — §2's count, flow diagram and Step Breakdown table, §5's target-files step numbers, and §4/§5's `create_pre_permits` / `assert_pre_permit_aging` references — from `manifest.chains.coa` (16 steps). That is the scope of the re-derivation and nothing wider: the rest of §3–§5's prose (row-count figures, the "6 unique CoA-side records" validation claim) was NOT re-measured, and §6 Implementation Plan is untouched — it remains the dated design record for the #coa-pipeline-parity work, including its still-deferred items (e.g. §6.12 `link_similar`) and its step numbers as they stood when it was written.

**Trigger:** `node scripts/run-chain.js coa` or `POST /api/admin/pipelines/chain_coa`
**Schedule:** Daily
**Steps:** 16 (sequential, stop-on-failure) — count DERIVED from `manifest.chains.coa`, single source of truth
...
| 4 | `link_coa_to_parcels` | `link-coa-to-parcels.js` | Links CoA applications to parcels via Tier 1a/1b **address-only** matching (§6.5 step 9; the bridge path `address_points → parcel_address_points → parcels` runs ahead of the legacy parcels-table exact query, disambiguated by the uniform 3-level rule `address_class_desc` Structure > Structure Entrance > Land > smallest `ST_Area(geom::geography)` > `address_point_id ASC`). Bundles two derived passes per CoA — point-in-polygon `neighbourhood_id` off the matched parcel's centroid, and a lat/lng back-fill from `parcels.centroid_lat/lng` — plus the `parcel_linked_at = RUN_AT` marker, under a per-record SAVEPOINT (§6.11.1). **No Tier 2 spatial cascade exists, by construction:** CoAs carry no lat/lng before this step, so a Tier-1 miss has no coordinate to fall back on. Writes lead-keyed rows (`lead_id = 'coa:' || application_number`) per §6.6.B Option C | lead_parcels, coa_applications |
| 5 | `enrich_coa_zoning` | `enrich-permits.js` (`ENRICH_TARGET=coa`) | **Spec 66 WF3** — copies the dominant linked parcel's zoning by-law feed (class, FSI, coverage, height, exception, `variance_context` jsonb) onto `coa_applications` via `lead_parcels`, joined on the stored `coa.lead_id`. Always-full relational join; emits the F-H12 `coa_zoning_class_coverage_pct` gate (FAIL <80). Runs immediately after `link_coa_to_parcels` — the manifest order is load-bearing (§6.5 step 9b) | coa_applications |
| 6 | `classify_coa_scope` | `classify-coa-scope.js` | Description-keyword scope classifier (§6.5 step 5; §6.6.D). Writes 5 derived columns — `coa_type_class`, `project_type`, `scope_tags` (~30-tag reduced taxonomy), `scope_classified_at`, `scope_source='description'`. A NEW script rather than a reuse of permit-side `classify-scope.js`, which reads `permit_type`/`structure_type`/`work`/`current_use`/`proposed_use`/`storeys`/`housing_units` — fields CoA does not have. Pure classifier lives in `scripts/lib/coa-scope-classifier.js` with the Spec 84 §7 dual-path TS twin. Unmapped-rate threshold is a WARN, not a FAIL (`logic_variables.coa_scope_unmapped_threshold_pct`) | coa_applications |
...
**Table reconciled + drift-locked (WF2 SPECTBL-1, 2026-09-15, `.cursor/wf2_spec43_step_table_active_task.md`):** the table above is now a full re-derivation from `manifest.chains.coa` by a `node` walk (never hand-counted). **Six** live steps were missing, not five: the note above (and the `review_followups.md` filing behind it) named `link_coa_to_parcels`, `enrich_coa_zoning`, `classify_coa_scope`, `classify_coa_trades` and `compute_coa_cost_estimates`, but **`compute_phase_calibration` (live position 15) was also absent** — even though this spec's own §5 target-files list (`scripts/compute-phase-calibration.js — step 15`) and §4 "shares 9 steps with permits" line already named it. Two retired Phase-G steps were still listed and are now removed: `create_pre_permits` (old row 5) and `assert_pre_permit_aging` (old row 6) — both retired by §6.5 step 18 / §6.11 Phase G, and neither is in `manifest.chains.coa`; their scripts and the reasoning behind their retirement stay documented in §5 and §6. All 16 rows renumbered 1…16; the §2 step count (12 → 16) and the flow diagram were re-derived with them. Membership and position for the six additions came from the manifest and this spec's own §5/§6.5 rows; only the Purpose / Writes-To prose is new, sourced from each script's header + its domain spec (Specs 66/80/83/84/88), never from the slug name. The next drift is now uncommittable: the `system-map.infra.test.ts` lock under `src/tests/` ("chain-spec Step Breakdown tables" describe) locks the `#` / Slug / Script columns of all six chain tables against `manifest.chains`, both directions, in the existing pre-commit hook. Purpose and Writes-To stay hand-authored prose and are deliberately NOT locked (a generator would be a Spec 124 R-AA content move; filed as `SPECTBL-GEN`).

**Cutover note (batch1 I3 commit 9, 2026-09-14, PH-0/§5 spec-diff obligation):** `assert_engine_health` (row 12 — cited as "row 9" when written, against the then-drifted table; renumbered by WF2 SPECTBL-1, 2026-09-15) converted to the Spec 122 step standard, full nine-commit form (RECORDER archetype, Spec 124 R-AE — the `engine_health_snapshots` guarded upsert has no legal home under ASSERT's forced `outputs:"none"`). `scripts/quality/assert-engine-health.descriptor.json` is the behavioural contract from this commit forward. See `docs/reports/2026-09-14-batch1-i3-assert-engine-health-assessment.md` §9 and `.cursor/batch1_i3_assert_engine_health_active_task.md` §2.

**Trailing lifecycle classifier (step 13)** is the only path that routes
...
one exits cleanly with `skipped:true`. The phase distribution gate (step 14) uses its own
...
- **Infra:** `assert-coa-freshness.infra.test.ts` (`coa_freshness_warn_days` externalization §6.4; the WF2 P6.5 3-tier FAIL > WARN > PASS ladder)
<!-- The line this replaces claimed `quality.infra.test.ts` asserts "assert-coa-freshness exists, assert-pre-permit-aging exists". Measured 2026-09-15: that file contains no such assertion (it covers the /api/quality response shape and the snapshot schema), and `assert-pre-permit-aging.js` was deleted by Phase G. This block is NOT tool-maintained for this spec — `scripts/harvest-tests.mjs` skips any spec whose Target Files name no `src/tests/**.test.ts`, which is Spec 42's case — so it is hand-corrected here. -->
<!-- TEST_INJECT_END -->
...
- **Shared steps (current state):** `assert_schema`, `link_coa`, `refresh_snapshot`, `assert_data_bounds`, `assert_engine_health`, `classify_lifecycle_phase`, `assert_lifecycle_phase_distribution`, `compute_phase_calibration`, `assert_global_coverage` also appear in `chain_permits.md` — re-derived 2026-09-15 as the intersection of `manifest.chains.coa` and `manifest.chains.permits`, and locked against it (`system-map.infra.test.ts`, SHARED-STEPS PROSE arm). `create_pre_permits` stood on this line until then, three Phase-G-retirements late: §6.5 step 18 retired it, `scripts/create-pre-permits.js` is deleted, and it is in neither chain.
...
| 5 | `classify_scope` (`classify-scope.js`) | YES — NEW `classify-coa-scope.js` | Description-keyword classifier produces `coa_type_class`, `project_type`, `scope_tags`. Cannot reuse the permit-side script because permit `classify-scope.js` reads `permit_type`, `structure_type`, `work`, `current_use`, `proposed_use`, `storeys`, `housing_units` (`scripts/classify-scope.js:631`) — most of which CoA doesn't have. NEW SCRIPT. **NEW consumer (WF2 2026-07-06, Spec 83 §3-ARCHETYPE):** the archetype cost mapper reads these outputs at COST time to select the Spec-88 line (CoA rules: `NewConstruction`→`coa_build`; `Mixed`→tag dominance, tagless→`coa_build`) — no change to this step's outputs was needed. |
...
| 9 | `link_parcels` | YES — NEW `link-coa-to-parcels.js` | Spatial linkage to `parcels` polygons. Mirror of `link-parcels.js`. Writes to **unified `lead_parcels` table** (lead_id = `'coa:' || application_number`) per §6.6.B Option C. Also writes `coa_applications.neighbourhood_id` as a final UPDATE step (bundled — no separate `link-coa-neighbourhoods.js` script). NEW SCRIPT. **WF1 #parcel-address-bridge Phase 2e (2026-05-23, commit `986409e`):** new bridge-path query added BEFORE the existing legacy Tier 1a parcels-table query. Bridge path: `address_points ap → parcel_address_points pap → parcels p`, filtered `MAINT_STAGE=REGULAR + ADDRESS_STATUS=CURRENT` (NULL fallback), disambiguated by plan v4 fold H5/C2/F19 uniform 3-level rule (`address_class_desc` Structure > Structure Entrance > Land > `ST_Area(p.geom::geography) ASC` > `ap.address_point_id ASC`). On bridge hit: `match_type = 'address_points_exact'` (new value in `lead_parcels.match_type`), increments BOTH `tier1aExact` (F17 legacy preserved as rollup) AND `tier1aViaBridge` (new INFO sibling). Legacy Tier 1a fallback guarded by `!parcelMatch`. F17 audit_table: `tier_1a_exact` preserved + `tier_1a_via_bridge` INFO sibling row added. F16: verdict cascade upgraded from `hasWarn ? WARN : PASS` to row-derived per Spec 48 §3.6. F14: emitMeta reads expanded with `address_points` + `parcel_address_points`. |
| 9b | `enrich_coa_zoning` | YES — `enrich-permits.js` (`ENRICH_TARGET=coa`) | **Spec 66 WF3** — copies the dominant linked parcel's zoning by-law feed (class, FSI, coverage, height, exception, `variance_context` jsonb) onto `coa_applications` via `lead_parcels` (joined on the stored `coa.lead_id`). Always-full relational join; emits the F-H12 `coa_zoning_class_coverage_pct` gate (FAIL <80). Runs immediately after `link_coa_to_parcels` (manifest order). |
...
### 6.13 Decisions Log _(all open decisions resolved as of WF3 2026-05-19)_
...
- **Classifier method** for `classify-coa-scope.js` (formerly Q1 of §6.13 "open"): **resolved heuristic v1** during Phase D (DELIVERED 2026-05-14). Audit_table tracks ambiguous-classification rate; LLM v2 path remains available if heuristic accuracy < 80% on production data.
- **Geocoding** (formerly Q2 of §6.13 "open"): **resolved as DEFERRED** during Phase D per §6.6.X — CKAN's CoA dataset provides no GEO_ID FK; lat/long ownership shifted to `link-coa-to-parcels.js` + `link-coa.js`. Original bundle-into-load-coa.js recommendation superseded.
- **`lead_analytics.lead_key` rename to `lead_id`** (formerly Q3 of §6.13 "open"): **resolved as "leave-as-is"** — no rename, no alias view; external SQL queries continue using the `lead_key` column name. Cited as standing decision in the Phase I row.
- **Band recalibration depth** (formerly Q4 of §6.13 "open"): **resolved as 7 consecutive PASS runs** during Phase E.5 (DELIVERED 2026-05-16 commit `0d90571`). Promotion path codified in `assert-lifecycle-phase-distribution.js` per-kind posture flags.
