...
| **C** `staging_full_replace` | temp → `DELETE` → `INSERT…SELECT` — **legal, requires `why`** | 7 | `implemented` (INGESTOR prerequisite 0h, 2026-09-24 — `write.executeStagingReplace`, dispatched by `runIngestPhase`; RE-FREEZE #20) |
...
| 1c | ⚠️ **Check-query errors** | `execution.on_check_error` | `fail_step` · `warn_row` · `omit_row(+why)` — **NEW: today a dropped table is indistinguishable from a healthy one**. **EXECUTED since POST-B1-1 (2026-09-15, `scripts/lib/step/verdict.js` `checkRow`): `fail_step` = a FAIL row *regardless of the check's own severity* → step `failed` / terminal `fail_check` via the row-derived cascade (the declaration is per STEP, so it outranks the per-check severity; a step wanting an errored check tolerated declares `warn_row`); any value outside the frozen enum throws (Rule 12). An errored check is NEVER covered by `override.accept_anomaly` — acceptance prices a *measured* anomaly, and a query that threw measured nothing, so the row keeps an explicit `errored: true` marker (`checkRow`, errored arm only), stays in `unaccepted` (`index.js#partitionFailedRows`, shared by the status cascade and the `pre_write` gate) and lands the run on `failed` / `fail_check` rather than `completed_with_errors`. A `when:"pre_write"` check erroring aborts the write, and that abort now lands ONE `pre_write_gate` FAIL row (`value: "aborted: <ids>"`, `source: "gate"`) on the final audit table — before this, `failedPreWrite` / `write_skipped_pre_write_fail` had no consumer and a skipped write could end `completed`/PASS with no row saying so.** Before it, `fail_step` was the fall-through and inherited the check's severity — every assert-fleet check is WARN, so a thrown query read as an ordinary WARN |
...
>
> ⚠️ **C4 step H (2026-09-11) — the differential is complete AND fresh, by checker, on both sides.** Five halves, each enforced: POST completeness per declared chain (`derivedInvocations` over `manifest.chains` → G8 `missing-invocations`); PRE completeness (GOLD-PRE, `ec28216a`, declared gaps pinned); POST currency (R-C fingerprint → `stale-fingerprints`); zero unexplained diffs (deepest-leaf citation); and — new — PRE **freshness** (Spec 124 R-AC): `capture-step-golden.js --out` refuses to overwrite any capture git cannot restore unless `--overwrite` names a committed/indexed, worktree-clean file (e49bd594), and fast invariant #22 GOLD-PRE-FRESH hard-stops a converted slug whose `pre/` captures are untracked or locally modified (2f4bd3f7). One shared definition of "recoverable": tracked in the index + worktree == index. Programme item `C4-GATE`.
...
**RE-FREEZE #7 — warn_limit (2026-09-11):** `checks[]` gains two optional fields, `warn_limit` (same frozen string bound grammar as `limit` — `definitions.bound`'s string arm, byte-duplicated rather than `$ref`'d since `bound` itself is `x-frozen`) and `warn_limit_from_config` (same substitution semantics as `limit_from_config`) — a second, looser, config-driven bound giving a check 3-tier PASS/WARN/FAIL coverage in one row: `limit` ok → PASS, else `warn_limit` present and ok → WARN, else the check's declared severity; one row per metric, names unchanged, both tiers config-substituted. Operator ruling for batch1 I1 (`.cursor/batch1_i1_assert_global_coverage_active_task.md` Fold B). Payment record: `122a_step_optimization_appendix.md` Appendix §A9 (body text under the historical #1-#4 heading, after #5). Spec 124 §5 register row R-AD.

**RE-FREEZE #8 — the generic ENRICHER runner (2026-09-15, batch-2 Phase 0.10):** `execution` gains three fields — `heartbeat_minutes_from_config` and `lock_timeout_ms_from_config` (both REQUIRED on the ENRICHER profile; `runEnrichPhase` read both out of hardcoded `config.enrich_parcels_*` keys, so a second enrich-shaped step resolved both to NaN and ran with NO heartbeat ticker and NO `SET LOCAL lock_timeout`, silently — ER-D1, Spec 48 §3.6) and the optional `enrich_hooks` (the Spec 58 §9/§11 contract read and the Spec 122 §3.0b defer-scope decision, DECLARED instead of hardcoded to one step's compute exports). `phase_runners[runEnrichPhase].phase_order` also moves — the enrich path now reaches `write.buildWritePlan`/`writeBeforeImage`/`executeSetBasedClear`/`executeSetBasedJoinUpdate` through two runner-owned `passCtx` seams (`ctx.retract`, `ctx.joinUpdate`), which is what makes `recovery.before_image: "generated"` mean something on an enrich shape; it was a no-op there before. The four entries render immediately after `preWriteGate` because that is where the seam FACTORY is declared — it executes inside the phase loops, after `pipeline.withTransaction`. No other runner's `phase_order` changes. Both interval fields admit the literal `"none"` as a DECLARED disable (the same escape `execution.phases[].timeout_minutes_from_config` already carries) — an unresolvable NAME still throws, so the accident and the declaration never render identically; the ENRICHER exemplar fixture declares `"none"` for both. The per-target counter block is also generalized in the same row: fifteen hardcoded `write.targetKey(0..4)` assignments become a loop over `execution.phases[]` keyed by each entry's own `writes_ref`, so an ENRICHER with any number of write targets >= 1 runs (it used to throw `TypeError` after the transaction had already COMMITted), and a `writes_ref` past the declared array is a named throw. `enrich_parcels` declares neither class and `before_image: "none"`, so its own path is byte-untouched and its five committed goldens must hash-equal. Payment record: `122a_step_optimization_appendix.md` Appendix §A9 (after #7). Spec 124 §5 register row R-AK.

**RE-FREEZE #6 — EP-D17 (2026-09-11):** `execution.maintenance.txn_scope` enum gains `"step"` (one enum line + its description; `generate-schema-baseline --check` 0 new fields) so a step-scoped runner can declare the maintenance the library now executes (`runMaintenance`, Spec 122 §4.3). Payment record: `122a_step_optimization_appendix.md` Appendix §A9 (body text under the historical #1-#4 heading).

**RE-FREEZE #9 — the COLUMN-STAMPING LINK runner (2026-09-16, batch-2 I4 `link_neighbourhoods`):** `execution.shape`'s `x-frozen` enum gains a TENTH value, `link_column`, and `phase_runners` gains a NINTH runner, `runLinkColumnPhase` (Ask 1 ruling (B) FORK, re-ruled 2026-09-16). All three share-rungs (`link_keyed`, `link`, `backfill`) were refuted by EXECUTION and are recorded in the `shape` node's own `x-ruling.rungs_tried`, extended rather than overwritten. ⚠️ **The re-freeze is SPLIT across two commits** because `deriveArchetypeProfiles` iterates `converted.json.converted` and never reads `pending`: this commit carries `schema_sha256` + the new `phase_runners` row, and `archetype_profiles[LINK]` + the `template-freeze.infra.test.ts` shapes pin flip land at cutover. Record: `122a` §A9 (after #8).

**RE-FREEZE #10 — the ENRICHER POST-PHASE seam (2026-09-16, batch-2 Phase 0.10b):** `execution.enrich_hooks` gains ONE optional field, `post_phase` — the compute export `runEnrichPhase` calls once after the last phase, returning `{matched, compute}`. Completes RE-FREEZE #8, whose record named this residue: the region still ran three unconditional `parcels` COUNTs for every enrich step and called `computeAggregateRecordsUpdated` UNGUARDED — a `TypeError` after COMMIT. The hook NAME resolves above the phases; `matched` merges under a reserved-key refusal ENUMERATING the runner-owned keys; `matched.compute` is finite-or-throw and, with no hook, DERIVED over the DISTINCT `writes_ref` set only. ⚠️ A BARE `compute.*` counter source resolves NULL for every ENRICHER — declare `matched.compute.*` or `written.e<N>.*` (filed HIGH; §1.1 forbids fixing it here). `phase_order` unchanged; goldens recaptured, **Class A empty**. Record: `122a` §A9.

**RE-FREEZE #11 (2026-09-18, WF3):** `execution` gains 2 optional fields (RECORDER read-phase bound, Spec 118 cause B) — detail 122a §A9.

**RE-FREEZE #12 — execution.on_row_error_why (2026-09-21, WF2 "runner row-error policy", R-AX):** one additive field, `execution.on_row_error_why` (`$ref` `#/definitions/why`, carrying `x-ruling`), required exactly when `on_row_error` is `skip`|`quarantine` — the same conditional shape its three siblings (`on_batch_error_why`/`on_check_error_why`/`on_degrade_why`) already use. `on_row_error_max_pct` gains `x-banned-for-new: true` (stays declarable — R-X: a banned value is a declaration, never a deletion — but the two literal instances that ever existed, load_ravines 0.05 and compute_parcel_cost_estimates 0, are deleted in the same commit as a Rule 3 second-source-of-truth violation, the bound already living once as a registered logic variable behind a declared FAIL check). **F-1, fixed in this same commit:** the pre-existing `on_row_error`/`on_row_error_max_pct` conditional (`step.schema.json:1490` pre-fix) used a `properties`-only `then` arm where its three siblings use `required` — AJV-proven live (2026-09-21) that a `skip`/`quarantine` descriptor with NO bound at all validated. The conditional is rewritten to `required: ["on_row_error_why"]`, closing the gap and matching its siblings' contract for the first time. No enum widened, no value pruned, no archetype profile touched, no `phase_runners` order re-derived. `schema_sha256` re-derived by `--refresh` in this commit; 4 descriptors truthed and recaptured in the same commit (load_ravines, compute_centroids, enrich_parcels, compute_parcel_cost_estimates — see Spec 124 §5 R-AX for the per-step table). Record: `122a` §A9.

**RE-FREEZE #13 — INGESTOR CSV acquisition (2026-09-23, WF2 batch-2 row 3.1 prerequisite 0b, Fold B4):** two additive fields on `inputs.reads.externals[]` — `format` (`enum: ["shapefile_zip","csv"]`, `x-frozen`, `x-ruling`) and `csv_options` (`{bom: boolean, relax_quotes: boolean}`, `additionalProperties:false`, `x-ruling`), required-iff-`csv` via the item's own `allOf` (same `if`/`then` shape `cache`/`cache_why` already uses). `acquireExternal` (`acquire.js`) was dispatch-free before this brief — `downloadArchive → extractArchive → locateShapefile → parseShapefile`, unconditionally, for every declared external; three of the eight INGESTORs still to convert (address_points, parcels, load_wsib) are bare CSVs. `format` now selects the parser (branch on the declared value, never a URL-extension sniff — CLAUDE.md PD#10); an unrecognised value throws by name rather than falling through to the shapefile arm. `downloadArchive`/`hashThrough` (FENCE 0b230472) are untouched — bytes are hashed as they land on either branch. The new `parseCsv` mirrors `parseShapefile`'s `{features, badKey, nullGeometry}` shape, streaming `csv-parse` with the three options every CSV loader agrees on hard-wired (`columns`/`skip_empty_lines`/`relax_column_count`) and the two they disagree on read from the descriptor (`bom`, `relax_quotes`). **The `compute.shapeRecord` contract:** `runIngestPhase` requires a CSV-format INGESTOR's compute to export `shapeRecord(record) → columns | null` — refused by a named Error BEFORE the HEAD request (no network, no pool access) if the export is absent, the same "a mis-declared step must cost nothing" posture the `writes.length !== 1` guard already uses. `shapeRecord` maps ONE parsed CSV row to the column values the write plan binds (`geojson` is the geometry field name `columnValues`/`validateGeometries` read); `null` means "this row is not loadable," counted as the new `shaped_skipped` acquired-block field and dropped BEFORE `compute.dedupeBySourceId`, never silently. The shapefile arm is byte-identical to before — its features already carry `geojson`, so `shapeRecord` is never called on that path. **`format`'s own required-ness is scoped, not universal:** the item's `required` array does NOT list `format` (matching the existing `key_property`/`url` idiom, neither of which is unconditionally required either) — a second `allOf` entry requires it only when `kind === "http_file"` AND a non-empty `url` is declared, i.e. exactly the external `runIngestPhase`'s own `externals.find(e => typeof e.url === 'string' && e.url.length > 0)` will actually acquire. A documentation-only external with no `url` (assert_schema's shared reference list of the eight not-yet-converted sources, none of which the ASSERT archetype ever downloads) declares no payload format, because none is ever fetched — the field's own description says "REQUIRED, per record-bearing external," and a blanket requirement would have forced a fabricated value onto entries nothing reads. `load-ravines.descriptor.json` gains `"format": "shapefile_zip"` (required now that its external is record-bearing — nothing hidden). Five new locks in `step-library.logic.test.ts` (T1-T5, one new describe): `acquireExternal` parses a 5-row CSV fixture into `{[keyColumn]: key, record}` with `bad_key_count` 0 (T1); `csv_options.bom` is honoured in both directions on a BOM-prefixed fixture, not inferred (T2); `format:"shapefile_zip"` keeps the extract→locate→parse arm byte-identical (T3); AJV fails a `csv` external with no `csv_options` and an unrecognised `format`, and passes `load_ravines` (T4); `runIngestPhase` refuses a `shapeRecord`-less CSV compute before any download, and counts `shaped_skipped` correctly on the other direction (T5). `schema_sha256` re-derived by `--refresh` in this commit; `load_ravines` golden recaptured (its `fingerprint_inputs` name `acquire.js` and its own descriptor) — data byte-identical. Record: `122a` §A9.

**RE-FREEZE #14 — `outputs.writes[].geometry_kind` (2026-09-23, batch-2 row 3.1 prerequisite 0e):** one additive field, `geometry_kind` (`polygon|point`, `x-frozen`, `x-ruling` — declared, never sniffed: runtime derivation from `ST_Dimension` would silently widen `load_ravines`' polygon guard on a converted step with a golden, and a `geometry_columns` catalog read hides the contract in the DB), REQUIRED under the INGESTOR `x-profile` for any write whose `columns[].bind` is `wkb_geometry` — scoped to that profile because only `runIngestPhase` runs `validateGeometries`; LINK/CASCADE/MATERIALIZER plans bind geometry from server-side SELECTs and never read the kind (a plan-level throw REDed LW-D15 and Fold D, measured). `scripts/lib/step/write.js` `geometryValidationSql(keyType, kind)`: polygon arm byte-identical to the prior text; point arm extracts dimension 1 without `ST_Multi` and accepts `ST_Point`; the `collection_extracted/accepted/skipped_null/skipped_unsupported_type` vocabulary is unchanged. Founding case: `address_points` (class-A Point INGESTOR) crashed on its first write with `Geometry type (MultiPolygon) does not match column type (Point)`. `load_ravines` declares `polygon`, 5 goldens recaptured with `--compare` 0 differences. `generate-schema-baseline --check`: the field sits below the baseline's one-level grain (no change); `template-freeze --refresh` re-froze the hash.

**RE-FREEZE #15 — INGESTOR `archetype_profiles` proven with its second member (2026-09-24, batch-2 row 3.1 commit 9, `address_points` cutover):** no schema byte changed — this is a re-freeze of the FROZEN tier's derived `archetype_profiles` entry, not a new field. Before this commit `converted.json.converted[]` carried exactly one INGESTOR (`load_ravines`), so `generate-template-freeze.mjs --refresh` derived an empty `shapes`/`runners` pair for the INGESTOR profile (nothing to cross-validate a single member against) while still recording `first_step: "load_ravines"` and `proven: true`. Registering `address_points` as the archetype's second converted member lets the generator cross-check both descriptors' `execution.shape`/runner wiring and derive `shapes: ["ingest"]` / `runners: ["runIngestPhase"]` for real, the same shape every other multi-member archetype (LINK, ENRICHER, MATERIALIZER, MATCHER, BACKFILL) already carries. This is the R-PACE-1 unlock named in Spec 123 §7 row 9 / Spec 124 R-AH: `archetype_profiles.INGESTOR.proven === true` AND ≥2 converted members admits the COMPRESSED commit form for the archetype's remaining members (3.2–3.8, per `.cursor/batch2_p3_1_address_points_active_task.md`'s Form ruling). `frozen_at` moves to this commit's own sha; `schema_sha256` is UNCHANGED (no `step.schema.json` byte moved). `template-freeze --refresh` run in the same commit as the `converted.json` registration it depends on.

**RE-FREEZE #16 — shapefile records carry DBF attributes through `compute.shapeRecord`; `outputs.writes[].geometry_kind` gains `"line"` (2026-09-24, batch-2 Phase 3 prerequisites 0f+0g for rows 3.2 `load_centreline`, 3.6 `massing`, 3.7 `parcels`):** two changes, one additive schema byte. **0f (no schema byte — `shapeRecord` is a compute export, not a descriptor field):** `parseShapefile` (`scripts/lib/step/acquire.js`) now pushes `{[keyColumn]: key, geojson, record: props}` per feature — `record` carries the DBF properties object, the SAME field `parseCsv` already carries, so a shapefile INGESTOR with attribute columns (load_centreline: 15 DBF columns + a per-feature street-class filter; massing: footprint attributes) can bind them. `runIngestPhase` resolves `compute.shapeRecord(record, {geojson})` for EVERY format, not csv-only — the ONE hook for attribute columns and a per-feature classify/refuse (`shaped_skipped`, unchanged semantics). The two formats differ on absence, deliberately: a `csv` external with no `shapeRecord` is still a HARD ERROR (unchanged from RE-FREEZE #13 — a CSV owes a shape, the named throw is untouched); a `shapefile_zip` external with no `shapeRecord` is LEGAL and keeps today's behaviour exactly — `load_ravines` (no `shapeRecord` export) passes every feature through unchanged, byte-identical, no throw, `acquired.shaped_skipped` stays 0. **0g (additive):** `outputs.writes[].geometry_kind`'s frozen enum gains `"line"` — one sentence appended to the field's existing `x-ruling` ("line added 2026-09-24 for the LineString INGESTORs: toronto_centreline's consumer enrich-centreline.js requires a true LineString"). `scripts/lib/step/write.js`: `GEOMETRY_KIND_EXTRACT_TYPE.line = 2`, accepts `ST_LineString` only, mirrors the `point` arm's single-member collapse (`ST_NumGeometries(ST_CollectionExtract(repaired, 2)) = 1 THEN ST_GeometryN(…, 1) ELSE ST_CollectionExtract(…, 2)`) so a MultiLineString extract stays multi and scores `skipped_unsupported_type` rather than silently merging; polygon and point arms stay byte-identical (pinned). Six new locks in `step-library.logic.test.ts` (T1–T6, one new describe at the end): `parseShapefile` on the existing `load_ravines` "missing-prj" fixture carries a named DBF attribute onto `record` (T1); `runIngestPhase` on a mocked `shapefile_zip` external whose compute exports `shapeRecord` shapes 2 of 3 features and counts `shaped_skipped===1` (T2); the same mock with NO `shapeRecord` export passes all 3 features through unchanged (T3); the csv hard-error arm is untouched (T4); `geometryValidationSql('line')` locks the extract-2 single-member collapse + `ST_LineString`-only accept + polygon/point byte-identity, and `assertGeometryKind` accepts `'line'`/throws on `'curve'` (T5); AJV accepts `geometry_kind:"line"` and still rejects `"curve"` (T6). A pre-existing lock in the adjacent "write.js geometry_kind" describe (0e's own T3) used `'line'` as its example of an unrecognised kind — fixed to `'curve'`, since `'line'` is now a real kind. `schema_sha256` re-derived by `--refresh` in this commit; `load_ravines` (5 goldens) and `address_points` (2 goldens) recaptured with `--compare` — data byte-identical, only nondeterminism-masked keys and the `migrations=` INFO line differ. Record: `.cursor/engine-briefs/b2-p3-c0f-shapefile-record.md`.

**RE-FREEZE #17 — `outputs.writes[].geometry_srid` (2026-09-24, batch-2 Phase 3 prerequisite 0i for row 3.6 `massing`):** one additive field, `geometry_srid` (`type: integer, minimum: 1`, `x-ruling`) — the SRID the SOURCE geometry is actually in, declared per write target, never sniffed. OPTIONAL: absent means the source is already 4326 (no transform), the case for every converted step today, so the field is additive-only and every prior descriptor is untouched. Measured 2026-09-24 (row 3.6 `massing` grounding): its CKAN shapefile is named `_wgs84` but its coordinates are Web Mercator (EPSG:3857); the legacy loader did `ST_Transform(ST_SetSRID(ST_GeomFromGeoJSON(geometry::text), 3857), 4326)`, while the pre-0i validator ran `ST_GeomFromGeoJSON(g.geojson)` with no SRID handling at all — a mislabelled source would have been mis-located silently. `x-ruling` rejects two rungs: deriving from the shapefile's `.prj` (exactly what is wrong on the founding case) and reading the target column's SRID from `geometry_columns` (that says where geometry must END, not where the source STARTS) — Rule 8: per write target, declared. `scripts/lib/step/write.js`: `geometryValidationSql(keyType, geometryKind, geometrySrid)` gains a fourth parameter — absent/4326 keeps the `input` CTE BYTE-IDENTICAL to the pre-SRID text (pinned); a validated integer ≠ 4326 rebuilds the geom expression as `ST_Transform(ST_SetSRID(ST_GeomFromGeoJSON(g.geojson), <srid>), 4326)`, interpolated numerically (never as a string) after `InvalidGeometrySridError` asserts it by name. `buildWritePlan` carries `plan.geometry_srid` (validated at plan time too) and threads it into `validation_sql`; `validateGeometries` re-asserts the plan's value as its own backstop for a caller that built `validation_sql` outside `buildWritePlan`. Four new locks in `step-library.logic.test.ts` (T1–T4, one new describe): the absent/4326 `input` CTE is byte-identical to the pre-0i text, including the exported `GEOMETRY_VALIDATION_SQL` constant and a no-arg `geometryValidationSql` call (T1); srid 3857 (and a second non-default SRID, 26917) render the declared transform with a bare numeric literal, never a quoted string (T2); AJV accepts `geometry_srid: 3857` on an INGESTOR write and rejects the string `"3857"` and `0` (minimum 1) while absent stays legal (T3); a non-integer/zero/negative/fractional plan value throws `InvalidGeometrySridError` by name from both `buildWritePlan` and `geometryValidationSql` directly (T4). `load_ravines` (no `geometry_srid` declared) stays byte-identical — no golden recapture needed for this field alone. `schema_sha256` re-derived by `--refresh` in this commit. Record: `.cursor/engine-briefs/b2-p3-c0i-geometry-srid.md`.

**RE-FREEZE #18 — `columns[].written` gains `"insert_only"` (2026-09-24, batch-2 Phase 3 prerequisite 0j for row 3.6 `massing`):** one additive enum value on the FROZEN `columns[].written` field (`x-frozen`, `x-ruling` extended with one sentence) — `step` (default, inserted AND updated), `insert_only` (inserted and bound, never rewritten by the conflict `DO UPDATE SET` or its `IS DISTINCT FROM` guard), `db_default` (neither, unchanged). Measured 2026-09-24 (row 3.6 `massing` grounding): `footprint_area_sqm`/`footprint_area_sqft` are computed DB-side after load and are INTENTIONALLY OMITTED from the legacy UPDATE SET (`scripts/load-massing.js`) — the pre-0j column model was BINARY (`step` in both the INSERT list and the SET, `db_default` in neither), so no value expressed "the step seeds this on INSERT and must never rewrite it"; under `step`, a reload would NULL/overwrite the DB-recomputed value on every conflict, and under `db_default` the seed value would never be written at all. `x-ruling` rejects rung (b) (special-casing the column name inside the generator — a domain literal `write.js` Gate 0 bans) and rung (c) (a validator rule cannot change which columns the generated SET writes). `scripts/lib/step/write.js`: `buildWritePlan`'s `stepColumns`/INSERT-and-`bindRow` list now includes `step` AND `insert_only` columns (the INSERT text is unchanged in shape — a wider membership, not a new branch); a new `changeableStepColumnNames` (the columns the update CAN change) feeds both `resolveGuardColumns`'s `all_declared` expansion and `updateColumns`, so an `insert_only` column can never be dragged into the `DO UPDATE SET` or an auto-expanded guard — structurally, not by a validator check. The plan gains `insert_only_columns` (the declared set, carried explicitly on every return shape); the generated statement renders the exclusion as a visible SQL comment (`-- seeded on INSERT, never rewritten by the conflict UPDATE (DB-recomputed): <cols>`) rather than a silent absence. `write_inventory`/plan-shape summaries need no separate change — `update_columns` already excludes the set, and `insert_only_columns` is now on the same plan object. Four new locks in `step-library.logic.test.ts` (T1–T4, one new describe): an `insert_only` column (`load_ravines`'s `source_dataset_version`, with `guard_columns` widened to `all_declared` for the test so the auto-expansion is exercised, not just an explicit list) is in the INSERT column list and `insert_only_columns`, and absent from `update_columns`/`guard_columns` and from the rendered SET/WHERE text (T1); a `step` column is pinned present in all three (T2); a `db_default` column is pinned absent from all three (T3); AJV accepts `written: "insert_only"` and rejects the hyphenated `"insert-only"` (T4). `load_ravines` (no column declares `insert_only`) stays byte-identical — no golden recapture needed for this field alone. `schema_sha256` re-derived by `--refresh` in this commit. Record: `.cursor/engine-briefs/b2-p3-c0j-insert-only-columns.md`.

**RE-FREEZE #19 — `columns[].on_empty:"preserve"` + `outputs.invalidates[].set_null_on_change_of` (2026-09-24, batch-2 Phase 3 prerequisites 0l+0m — operator ruling 2026-09-24, standardized/observable/scalable/simple):** two additive schema fields, replacing per-step compute-authored upsert SQL (`write_discipline.set_source:"compute"`, prerequisite 0k, RE-FREEZE-free) as the standard for an INGESTOR's two most common escape-hatch reasons. **0m:** `columns[].on_empty` (`enum: ["preserve"]`, `x-frozen`, `x-ruling`) — on conflict, an EMPTY incoming value (`''` after trim) keeps the stored value: the codegen emits `col = COALESCE(NULLIF(EXCLUDED.col, ''), <table>.col)` in the `DO UPDATE SET`, and the SAME column's change-detection guard clause becomes `(NULLIF(EXCLUDED.col, '') IS NOT NULL AND <table>.col IS DISTINCT FROM EXCLUDED.col)` — the legacy loaders' own NULLIF-guarded WHERE form (`scripts/load-parcels.js`, `scripts/load-address-points.js`), mirrored BYTE-FOR-BYTE rather than an algebraically-equivalent `IS DISTINCT FROM COALESCE(...)` rewrite, so a converted step's generated SQL can be pinned against the pre-conversion file. Only meaningful for a TEXT-typed column — the schema carries no per-column SQL type, so a non-text column declaring it is a runtime NULLIF cast error, not something the validator catches. **0l:** `outputs.invalidates[].set_null_on_change_of` (`$ref columnName`-shaped, `x-ruling`) — EXECUTED, unlike the base `{table, column, when}` entry (which stays declarative-only, e.g. `load_ravines`' own entry naming `parcels` while its write target is `ravines`): the codegen appends `<column> = CASE WHEN <table>.<watched>::jsonb IS DISTINCT FROM EXCLUDED.<watched>::jsonb THEN NULL ELSE <table>.<column> END` to the write target's `DO UPDATE SET`, and folds the watched column into that write's change-detection guard (deduplicated against `write_discipline.guard_columns`, placed FIRST — the founding case's own WHERE clause opens on the watched column), so the CASE arm's own comparison is live on every run that reaches the UPDATE. A watched column whose `columns[].bind` is `"wkb_geometry"` (a real PostGIS geometry value, which cannot cast to jsonb) compares with a plain `IS DISTINCT FROM` instead. VALIDATED (`scripts/lib/step/validate.js assertInvalidatesTargetMatchesWrite` — AJV cannot cross-reference two array entries on the same descriptor): an entry's own `table` must equal one of `outputs.writes[].table`, else a mismatched table would silently never fire (no CASE arm, no error, the stamp never nulled). Founding cases, measured 2026-09-24: `scripts/load-parcels.js`'s legacy UPSERT — five `COALESCE(NULLIF(EXCLUDED.<col>, ''), parcels.<col>)` preservations (mig 162 Day-1 critical-safety fix, WF1 #parcel-address-bridge) and three `<stamp> = CASE WHEN parcels.geometry::jsonb IS DISTINCT FROM EXCLUDED.geometry::jsonb THEN NULL ELSE parcels.<stamp> END` arms (DEC-FENCE2, #418) — reproduced from these two declared axes alone, whitespace-normalised, by a synthetic plan declaring the five `on_empty` columns + three `invalidates` entries + the seven `guard_columns` (T5, below); `scripts/load-address-points.js` at `git show 120b2b99` carries the same preservation on 12 columns (10 NULLIF-guarded TEXT + 2 NULL-form), which the converted default codegen silently dropped (AP-D7, fixed 2026-09-24 in `address_points`' own post-conversion fix commit — this commit is library-only). `buildWritePlan` gains `plan.on_empty_columns` (the declared set) and `plan.invalidated_on_change` (`{column, watched}` pairs, in declaration order) so a plan-shape summary (`write_inventory`) can list both axes without re-deriving them from the SQL text. Five new locks in `step-library.logic.test.ts` (T1, T2, T2b, T3, T4, T5, one new describe at the end): one `on_empty:"preserve"` column renders the COALESCE SET form and the NULLIF-guarded WHERE form, absent for every other column (T1); an `invalidates[]` entry with `set_null_on_change_of:"geom"` (a `wkb_geometry`-bound column) appends the CASE arm and folds `geom` into the guard, deduplicated against an explicit `guard_columns` entry (T2); the same against a `bind:"value"` watched column compares via `::jsonb`, and a foreign-table BASE entry (no `set_null_on_change_of`) is silently excluded from the rendered plan (T2b); `load_ravines` and `address_points` stay BYTE-IDENTICAL with neither axis declared (T3, pinned); AJV accepts both fields and rejects `on_empty:"keep"`, and the semantic layer rejects an `invalidates[]` entry naming a foreign table with `set_null_on_change_of` (T4); the legacy `parcels` UPSERT is reproduced whitespace-normalised from the two declared axes alone (T5) — the standard's proof of sufficiency. `schema_sha256` re-derived by `--refresh` in this commit. Record: `.cursor/engine-briefs/b2-p3-c0lm-declared-upsert-axes.md`. **0m follow-on (2026-09-24):** `columns[].on_empty` gains a second declared value, `"preserve_null"` — the NULL-form of the axis for a non-text column (no empty-string representation to `NULLIF` against): the codegen emits `col = COALESCE(EXCLUDED.col, <table>.col)` and the guard `(EXCLUDED.col IS NOT NULL AND <table>.col IS DISTINCT FROM EXCLUDED.col)`, mirroring `scripts/load-parcels.js`'s legacy `date_effective` arm byte-for-byte; two new locks in `step-library.logic.test.ts` — one `on_empty:"preserve_null"` column renders the NULL-preserving SET/guard forms with no `NULLIF` (T6), and T5's synthetic legacy-parcels plan extended with a `preserve_null` `date_effective` column reproduces the legacy UPSERT in FULL, minus only the conditional `${geomLine}` PostGIS axis (T7). `schema_sha256` re-derived by `--refresh` again in this follow-on commit.

**RE-FREEZE #20 — class C `staging_full_replace` executor + `checks[].on_warn:"skip_write"` (2026-09-24, INGESTOR prerequisite 0h, plan `.cursor/wf2_class_c_staging_replace_active_task.md` D1-D5 + Fold A F1-F6):** one additive schema field, `checks[].on_warn` (`enum: ["skip_write"]`, `x-frozen`, `x-ruling`, required-iff-`when:"pre_write"` via the item's own `allOf`), plus class C's registry flip (`write-class-disposition.json` `staging_full_replace` → `implemented`, no schema byte). **The executor (`write.js#executeStagingReplace`, dispatched by `runIngestPhase` on `write_discipline.class === "staging_full_replace"`):** ONE `pipeline.withTransaction` — `CREATE TEMP TABLE <t>_staging (LIKE <t> INCLUDING DEFAULTS INCLUDING CONSTRAINTS) ON COMMIT DROP`, batched staging INSERTs (the same `bindRow`/`maxRowsPerInsert` contract class A/B use), `DELETE FROM <t>`, `INSERT INTO <t> (...) SELECT ... FROM <t>_staging`; `carried.length === 0` short-circuits before the DELETE (`replace_skipped_empty_guard`), a structural defense in depth alongside the step's own DECLARED `pre_write` floor checks (Fold A F1). **The gate's third arm (Fold A F1):** before this the pre_write gate was binary (`abort` on an unaccepted FAIL, else write); a `pre_write` check that declares `on_warn:"skip_write"` and reports WARN now makes the gate SKIP the write (no transaction opened, the target untouched, every write counter zero) while the run COMPLETES with verdict WARN — the legacy `load_centreline` F-C1 "warn and preserve" arm, made declared and observable, landing ONE `write_skipped_pre_write_warn` audit row (`source:"gate"`, naming the triggering check ids) via `§preWriteSkipRows`. Precedence is FAIL-first: an unaccepted FAIL still aborts even if a `skip_write` WARN also fired. The pre-0h gate return shape (`{abort, failed}`) stays byte-identical for every descriptor that never declares the arm — `skipWrite`/`skipWriteChecks` are added to the decision object only when it actually fires, never as an always-present `false`/`[]`, so the LR-D9 locks (`step-library.logic.test.ts`) keep passing unchanged. Ten new locks in `step-library.logic.test.ts` (T1-T6, three new describes at the end): the executor's statement ORDER and counters (T1), the empty-carried-set guard (T2), ROLLBACK on a staged-insert failure (T3); `runIngestPhase` dispatches class C to `executeStagingReplace` and class A/B still to `executeWrite` (T4a/T4b); the gate's three-way decision shape both directions (T5a/T5b), the skip-write arm end-to-end via `runIngestPhase` plus `preWriteSkipRows`'s own row (T5c), and a WARN without `on_warn` still proceeding (T5d, pinned); AJV accepts `on_warn:"skip_write"` only on a `when:"pre_write"` check (T6). `write-class-disposition.infra.test.ts`'s WD-1 RED fixture (`banned-class-declared.descriptor.json`) is repointed from `staging_full_replace` to `set_based_unscoped` (still `banned_for_new`) — the class stopped being banned the moment it gained a real executor, so a fixture still declaring it would have stopped proving anything. `grandfathered.json` gains NO entry in this commit — the guard-path grandfathering for `toronto_centreline` (`guard:"none"`, Spec 62 L26) belongs to `load_centreline`'s own descriptor at 3.2 ②, not to the library prerequisite. `schema_sha256` re-derived by `--refresh` in this commit. Record: `.cursor/engine-briefs/b2-p3-c0h-staging-replace.md`.

**Seam note — 0p (2026-09-24, no schema byte, no RE-FREEZE number):** `shapeRecord` may return a row (kept), a **string** — the skip REASON — or `null` (skip, reason `"unspecified"`), and may tag a kept row via `ctx.tag(name)`. `acquired.shaped_skipped_by_reason` (Σ = `shaped_skipped`) and `acquired.shaped_tags` are ALWAYS objects, golden-neutral; a `shapeRecord` throw propagates (no terminal stamped — LC-D10). Lock: `runIngestPhase` ctx widens by `tag` (0n lock).

**RE-FREEZE #21 — `network.retries_from_config` / `retry_backoff_from_config` + executed download retries (2026-09-24, INGESTOR prerequisite 0q):** two additive `execution.network.*` string fields (`timeout_from_config`'s own shape), naming the logic var whose RESOLVED value overrides the frozen `retries` literal (non-negative integer only) and the constant inter-attempt backoff (ms, finite ≥ 0); `"none"` disables the override, `retries` stays the seeded-DB fallback. `resolveRetryPolicy(descriptor, config)` (`scripts/lib/step/acquire.js`) resolves both; `downloadWithRetries` wraps the untouched `downloadArchive` (FENCE 0b230472) — `attempts = retries + 1`, WARN only when a next attempt remains, partial `destPath` removed between attempts, `backoffMs` sleep (never last), LAST error rethrown. Download-only, HEAD never retried. `retries: 0` (every current declarer) is BYTE-IDENTICAL to pre-0q. `schema_sha256` re-derived (nested field, no contract change). Registered in `execution-budget-disposition.json` as `network.retries` — R-X's first NON-duration row. Locks: `src/tests/ingest-prereq-0q.logic.test.ts` (NEW file, T1-T8). POST-B1-8 PARTIAL (mechanism only; see `review_followups.md`).

**RE-FREEZE #22 — `inputs.reads.externals[].on_head_error` (2026-09-24, INGESTOR prerequisite 0r):** one additive nested enum field, `on_head_error: "fail_step" | "warn_row"`, `x-frozen`, carrying its own `x-ruling` — only a DECLARATION can decide whether the acquisition library throws (Rule 10), and the WARN row it emits is library-owned. ABSENT = `"fail_step"` = today, byte-identical (the HEAD throw escapes `acquireExternal`). `"warn_row"` swallows a HEAD 4xx/5xx, records `acquired.head_error`, hands the tier-1 gate null validators (so the `no_validators` arm decides to download), and lands ONE library WARN row `head_error` (`§headErrorRows`, `source:"gate"`, `errored` deliberately absent — a declared posture, not an `accept_anomaly`-switchable anomaly). Locks `src/tests/ingest-prereq-0r.logic.test.ts` (NEW file, T1-T6). No new library call in a runner. `schema_sha256` unchanged (nested field under the existing `externals.items.properties`; no contract-surface byte at the schema root).

**Seam note — 0s (2026-09-24, no schema byte, no RE-FREEZE number):** `parseShapefile` calls `coerceKey(props[keyProperty], { geojson })` — the GeoJSON string built ONCE from `r.value.geometry` and reused in the push, so the 2nd argument carries data only (Rule 2) and every existing 1-arg `coerceKey` stays byte-identical. A geometry-derived key (massing's `'hash_' + md5(geojson).slice(0,12)`, its DBF having no id) is compute-domain logic the step's own `coerceKey` owns; the parse stays domain-free. `crypto` is NOT imported into the seam. A null geometry still takes `key == null → bad_key` before `nullGeometry` (order pinned). Lock: `src/tests/ingest-prereq-0s.logic.test.ts` (T1-T4).

**RE-FREEZE #23 — outputs.writes[].geometry_repair (2026-09-24, 0t):** make_valid (default, byte-identical) | none; detail 122a §A14.

**RE-FREEZE #24 — columns[].derived_from_geometry (2026-09-24, 0u):** {measure,unit,scale}, geodesic_area/m2|ft2, additive; detail 122a §A14.

**RE-FREEZE #25 — externals[].format gains "geojson" (2026-09-25, 0v):** parseGeoJson ≡ parseShapefile tallies/order; other arms byte-identical; detail 122a §A16.

**Schema byte change (output-panel peel, 2026-09-22, R-X's own `x-banned-for-new` enforcer, no new RE-FREEZE number):** `x-banned-for-new` gains a `scalar_fields` sibling to `values`/`rules` — a field banned in its ENTIRETY (any presence, not one enum value) — and `execution.on_row_error_max_pct` is registered under it (`paths: ["execution.on_row_error_max_pct"]`). Closes a gap the peel's own DeepSeek review found live: the field's inline `"x-banned-for-new": true` annotation (RE-FREEZE #12, above) had no consumer — `assertGrandfathered` only ever read `values[GUARD_PATH]` — so nothing checked this path at all. Enforced by `scripts/lib/step/validate.js assertBannedScalarFields`, same allowlist file (`grandfathered.json`), a sibling shape (`scalar_fields: string[]` naming the path, never a value). Not a numbered RE-FREEZE: no `properties` field is added, widened or required — this is the SAME class as R-X's `write-class-disposition.json` registry, which also changed schema bytes (its own `values`/`rules` keys) without a numbered RE-FREEZE, because the descriptor CONTRACT surface (what a step may legally declare) is unchanged; only which declarations the ENFORCER checks changes. `schema_sha256` re-derived by `--refresh` in this commit regardless, per the R-E lock's own scope (every schema.json byte change, not only contract-surface ones).

**Schema byte change (WF3 "class-C retract-all guard", Commit 2 "anti-bypass hardening", 2026-09-24, SAME class as the 2026-09-22 peel immediately above — no new RE-FREEZE number):** `x-banned-for-new.values` gains a third key, `outputs.writes[].write_discipline.class`, listing `["staging_full_replace"]` — declaring class C is now `x-banned-for-new` for any step other than the one this same day's operator ruling adjudicated. `assertGrandfathered` (`scripts/lib/step/validate.js`) is extended, narrowly, to ALSO enforce this path (a `GRANDFATHERED_VALUE_PATHS` list of `{GUARD_PATH, CLASS_PATH}`, each keyed to its own descriptor field and sibling `why`) — deliberately NOT generalized to the OTHER two `values` keys (`replay:"append_unsafe"`, `execution.criticality:"best_effort"`), which remain unenforced (followup filed, `docs/reports/review_followups.md`, MED). `grandfathered.json` gains the `load_centreline` entry naming this exact path+value (Rule 9 adjudication, operator ruling 2026-09-24) — the CLASS half only; the GUARD half for the same step's `guard:"none"` write is explicitly deferred to `load_centreline`'s own descriptor-authoring commit (batch-2 Phase 3, 3.2 ②), per RE-FREEZE #20's own record above. Not a numbered RE-FREEZE for the same reason as the 09-22 peel: `staging_full_replace` already existed in the `class` enum (RE-FREEZE #20 gave it an executor, not a new enum value) — the descriptor CONTRACT surface is unchanged, only which declarations `assertGrandfathered` refuses. `scripts/lib/exec-policy.json` `registry_reserved` also gains `scripts/steps/_schema/grandfathered.json` and (redundantly, defense-in-depth — it was already on the DeepSeek execution engine's hardcoded `SELF_PROTECT_DENYLIST`, `scripts/lib/exec-tools.js`) `scripts/lib/exec-policy.json` itself, so an engine brief cannot self-grandfather a banned class by naming either registry in its own `write_scope`. Locks: `src/tests/wf3-class-c-antibypass.infra.test.ts` (NEW file) — the class-path gate both directions (with/without the `load_centreline` grandfathered entry), the guard axis staying independently enforced under the SAME adjudicated slug, both registries present in `exec-policy.json`, and a live DeepSeek-engine `write_file` attempt against `scripts/steps/_schema/grandfathered.json` refused `PATH_RESERVED`. `schema_sha256` re-derived by `--refresh` in this commit.
...
**C2 RE-EVALUATED AT k=9 — RULED NOT FIRED (operator, 2026-09-11; measured by the orchestrator at pilot 9 commit 9 `3c1f1923` + FREEZE-1 `21aca645`/`2c3dea53`, adjudicated by the operator per Spec 124 §4.2 — the C4 entry gate, `.cursor/c4_batching_entry_active_task.md` step G).** All four readings re-taken at the cutover SHA, none pre-dating it (the 2026-09-09 readings in the C4 plan §2 all did): **K1** (retired, R-Y — measured anyway) every one of the 9 `converted.json` descriptors carries exactly 20 top-level non-dollar keys, `beyond20 = 0` (`assert_schema` 581 lines · `load_ravines` 813 · `link_massing` 1,026 · `link_wsib` 670 · `link_parcel_addresses` 452 · `compute_centroids` 389 · `link_parcels` 688 · `refresh_snapshot` 391 · `enrich_parcels` 826 — all inside declared categories); **K2** `grep -rnE "slug === |=== '<each of the 9 slugs>'" scripts/lib/step/` → **0 hits**; **K3** `node scripts/hooks/check-step-shape.mjs` → `✅ Step-shape gate clean (9 converted step file(s) enforced; 9 compute module(s) enforced)`, and a per-concept grep (`pipeline.` / `withTransaction` / `getDbTimestamp` / `streamQuery` / `emitSummary` / `emitMeta` / `advisoryLock`) over `scripts/lib/compute/**` with comment-only lines excluded → **0 hits**; **K4** `step-validate --all --fast` → `unexplained-diffs=0` and G8 **3/3 on all 9** (the 2026-09-09 reading's one non-green G8 — `enrich_parcels`' declared recapture deferral — is closed by commit 9's recapture, fingerprint `0501de76`; scores 16/16/16/16/14/14/16/16/16, hard-stop=false ×9). **Consequence:** C4 batch 1 is entered — `assert_global_coverage` first, then `assert_data_bounds` (both in the full nine-commit form), compressed three-commit form permitted from `assert_engine_health` onward (Spec 124 R-PACE-1, corrected 2026-09-13 — the prior citation "R-AA" here was wrong, R-AA is the spec-move rule). No register row is added: this is the pre-declared gate evaluating, not a policy change.
...
| 10 | ⚠️ **A declared self-consumed `code_version` trigger baseline that the compute never actually emits reads "unchanged" forever** — `link_parcels` declared `staleness.trigger:[{signal:"code_version", emit_key:"code_version"}]` but `buildLinkMeta` never returned it; `selectMode` treats an absent baseline as `changed:false` by design, so a `logic_version` bump could never force FULL (LP-D16, 2026-09-26) | cross-check `records_meta`/POST goldens against the descriptor's own `staleness.trigger[].emit_key` — a declared trigger absent from the compute's return is a dead fence, invisible from the descriptor alone. Fixing it does not retroactively rebuild data: the first post-fix run's own prior baseline is still absent, so the corpus needs one explicit forced-FULL run to stamp a truthful baseline (GC-12) |
