...
| 8 | `geocode_permits` | `geocode-permits.js` | Assign lat/lng by equijoin of `permits.geo_id` to `address_points.address_point_id` (no address-string match, no network) | permits |
| 9 | `link_parcels` | `link-parcels.js` | Spatially/address-links permits to property lot polygons via a 4-strategy cascade (address_points bridge exact → legacy parcels exact → name-only → spatial). **Writes to `permit_parcels`** (the original, legacy-named table — the "WF1 Phase C extension: writes to unified `lead_parcels`" claim that stood here through 2026-08 was never true for this step; `permit_parcels` rows are mirrored into `lead_parcels` by migration 144's own trigger as a downstream side effect, not written directly by this script). **WF1 #parcel-address-bridge Phase 2d (2026-05-23, commit `1ba020b`):** added the top-of-cascade Strategy 1a (address_points exact via `parcel_address_points` bridge, confidence 0.97) ahead of the legacy parcels-table exact strategy (1b); disambiguated among same-address candidates by a 3-level rule (`address_class_desc` Structure > Structure Entrance > Land > smallest `ST_Area(geom::geography)` > `address_point_id ASC`). **Pilot 7 / Spec 122 conversion (2026-08-30):** converted to the frozen step shape (`link-parcels.descriptor.json` + `scripts/lib/compute/link-parcels.js`); the spatial fallback tier (Strategy 3 Step 2, for permits with no address match) now ranks candidate parcels by live boundary distance (`ST_Distance`/KNN `<->` on `parcels.geom`), not the materialized `centroid_lat/lng` columns it used before; **LP-D9 (commit 8a):** Strategy 1a's JOIN now also matches on the permit's own `street_type` (mirroring Strategy 1b's predicate) — it originally matched on house-number + street-name alone, which could silently resolve to a same-named street of the WRONG type elsewhere in the city. | permit_parcels |
...
3. **Stale permit closure** — Permits not seen since the last successful load move to `'Pending Closed'`, then to `'Closed'` after the grace period; post-C7, `enriched_status` is cleared in the same UPDATE (the status-grain rule, Spec 44 §3).
