...
**⚠️ RE-FREEZE #24 — columns[].derived_from_geometry (2026-09-24, prerequisite 0u).** Legacy massing seeds area columns via a SECOND statement, a post-INSERT UPDATE (`scripts/load-massing.js`, `ROUND((ST_Area(<3857 transform>::geography))::numeric,2)` + ft² twin). `derived_from_geometry {measure:"geodesic_area", unit:"m2"|"ft2", scale:0-6}` (required, `x-frozen` measure/unit) renders the SAME expression into the validator's inner subquery over the PRE-repair `geom`, carried to the row `validateGeometries` returns — one round-trip, no second statement. `ft2` factor `10.7639104167` is a UNIT CONSTANT (`write.js` `DERIVED_UNIT_FACTOR`), not a tunable. Absent = byte-identical (T0 re-pins the six 0t hashes). `DerivedMeasureError` guards: no `wkb_geometry` column, or column neither `step` nor `insert_only`. Pairs with `written:"insert_only"` (0j). `rungs_tried`: insert_only alone, GENERATED column (rewrites 427K rows), trigger, set_value, JS area, class E step — `why`: only the validator holds pre-repair geom in the INSERT txn. DB proof: `src/tests/db/ingest-prereq-0u.db.test.ts`. G-1: +1 field, `x-ruling`-exempt.

---

## Appendix §A15 — WF3 class-C retract-all guard exemption (2026-09-24)

**`buildWritePlan`'s retract-all/no-scope guard now exempts class C only.** The guard (`retract==="all" && !scope` → throws) predates class C (RE-FREEZE #20) and blocked `load_centreline`'s declared `retract:"all"`/`scope:"none"` — the sanctioned whole-table replace. Keyed on class, not step name: exempt iff `write_discipline.class === STAGING_FULL_REPLACE_CLASS`; every other class still throws. `plan.delete_sql` for class C is the unconditional `DELETE FROM <table>` — truthful, never read by `executeStagingReplace` (own `DELETE`, own txn, empty-set guard). Locked: `src/tests/wf3-class-c-retract-all.logic.test.ts`.

---

## Appendix §A16 — RE-FREEZE detail, neighbourhoods prerequisites 0v/0w (2026-09-25)

**⚠️ RE-FREEZE #25 — `inputs.reads.externals[].format` gains `"geojson"` (2026-09-25, prerequisite 0v).** The 0b format axis selected the parser by a DECLARED payload format, but its `enum` was `["shapefile_zip","csv"]`, so the neighbourhoods source — one bare 2,141,269 B FeatureCollection (158 MultiPolygons, CRS84, `AREA_SHORT_CODE` a string on all 158) — had no arm and the seam threw `… which no parser … handles`. `parseGeoJson(filePath, keyProperty, coerceKey, keyColumn)` (`scripts/lib/step/acquire.js`) reproduces the legacy in-module parse generically: `fs.readFileSync(…,'utf8')` (the download already streamed to a temp file through the hash-through, FENCE 0b230472 — the seam never buffers a download), `JSON.parse` with the legacy failure text (`Failed to parse GeoJSON file ${path}: ${err.message} (first 100 chars: ${raw.slice(0,100)})`, `scripts/load-neighbourhoods.js:115-121`), and a named refusal when `features` is not an array (a `{type:"FeatureCollection"}` with no features would otherwise parse to zero rows under a green verdict). Feature shape and DROP ORDER are `parseShapefile`'s exactly: `rowsParsed++` first, then `coerceKey(props[keyProperty], { geojson })` (`props = f.properties || {}`; `geojson = JSON.stringify(f.geometry)` built once, `null` for a null geometry) → `key == null` is `badKey`, and ONLY THEN a null geometry is `nullGeometry` (a feature both keyless and geometry-less counts ONCE, as a bad key); the kept row is `{[keyColumn]: key, geojson, record: props}`. The `coerceKey` 2nd argument is `{ geojson }` and nothing else (0s, Rule 2 — data, not a service), so a geometry-derived key stays expressible without `crypto` reaching the seam. The download name is `source.geojson` (`{ csv: 'source.csv', geojson: 'source.geojson' }[format] || 'source.zip'`); the shapefile and csv arms are byte-identical; the unrecognised-value throw now lists `geojson`. No runner change: a geojson primary without `compute.shapeRecord` passes through like a shapefile, and with one, `shapeRecord(record, { geojson })` binds columns exactly as for the other arms. `x-ruling.rungs_tried` gains `"0v: csv/shapefile arms wrong payload; sniff = PD#10; compute I/O banned"`. No `generate-schema-baseline` row moves (the change is one enum value below the baseline's `category.field` grain); `schema_sha256` re-derived by `--refresh` at landing. Locks: `src/tests/ingest-prereq-0v.logic.test.ts` (T1-T6). NB-X3 (comments only): the 0q JSDoc and `ingest-prereq-0q.logic.test.ts` header claimed the legacy loaders "retried THREE times" — false; `load-neighbourhoods.js`'s `downloadFile` and its address-points/parcels/massing copies made ONE attempt, and only centreline's `downloadZipWithRetry` retried (3) (Fold G-4, 2026-09-25, measured).
