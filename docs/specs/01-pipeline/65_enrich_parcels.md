...
- **MB-2 Lot-validation gate (Phase 1).** `lot_size_confidence` (high/medium/low) from a 3-way cross-check: stored `lot_size_sqm` vs inline `ST_Area(geom::geography)` vs `frontage_m × depth_m` (NO `stated_area_raw` re-parse — `lot_size_sqm` is already the parsed value). `high` = all 3 pairwise within 15%; `medium` = ≥1 pair agrees; `low` = none agree OR out-of-bounds (<50/>2000 m²). The envelope (Phase 2/3) emits ONLY when `lot_size_confidence ∈ {high, medium}` — else NULL + a split `envelope_constraint_reason` (WF3): **`lot_too_small`** (`lot_size_sqm < 50` — genuinely unbuildable sliver, ~6K), **`lot_too_large`** (`lot_size_sqm > 2000` — a large lot that merely EXCEEDS the residential max-build MODEL range, ~18K; **buildable, just not modelled** — the UI must not say "can't build"), else **`low_lot_confidence`** (in-range single-source or NULL lot).
