# Enrich: Parcel Zoning (Spec 58 WF2) — **v1.1**

**Version:** 1.1 — authored 2026-05-31 via WF1/Genesis; v1.1 (2026-07) applies the mig-201 imagery-roof column rename (`existing_footprint_sqm`→`imagery_roof_footprint_sqm`, `existing_gfa_sqm`→`imagery_roof_gfa_sqm`). Second of Spec 58 §8a's three-WF sequence (WF1 ingest ✅ `58914fa` → **WF2 parcel enrich (this spec)** → WF3 permit/CoA enrich). Folds two adversarial plan-review rounds (Gemini + DeepSeek) and a live data-profiling spike (`docs/runbook/65_enrich_parcels_spike.md`).
...
- **`scripts/enrich-parcels.js`** (NEW) — Spec 47 §R1–R12 skeleton; `ADVISORY_LOCK_ID = 65`; `require.main === module` guard (per `tasks/lessons.md` — loaders without it fire a real DB run when `require()`'d in tests).
  - **Converted to the Spec 122 step standard — pilot 9 (ENRICHER), cutover commit 9, 2026-09-11.** `scripts/enrich-parcels.js` is now the thin shell; the contract lives in `scripts/enrich-parcels.descriptor.json` (+ `scripts/enrich-parcels.notes.json`) and the logic in `scripts/lib/compute/enrich-parcels.js`, dispatched by `runEnrichPhase` (`scripts/lib/step/index.js`, `execution.shape:"enrich"`). Registered in `scripts/steps/_schema/converted.json`; the nine-commit record is `docs/reports/2026-09-04-pilot9-enrich-parcels-assessment.md`. Behaviour documented in §3–§8 is unchanged by the conversion (golden differential G8, Spec 123 §6); the declared maintenance (§3c Known Failure Modes, EP-D17) is executed by the library, not by this file.
...
- **Height-overlay weld + spillover (WF3)** — the `height_agg` CTE took `MIN(height_max_m)` and `MIN(ht_stories)` INDEPENDENTLY across all covering overlays, and lacked the `NOT ST_Touches` guard → storeys from an edge-touching mid-rise overlay were WELDED onto the height from the real overlay (e.g. 6 storeys @ 9m = 1.5 m/storey, physically impossible; ~207 lots). Guard: `NOT ST_Touches` + `DISTINCT ON (parcel) ORDER BY height_max_m ASC, ht_stories ASC NULLS LAST, source_id` — pairs the storeys OF the most-restrictive-height row (MIN-height conservatism preserved). NB: a **generous height cap + low storey cap** (e.g. 11.5m / 2 storeys = 5.75 m/storey) is GENUINE low-density zoning, not a weld — the sanity audit's m/storey bound is asymmetric (too-low `< 2.5` = bug; too-high = INFO).
- **Wrong-axis setback charge (WF3 Phase 1 D-A, 2026-08)** — the corner branch charged the FRONT setback (a DEPTH loss) against the WIDTH: 13.49 m frontages emitted 2.99 m-wide envelopes on ~6,961 corners, priced by the cost model. Guard: `enrich-parcels-maxbuild-corner.db.test.ts` (axis lock) + the `max_build_dim_exceeds_lot_dim` gated sanity invariant (width ≤ frontage, length ≤ depth — kills the whole opposite-sign error class at the data layer).
- **Degenerate LEAST term below the viability floor (WF3 Phase 1 D-C + step-8 catch, 2026-08)** — a floor applied to ONE term of `LEAST(buffer, box, coverage)` leaves the siblings free to win with degenerate values: after the box dims were floor-clamped, 602 ravine parcels' −10 m buffer inset collapsed to 0.01–9.98 m² slivers that won the LEAST and were priced. Rule: a viability floor on a min-aggregate must bind EVERY term (box per-axis ≥ `max_build_min_dimension_m`; buffer ≥ minDim²), or the min inherits the artifact. Guards: `enrich-parcels-clamp.db.test.ts` (incl. the buffer-degenerate case) + the `max_build_dim_below_floor` gated check. Residual: the above-floor ravine class still persists pre-buffer rect dims beside buffer-clipped footprints (`priced_newbuild_lt_30sqm` INFO tripwire; redesign = the ravine-directionality WF). — per-parcel correlated `EXISTS` membership subqueries and computing exact `ST_Intersection(...)::geography` for *every* candidate pair made the 486K-parcel run intractable (>9 min, no completion). Guard: (1) memberships are set-based `DISTINCT` join CTEs (one GiST spatial join each), never correlated per-row; (2) exact intersection area is computed ONLY for multi-candidate parcels via a lazy `CASE WHEN COUNT(*) OVER (…) = 1 THEN 1.0 ELSE ST_Area(ST_Intersection(...)::geography) END`. Full run → ~7–8 min (50K ≈ 45 s, benchmarked).
- **Pass 4's own write bloats the heap the step's own post checks then pay for (EP-D17, WF3, 2026-09-10)** — pass 4's comparable-builds UPDATE (~344,316 rows, measured ~1.35M dead tuples afterward, ratio 3.91:1 unexplained/filed LOW) leaves `parcels` at `dead_ratio` 0.697 by the time the step's run-end `invariants[]`/`plausibility[]` post checks execute; those 5 full-Seq-Scan checks (plus `run-chain.js`'s T1/T4 telemetry) then cost a measured 150x more — 16-30s clean vs 40+ minutes bloated (cloud EXPLAIN, run 34506962436) — because passes 1-4 share one `txn_scope:"step"` transaction whose xmin pins the vacuum horizon for its whole ~93-minute duration, so nothing can reclaim the dead tuples until AFTER this step's own COMMIT. Guard: `execution.maintenance` now declares a `vacuum_analyze` target on `parcels` (`scripts/lib/step/plausibility.js` `runMaintenance`, wired right after the step's write phases) + a declared `parcels_dead_tuple_ratio` plausibility bound making the ratio observable + migration 247's tuned autovacuum params (lower scale factors, `cost_delay=0`) speeding the post-COMMIT catch-up + a per-entry `statement_timeout` ceiling on the 5 post checks so a regression FAILs loudly instead of wedging the chain. Residual: the ONE remaining full-scan reduction (folding the 5 post checks' own predicates into one query, RE-FREEZE #4's deferred scope) was explicitly NOT taken this WF3 — concurrency (C2b) already gets the wall-clock cost to a single shared ~5-30s window without it.
</failure_modes>
...
- **MB-2 Lot-validation gate (Phase 1).** `lot_size_confidence` (high/medium/low) from a 3-way cross-check: stored `lot_size_sqm` vs inline `ST_Area(geom::geography)` vs `frontage_m × depth_m` (NO `stated_area_raw` re-parse — `lot_size_sqm` is already the parsed value). `high` = all 3 pairwise within 15%; `medium` = ≥1 pair agrees; `low` = none agree OR out-of-bounds. The band is `[max_build_lot_min_sqm, max_build_lot_max_sqm]` (**WF3 S0.1, Rule 3 / R-G, 2026-09-21**: registered logic variables, defaults **50 / 2000 m²** — the SAME already-ratified band, promoted from the bare `LOT_MIN_SQM`/`LOT_MAX_SQM` literals at `scripts/lib/max-build.js:27-28`; inclusive at both ends, i.e. in-range ⟺ `lot_size_sqm >= max_build_lot_min_sqm AND lot_size_sqm <= max_build_lot_max_sqm`; consumed by `buildMaxBuildSql`'s `lotMinSqm`/`lotMaxSqm` params, threaded from `config.max_build_lot_min_sqm`/`config.max_build_lot_max_sqm` in `runPass2`, with `mb.LOT_MIN_SQM`/`mb.LOT_MAX_SQM` surviving only as the JS-fallback defaults for callers that omit them). The envelope (Phase 2/3) emits ONLY when `lot_size_confidence ∈ {high, medium}` — else NULL + a split `envelope_constraint_reason` (WF3): **`lot_too_small`** (`lot_size_sqm < max_build_lot_min_sqm` — genuinely unbuildable sliver, ~6K), **`lot_too_large`** (`lot_size_sqm > max_build_lot_max_sqm` — a large lot that merely EXCEEDS the residential max-build MODEL range, ~18K; **buildable, just not modelled** — the UI must not say "can't build"), else **`low_lot_confidence`** (in-range single-source or NULL lot). **Cross-reference (`wf3_existing_structure_area_artifacts_active_task.md` §0.1, `tasks/lessons.md:30`): `compute_parcel_cost_estimates`' reno cost lines honour the SAME gate as of S0.2 — the two passes previously disagreed on which parcels are "in the model."**
- **MB-3 Geometric footprint + rect box (Phase 2; buffer party-wall-scaled WF3-B; coverage-default WF3).** `max_buildable_footprint_sqm = LEAST(ST_Area(ST_Buffer(geom::geography, -(side_setback × side_count/2.0 + ravine_red))), box_area, lot×coverage)` where **`coverage = COALESCE(bylaw_max_coverage_pct, zone-class default)`**. **WF3 coverage-default:** `bylaw_max_coverage_pct` is NULL on ~37% of parcels; without a fallback `LEAST` drops the term and the footprint balloons to the raw setback box (~67% coverage, 42K parcels > 50%). A zone-class default (`COVERAGE_DEFAULTS` in `max-build.js` — empirical medians RD/RS/RT 33, RM 30, R 35; commercial/other permissive; `buildCoverageCase` SQL mirror, JS↔SQL parity-tested, same key set as `SETBACK_DEFAULTS`) fills the NULL via `COALESCE` (never overrides a present value). `coverage_cap` stays NULL only when `lot_size_sqm` is NULL (can't cap without an area) → footprint falls to the box for those. Reported by `max_build_coverage_defaulted_count` / `_binding_count` (MB-8). Full logic_variables externalization of the geometry defaults (setbacks + storey-heights + coverage, all currently JS constants) is a tracked follow-up. — the negative buffer is shape-aware (irregular/pie-safe) but directionally-blind; the `max_build_width/length_m` rect box (`max_build_basis='rect_approx'`) is directional but shape-blind; the two cross-check via `LEAST` (which ignores NULLs). **`box_area` is the only term capturing front/rear setback DEPTH loss (the side-only buffer cannot) — it is load-bearing in the LEAST + co-gates `setback_exceeds_lot`/`envelope_constrained`; not droppable *unconditionally*.** **WF3 Phase 1 D-C (below-floor clamp):** a width/length below the viability floor (`max_build_min_dimension_m` logic-var, default 3.0 m; seed JSON + migration 239 + `_contracts.json`, parity-locked incl. the sanity-audit literal) is NULLed per-axis — it is evidence the zone-default setbacks do not describe the lot, not a geometry to price; exact-zero rows join the same contract (no 0.0-vs-0.29 split). **The BUFFER term carries the same floor** (step-8 gate catch): a negative-buffer sliver below `minDim²` (cannot seat a minDim×minDim square) is excluded from the `LEAST` — else a long-skinny ravine lot's −10 m inset collapses to ~0.01 m² and prices an absurd envelope (602 rows caught by the (0,10)-GFA gate on the first full re-run). Below the floor the box is excluded and nothing models depth loss — the **non-ravine** envelope is **coverage-only** (footprint = coverage cap; the degenerate buffer is excluded too), flagged `max_buildable_gfa_basis='coverage_only'` + `envelope_constrained=true`, confidence `low`. Consumers of `'coverage_only'`: `assert-global-coverage.js` (CoA + permits basis rows) · the producer basis-distribution audit rows (`max_buildable_gfa_basis_coverage_only_count`, `ravine_constrained_count`, `max_build_box_excluded_count`) · runbook · `enriched-parcels-field-spec.md`. **Ravine sub-floor residual → `'ravine_constrained'`** (new `envelope_constraint_reason`, ordered ABOVE the unconditional `'ravine'` branch, heritage excluded — the freeze path owns heritage): the WHOLE envelope is withheld (dims/height/stories/basis/GFA all NULL, confidence `low`, `envelope_constrained=true`) — a coverage×lot fallback is ravine-blind and would re-price deleted protection; the above-floor ravine majority keeps `'ravine'` + its envelope. Locked by `enrich-parcels-clamp.db.test.ts`. **WF3-B (party-wall):** the buffer side inset is scaled by `side_count/2.0` so attached shared walls aren't over-inset (RD ×1 unchanged; RS ×0.5; RT ×0). Empty buffer (lot < 2×inset) → NULL + `setback_exceeds_lot`. Widths/lengths clamp `GREATEST(0,…)` → NULL + `lot_too_narrow`. Stories (`max_build_stories`, WF3-C2 3-way): `bylaw_max_stories` → that (basis `'bylaw'`); else pocket p50 `LEAST(pocket_p50, height_implied)` (basis `'pocket'`, legal-height-capped — §8); else `round(height/storey_height)` (basis `'derived'`); both NULL → NULL. GFA = `LEAST(footprint×stories, lot×FSI)`; basis `fsi`/`coverage_box`.
- **MB-4 Setbacks + party-wall `side_count` (WF3-B).** `bylaw_standard_setback_m` (from `STAND_SET`, front-aligned) is the FRONT setback when present; side/rear/flankage have NO source field → always from the coarse `zoning_class → {front,side,rear,flankage,side_count}` table in `scripts/lib/max-build.js` (`SETBACK_DEFAULTS`, documented approximations). `max_build_setback_basis` records `bylaw` vs `zone_default`. **`side_count` = number of NON-party sides that take a side setback: RD/R/RM = 2 (both sides open), RS = 1 (one shared party wall — semi), RT = 0 (interior unit, both shared — the MODAL townhouse; end units over-counted slightly). `width_raw` non-corner = `frontage − side_count × side_setback` (was `2×`); the buffer side inset scales `side_count/2.0` (MB-3).** Read via `buildSideCountCase`/`lookupSideCount` (NOT `buildSetbackCase`, which is `SETBACK_DIMS`-gated). An INFO audit row `attached_modal_side_count_parcels` surfaces the RS/RT count (transparency). The wider attached `width_m` also flows into the Phase-3 accessory `rear_yard_area` (intended). Setbacks always resolve (DEFAULT row), so the front setback is never NULL — no `no_setback_data` outcome.
- **MB-5 Location reductions (Phase 3; corner axis corrected WF3 Phase 1 D-A).** Corner → `width = frontage − side_setback × MIN(side_count, 1) − flankage` — the front setback is a DEPTH loss and must never be charged against the WIDTH (the pre-fix `frontage − front − flankage` read 13.49 m frontages as 2.99 m-wide envelopes on ~6,961 corners); a corner lot loses its flankage side plus at most ONE interior side setback, side_count-aware so attached corner units (RS/RT) don't double-inset (regression-locked in `enrich-parcels-maxbuild-corner.db.test.ts`). Through-lot → `length = depth − 2×front`. Ravine → subtract a FIXED `RAVINE_SETBACK_M` (10 m, Ch.658 stable-slope); `ravine_distance_m` is display-only, **NOT** a multiplier (Spec 59 L2: it is signed proximity, not a gradient). Heritage → FREEZE the FOOTPRINT to the existing structure (`SUM(footprint_area_sqm)` across primary `parcel_buildings`→`building_footprints`); no primary building → NULL + `heritage_no_massing`; **`bylaw_max_*` is never overwritten** (variance_context depends on it). **WF3 heritage storeys:** `max_build_stories` (and heritage GFA = `frozen_footprint × stories`) use the SAME bounded `stories_calc` as non-heritage (`bylaw_max_stories → LEAST(pocket_p50, height_implied) → pocket_p50 → derived`) — **NOT** the massing `estimated_stories`, which is tree-canopy contaminated (bungalows at 5–79 storeys / FSI to 57) and retired as a storey source everywhere (closes the ES-6 §L213 carve-out). Heritage GFA carries **no FSI cap**: frozen-real footprint × bounded storeys self-bounds, and a cap would understate a legitimately grandfathered structure. **WF3 mislink guard:** a primary-massing footprint > `lot_size_sqm × (1 + mislink_footprint_lot_tol)` means the WRONG building was linked (a heritage building cannot exceed its lot) — the freeze treats it as *no trustworthy massing* (NULL footprint/GFA/stories, reason `heritage_footprint_exceeds_lot`, counted `heritage_mislink_footprint_count`) rather than pricing garbage (unguarded, it produced FSI 20+ → multi-$M cost). This uses the SAME tolerance the existing-structure pass already applies, making the two passes agree. **Coupled optconfig reset:** because the optimal-config pass streams only `max_buildable_footprint_sqm IS NOT NULL`, a parcel that LOSES its footprint (this guard, or any reason) would keep a stale `opt_aor` — and the cost model's `COALESCE(opt_aor, max_buildable_gfa)` would still price it. So `enrichOptimalConfig` first NULLs `opt_*` for in-scope parcels with a null footprint but a stale `opt_config_confidence` (counted `opt_config_reset_ineligible_count`). Garden suite (rear-yard) gated on min lot area + usable rear yard, excluding ravine/heritage — NOT laneway (laneway-suite deferred #431-FU2).
...
- **ES-2 The 10 columns** (mig 187, all nullable; `ROUND(…,2)` on numerics for idempotency): `imagery_roof_footprint_sqm`, `existing_stories` (height-derived est.), `existing_height_m`, `imagery_roof_gfa_sqm` (= `footprint × GREATEST(1, COALESCE(stories,1))` — floored so a <3 m bungalow isn't GFA-zeroed), `existing_width_m`/`existing_length_m` (the two sides of `ST_OrientedEnvelope(bf.geom)` measured in **metres via `::geography`** at the point level; areal-geom guarded; NULL when geom NULL/non-areal), `existing_structure_confidence` (`high` if `pb.confidence >= 0.90` i.e. centroid-in-parcel, else `low` for nearest-fallback — a low link may be a neighbour's building), `existing_other_structures_count`/`_sqm` (non-primary buildings), `existing_greenspace_sqm` (`GREATEST(0, lot − primary − other)` — unbuilt open area; assumes non-overlapping footprints; not vegetation-verified, excludes unseen paving).
...
- **ES-4 Propagation + observability.** `enrich-permits.js` `EXISTING_STRUCTURE_COLS` through the 4 §8e surfaces (nullable TEXT confidence → generic `=NULL` orphan path; no NOT-NULL bools) + `assertExistingStructureColumns` guard (mig 187/188). Three-layer INFO (enrich-parcels + enrich-permits + assert-global-coverage), all `infoRow`, never gated; `existing_structure_confidence` as `_high_count`/`_low_count` buckets. **WF3 S0.3 addition (2026-09-21):** the observability gap `review_followups.md:3123` named — "0 of 42 `assert_parcel_sanity` rules reference `parcel_buildings`/`match_type`/`confidence`/`is_primary`" — is closed: `existing_structure_shared_with_other_parcel` (a parcel's `existing_*` fields are sourced from a building that is ALSO another parcel's primary — 97,987, measured THROUGH the real RES-scoped step; the plan's own originally-cited 108,333 was the UNSCOPED count, corrected 2026-09-22), `existing_structure_borrowed_primary` (the narrower `match_type='nearest'` cut — 47,882, RES-scoped; originally-cited 53,926 was likewise unscoped), and a `validate_only` invariant `existing_structure_onlot_share_low` (real `ST_Intersection(parcel,building)`-measured on-lot share below a zone-aware floor — RD 0.90 / attached 0.50, non-tunable literals as of the 2026-09-22 slice-0 peel finding O1, measured 59,661). All three WARN with a declared `retighten_when`, never FAIL — a structural fact of the un-apportioned massing link (Spec 56 §3), not a bug any single run introduces; each closes once slice 1's on-lot apportionment lands.
- **ES-5 Cost-model alignment.** `imagery_roof_gfa_sqm` IS Spec 83 Step A for the existing structure; the Phase-2 reno scenarios (basement/storey/kitchen/bath/interior) derive their Area_Eff from these fields via the archetype `geom_basis`. No cost-model change in Phase 1 (fields only).
- **ES-6 Existing-structure honesty (WF3-A, 2026-06-24 — mig 193/194).** The massing `estimated_stories`/`max_height_m` are tree-canopy contaminated (dev: `existing_stories` mode = 3 storeys on a bungalow city, only 495 one-storey across 277K low-rise; heights to 85–95 m). Footprint is independent of canopy (`corr(fp,height)=0.116`) and trusted. Three changes, all inside the existing-structure pass (`buildExistingStructureSql`):
  1. **RETIRE** `existing_stories` + `existing_height_m` → always `NULL` on re-enrich (columns kept in `EXISTING_COLS` so the array-driven UPDATE NULLs their values; not dropped this phase). They are never trustworthy and we do not keep a value we know is wrong.
  2. **Mislink guard** (`mislink_footprint_lot_tol` logic-var, default 0.05): `imagery_roof_footprint_sqm > lot_size_sqm × (1+tol)` ⇒ the WRONG building was linked (block/neighbour attribution — dev: RT 56.5%, R 23.8%). A `mislink`/`eff_footprint` LATERAL NULLs the **entire** existing structure (footprint, dims, gfa, all `cur_*`) + sets `existing_data_quality_flag='footprint_exceeds_lot'` + `existing_structure_confidence='low'`.
  3. `imagery_roof_gfa_sqm` is no longer `footprint × contaminated-stories`; it is a **forward-compat default = `cur_pot_2story_gfa_sqm`** (footprint × 2). No live consumer reads it today (cost model computes GFA from `building_footprints`; Spec 83 §97); kept honest (NULL on mislink). `existing_data_quality_flag` joins `EXISTING_COLS` (11 cols) → propagates via §8e. ~~The heritage-freeze (§4 MB-5) reads `building_footprints.estimated_stories` via the max-build `massing` CTE — a different code path from the retired `parcels.existing_stories` column — so it is unaffected.~~ **CARVE-OUT CLOSED (WF3, 2026-07-02):** that heritage read WAS the last consumer of the contaminated massing storeys (dev: 5,107 `heritage_existing` parcels at 5–79 storeys / FSI to 57). The massing `estimated_stories` is now retired from the heritage-freeze too — heritage storeys use the bounded `stories_calc` (see MB-5). The `existing_stories` symbol is fully removed from the max-build `massing` CTE.
...
- **SC-4 Storey-height refinement (Part C).** `max_build_stories` height→storey translation is use-class-aware (`buildStoreyHeightCase`: residential = externalized `storey_height_m` 3.0; commercial/employment/institutional ≈ 4.0) instead of a flat 3.0; new `max_build_stories_basis` (`'bylaw'` when the by-law gives a storey count, else `'pocket'`/`'derived'`; the heritage-frozen `'existing'` value is **RETIRED WF3 2026-07-02** — heritage now flows through the same `bylaw`/`pocket`/`derived` basis as non-heritage, since it uses `stories_calc`). The height→storey translation makes the add-storey headroom robust to the absolute storey-height. (Re-touches the shipped max-build derivation; `MAX_BUILD_COLS` 16→17.)
...
- **SC-7 Current-building GFA range (WF3-A, 2026-06-24 — mig 193/194).** Replaces the single contaminated `existing_gfa` with a **menu of priceable scope options** off the trusted footprint (we present options, not one number; per-project scope is later selected from the permit/CoA description — Phase 3 / Spec 83). Four new `SCENARIO_COLS`: `cur_floor_gfa_sqm` (= footprint — the known single floor; serves basement / single-storey / minimum, cost *rate* differentiates not geometry; HIGH confidence), `cur_pot_2story_gfa_sqm` (= footprint × 2, always emitted), `cur_pot_3story_gfa_sqm` (= footprint × 3, **NULL unless the pocket supports 3** i.e. `max_build_stories ≥ 3`), `cur_gfa_range_basis` (`'1-2'` | `'1-3'` ASCII; NULL when `max_build_stories` or footprint unknown). Pure mirror `computeCurGfaRange` in `max-build.js` (unit-tested). The shipped `cur_basement_gfa_sqm`/`cur_storey_gfa_sqm`/`cur_interior_reno_gfa_sqm` are **deprecated** (NULL-cleared, kept in `SCENARIO_COLS` so the array-driven SET writes NULL); `ARCHETYPE_GEOM_BASIS` remaps BAS→`cur_floor_gfa_sqm`, ADD→`cur_floor_gfa_sqm`, INT→`cur_pot_2story_gfa_sqm` (dual-path .js/.ts).
...
- **C2 (SHIPPED, mig 196/197):** parcels have NO neighbourhood data → C2 spatial-joins `parcels.geom`→`neighbourhoods` (a LATERAL in the `scope` CTE: `ST_Contains(n.geom, ST_Centroid(p.geom))`, `ORDER BY n.id LIMIT 1` deterministic; GIST-served). It (a) reconciles `max_build_stories` 3-way (bylaw → pocket p50 `LEAST(p50, height_implied)` legal-cap basis `'pocket'` → derived), (b) adds `max_build_stories_aggressive` (pocket p90, UNCAPPED — the market ceiling via variance) + `market_exceeds_bylaw` (p90 > height_implied → CoA/variance hotspot), (c) surfaces `neighbourhood_id` (the reusable link, parcel-only — NOT propagated, permits have their own) + `neighbourhood_cost_premium` on EVERY parcel. **Premium parity:** `buildPremiumCase` is generated from `DEFAULT_PREMIUM_TIERS` (`cost-model-shared.js`) — the SAME constant the permit cost model applies (`compute-cost-estimates` builds config without `premiumTiers` → `computePremiumFactor` falls to the default), so a parcel and its permits get the identical 1.00–1.85× multiplier. NULL income (out-of-neighbourhood ~4%) → 1.00. (The DB `income_premium_tiers` logic-var is a different shape + display-only today; threading it into BOTH surfaces is a future Spec-83 WF.) The per-option DOLLAR costing — each reno/build option × trade-set base-rate × this premium, cross-checked vs industry $/sqft — is a separate future WF that consumes `neighbourhood_cost_premium`.
</storey-norms>
...
- **Max-build DB integration** (`src/tests/db/enrich-parcels-maxbuild.db.test.ts`, gated `BUILDO_TEST_DB=1`): lot-confidence tiers; geometric footprint vs coverage cap; ravine fixed-setback; heritage freeze with/without massing; garden-suite ravine exclusion; NULL-on-low-confidence; idempotent re-run. *(WF3 Phase 1 correction: this list previously ALSO claimed corner/through-lot reduction and narrow-lot-clamp coverage that did not exist — the false clauses are removed; corner width is now locked by the dedicated `enrich-parcels-maxbuild-corner.db.test.ts` (D-A), and the below-floor clamp by `enrich-parcels-clamp.db.test.ts` (D-C).)*
