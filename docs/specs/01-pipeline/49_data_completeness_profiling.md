...
**Two-layer parcel observability (WF2 — the coverage↔sanity boundary).** `assert_global_coverage` (sources) answers **"do the values EXIST?"** (existence/coverage — `IS NOT NULL` counts + the reason distribution). Its sibling **`assert_parcel_sanity`** (`scripts/quality/assert-parcel-sanity.js`, lock 107, sources chain immediately after coverage) answers **"are the values CORRECT?"** — the zone-aware BOUNDS + cross-field INVARIANTS + per-zone DISTRIBUTION from `scripts/analysis/parcel-sanity-audit.js` (`runSanity(pool)` — one folded `COUNT(*) FILTER` scan + parallel distributions, ~24s). Verdict is row-derived (Spec 48 §3.6) via a `gate` field on each check: a **`gate:true`** ZERO-BASELINE physical-impossibility/mislink/retired invariant that goes non-zero → **FAIL** (a definite regression, chain RED); a known non-zero residual → **WARN**; distribution outliers + visibility counts → **INFO** (never verdict-driving — they fluctuate on a 437K set). The status is derived purely from `gate`/`sev`/count — no per-check-id branching (no parallel-boolean, §3.6). Read-only Observer (`records_total:1`, no writes). Renders on the Spec 26 dashboard like any step (no Inspect link — no `telemetry_tables`).
...
| CoA Step D — compute-coa-cost-estimates | coa_applications.estimated_cost | `estimated_cost IS NOT NULL` | denominator = `coaTotal` on the global 90/70 rail (drift corrected 2026-08-03 — the implemented gate was never the "active CoAs ≥ 80%" this row previously claimed). **ACCEPTED BASELINE (Pipeline Rehab P4, 2026-08-03):** live coverage 61.1-61.2% is a structural gap owned by Spec 80 Phase 4 (forecast/cost reconciliation); the would-be FAIL emits as accepted-WARN in the gate with the `coa_cost_coverage_gate_accepted` row pair (Spec 48 §4.5/§4.9); acceptance self-retires at ≥ 90% |
