...
- **AP-D1 — PINNED, ruling owed.** §3 above narrates a `maint_stage = REGULAR` filter and an `address_status ∈ {NULL, CURRENT, NONE}` acceptance set; the loader has NO filter on either field — every source row inserts (measured: `grep maint_stage/address_status scripts/load-address-points.js` ⇒ INSERT column references only, zero WHERE clauses). The conversion carries the CODE unfiltered (zero-diff against the live table) and PINS the spec/code divergence as a `deviations[]` entry with both readings recorded: (a) the spec is aspirational and the unfiltered code is right; (b) the spec is right and the loader over-inserts RESERVED-stage (~1.5%, ~7.7K rows) and non-accepted-status rows. **Ruling owed** (operator/orchestrator) before any fix; the fix, if ruled, is a separate post-cutover commit with its own RED lock.
