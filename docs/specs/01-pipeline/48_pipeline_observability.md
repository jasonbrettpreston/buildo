...
**EP-D17 extension (WF3, 2026-09-10) — per-entry cost rows for the invariants[]/plausibility[] executor.** `sys_duration_ms` above prices the WHOLE run; a run-end `invariants[]`/`plausibility[]` entry (Rule 13's R-T addendum, §2 Rule 13) needed the SAME `{metric, value, threshold:null, status:"INFO"}` shape PER EXECUTED ENTRY, keyed `${entry.id}_duration_ms` (`scripts/lib/step/index.js`'s `durationRow`, an `extraRows` contributor — the same undeclared-process-row convention as `dry_run_no_writes`/`before_image_written:<table>`/`retired_var_row_present:<name>`, never a declared `checks[]` entry, since there is nothing for a compute to observe about its own executor's timing). Populated regardless of whether the entry PASSed/WARNed/FAILed (a timed-out entry's `{error}` observation still carries `duration_ms` — how long it ran before it died), so a future entry added to `invariants[]`/`plausibility[]` cannot land without its cost becoming visible on the audit table, closing the gap EP-D17 found: 5 declared checks whose `last_measured.cost_ms` (a single local golden-capture sample) understated cloud reality by 3 orders of magnitude, with nothing on the audit table itself to catch the drift.
...
**EP-D17 addition (WF3, 2026-09-10, corrected by output-panel fix F1):** `sys_${entry.id}_duration_ms` — NOT `${entry.id}_duration_ms` — is a distinct ROW (not a per-row key on the entry's own PASS/WARN/FAIL row) — one `extraRows` entry per EXECUTED `invariants[]`/`plausibility[]` entry, `value` a number (or `null` if the caller never timed it — a fixture gap, not a live step), `threshold: null`, `status: "INFO"`, mirroring `sys_duration_ms`'s own precedent (§3.5 above) one level down from the whole-run figure to the per-check one. The `sys_` prefix is LOAD-BEARING, not cosmetic: `scripts/analysis/capture-step-golden.js`'s `scrub()` drops a WHOLE row whose `metric` starts with a declared `VOLATILE_METRIC_PREFIXES` entry — an earlier draft of this row omitted the prefix and would have made every converted step's golden master non-reproducible the instant it captured a real `duration_ms`. Producer: `scripts/lib/step/index.js`'s `durationRow`, fed by `executeEntry`'s own `duration_ms` (`scripts/lib/step/plausibility.js`), which times BOTH the success and error path (a timed-out entry's `{error}` observation still carries how long it ran before it died).

**EP-D17 addition, `execution.maintenance` rows (WF3, 2026-09-10, C4):** `scripts/lib/step/plausibility.js`'s `runMaintenance` emits one `extraRows` entry per declared maintenance target, `metric: "sys_maintenance_${table}_${operation}"` (same `sys_`-prefix mechanism, same reason — the row's `value` is a live-measured `dead_ratio` and its `duration_ms` a live wall-clock number, neither reproducible across runs). These rows carry a `note` key (free-text: which action was taken/skipped and why) IN ADDITION to `metric`/`value`/`threshold`/`status` — registered here rather than added to `VOLATILE_KEYS`, since the `sys_` prefix already drops the whole row from golden comparison; `note` is documented so a `records_meta.audit_table.rows[]` reader on a LIVE run knows it is a legitimate, intentional key on this row shape, not a stray field.
