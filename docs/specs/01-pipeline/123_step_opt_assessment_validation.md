...
> Registered in `docs/specs/00_system_map.md`.
...
> ⚠️ **G2 known validator defect (R-T, 2026-08-29) — vacuous-green, not a churn×complexity plot.** `step-validate.mjs`'s `scoreG2` does not compute or check for a churn×complexity plot at all — it checks whether the assessment report claims `ASSESSMENT-INCOMPLETE` and, if so, whether a reason is stated nearby. **A report that never writes that literal string returns a full 1/1 pass "vacuously,"** with no plot ever having been produced (confirmed live 2026-08-29: `compute_centroids`'s own generated scorecard reads `G2 | 1 | 1 | ASSESSMENT-INCOMPLETE not claimed (vacuously satisfied)`). Owned as an S6b build-once item (`review_followups.md:2994`, operator ruling 2026-08-25, decision 2) jointly with G7's mutation-spike gap — filed, not yet executed; tracked in the programme backlog as item `G2` (`scripts/steps/_schema/programme-items.json`, Spec 122 §10.3).
>
