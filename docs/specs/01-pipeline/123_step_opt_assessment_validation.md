...
**R-PACE-1 (operator ruling, 2026-09-10) — the compressed 3-commit form.** Once **two** steps of an archetype have landed in the full nine-commit form above, further steps of that archetype MAY land in **3** commits instead: ① assessment + red tests (PH-0…PH-7) ② conversion + golden differential (descriptor+compute+runner+captures) ③ cutover + spec diff. Every gate/floor/lock above stays unchanged (the ≥14/17 floor, hard-stop=false, all G0–G9, Rules 1–13, both-directions locks) — what moves is WHERE the per-commit evidence lives: into the assessment report's own sections (PH-0 boundary, PH-3 intent ledger, PH-5 seam map, PH-6 classification, line accounting) instead of six separate commit messages. A step choosing this form declares it with the literal marker line `**Commit form: compressed (R-PACE-1)**` in its assessment report. `scripts/analysis/step-validate.mjs`'s fast invariant #23 (`COMPRESSED-FORM-ELIGIBLE`) enforces the precondition against that marker: legal only when `template-freeze.json.archetype_profiles[A].proven === true` AND at least 2 `converted.json` entries already share archetype A — RED, scoped to the offending slug, otherwise. See Spec 124 §5 R-PACE-1 for the full ruling text.

**Spec 124 R-AH (2026-09-14) restates this form as the DEFAULT, not an option, once an archetype is eligible** — a full-form plan for an already-eligible archetype is legal only when the assessment report states a reason (fast invariant #24, `COMPRESSED-FORM-DEFAULT`, the inverse of #23 above). See Spec 124 §5 R-AH.
...
| A5 | **Pre-dispatch checklist:** reconcile stranded `running` rows → manifest-registered prune → migration via `apply-migrations.yml` → `apply-logic-variables.js` → push + assert `headSha` → wait for the terminal status of any chain sharing a slug. Pace ruling — full text is the roadmap plan's own Ask. `npm run cloud:pre` (`scripts/analysis/cloud-pre-dispatch.mjs`) is the mechanical, read-only form of this checklist — six checks, one row-derived verdict, run against the live cloud DB before every dispatch. | `.cursor/wf3_ep_d14_pass5_recovery_scan_active_task.md` §"Cloud remediation sequence" |
| A6 | **Commit 9's obligations:** register in `converted.json` AND delete the `pending` entry in the SAME commit (R-K) · **RETAIN the `step-archetype-census.json` row — flip `status: "converted"` + `converted_at`, never delete it (Spec 124 R-AO, corrected 2026-09-15; pilots 1–9 and batch 1 DELETED it, which made fast invariant #25 ARCHETYPE-PARITY unreachable — with no row left, a valid-but-wrong archetype passed silently, proved by swapping `load_ravines` to ASSERT). The retained `archetype` is the census's OWN pre-cutover value and is never re-derived from the descriptor; re-deriving it would make #25 compare a value to itself** · STD-7→`BUILT` · RE-FREEZE flipping `archetype_profiles[ENRICHER].proven` → `true` · flip `template-freeze.infra.test.ts`'s pin · regenerate freeze/backlog/scorecards. Pace ruling — full text is the roadmap plan's own Ask; the operator's 2026-09-10 authorisation covers A4–A6 as a set. | `.cursor/wf2_freeze1_precondition_active_task.md` §"What pilot 9 commit 9 must contain for STD-7" |
...
| A9 | **BATCH 2 (C5 `sources`, added 2026-09-15).** Scope = **13** conversions, not 14: `reconcile` is a declared `runner_owned` census exemption (Spec 124 R-AP), never a conversion. Phase 0 closes the BEFORE-B2 rows first, each as its own WF with its own PLAN LOCKED gate; A1(batch membership)–A5(parallelism) are RULED in the plan's own "Asks A1–A5 — GROUNDED" section. Push cadence = **commits per step, one push per archetype group, one PINNED cloud dispatch at batch close** (R-AG + R-AB + R-AL, each ruling's own wording). Phase-0 row 0.8 landed Spec 124 `R-AL`…`R-AP` with their locks. | `.cursor/batch2_c5_active_task.md` (Phase 0 rows 0.7/0.8, §Asks A1–A5 GROUNDED, §Proposed policy amendments); Spec 124 R-AL/R-AM/R-AN/R-AO/R-AP |
