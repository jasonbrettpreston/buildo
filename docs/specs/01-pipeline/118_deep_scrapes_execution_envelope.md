...
| 2. Chain soft budget | `CHAIN_TIME_BUDGET_MINUTES` (Spec 115 §2.2; run-chain checks BEFORE each step, marks remainder `skipped-budget`, ends `completed_with_warnings`) | **SET to `140` for deep_scrapes** since `d6d0bab0`/`cad1017a` (WF3 F4, landed on `origin/main` — this row corrected 2026-09-18, WF3 `wf3_deep_scrapes_failures`; the layer was live at the time of the 09-15 incident and is not the gap that incident exposed) | Any SLOW-but-finite tail step | **Live, but note the limit: the check is per-step-boundary; it cannot preempt a step already running.** A 64-min `refresh_snapshot` starting at min 121 still rides to the axe — this is exactly what happened on 09-15, and is why layer 3 (below) is the real fix, not this layer |
...
2. **Layer 3: per-step ceilings in run-chain** (statement_timeout or spawn-timeout per step, manifest-configurable) — the missing hierarchy layer; a pathological step must die in minutes at run-chain's hands, not at the platform's. **PARTIAL** (WF3 2026-09-18, `wf3_deep_scrapes_failures`): the mechanism now exists in the library for the RECORDER shape (`runRecorderPhase`'s new `execution.statement_timeout_minutes_from_config`/`phase_deadline_minutes_from_config`, reusing EP-PHASE-DEADLINE's `pg_cancel_backend` approach) and closes it for `refresh_snapshot` specifically (Spec 118 §1's own cause-B incident). Not a `run-chain.js`-level, manifest-wide ceiling for every archetype — that remains the open item.
...
4. **Layer 2 for deep_scrapes**: set `CHAIN_TIME_BUDGET_MINUTES` (140) — useful boundary-stop coverage even though it cannot preempt mid-step. **DONE** (`d6d0bab0`/`cad1017a`, WF3 F4 — corrected here 2026-09-18; this item was stale, already landed before the 09-15 incident).
...
> **§9.1 — Candidate writer classes for step 5's correlation, and one hypothesis TESTED AND NOT SUPPORTED (added 2026-08-16, from the Phase B B3 grounding).**
>
> Step 5 says "correlate with the write-history on the writers" but never enumerated what a high-churn writer looks like. One class found in Phase B is worth naming, because it is invisible in the ledger: **an UNGUARDED blanket `UPDATE … FROM (VALUES …)` that rewrites the whole table every run while reporting a near-zero `records_updated`.** `enrich-parcels.js`'s optimal-config pass rewrites ~450,175 parcels per run and `comparable_builds` ~351,899, while the step's emitted counter read `1`. A writer that relocates hundreds of thousands of rows per run and *reports nothing* defeats step 5's correlation exactly when it matters — so when correlating, read the per-pass audit rows (`*_enriched_count`), never the headline counter. (Spec 47 §11.1 already requires that counter to aggregate; that it did not is filed as a Phase B fix.)
>
> **The tempting hypothesis, and why it is NOT in this spec as a cause.** It is natural to connect that blanket rewrite to the 08-12→08-14 `refresh_snapshot` regression, since §1 names heap-correlation destruction from mass UPDATEs as the mechanism. **It was measured and the connection is UNSUPPORTED — not confirmed, not refuted, unmeasurable from the available environment:** `pg_stat_user_tables` is fully zeroed on dev (`n_tup_upd = 0`, `stats_reset = NULL`) despite 486,530 real rows, so no write history exists to correlate; the incident occurred on the cloud instance, which the analysis session could not reach; and dev's `enrich_parcels` history shows 5 runs ever, the latest 2026-07-08 — over a month before the incident window. Note also that **§1 already refuted dead-tuple bloat for this incident** (ratio 0.01) and that `parcels` carries no `updated_at`-style trigger, so the mig-115 mechanism does not apply to it either.
>
> **Recorded deliberately as a negative result.** A plausible causal story about a pathology this spec exists to explain is worse than no story — it would have been cited in future diagnoses as settled. Guarding the blanket writes remains correct on general grounds (a suppressed UPDATE relocates no rows at all), but **it must not be claimed as a fix for the 08-14 incident** unless someone correlates it against cloud write-history directly.
