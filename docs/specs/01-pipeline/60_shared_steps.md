...
3. Tier 3 — `pg_trgm` fuzzy match via `similarity()` over GIN trigram indexes, article-stripped first-letter blocking (`d704a447`) **AND** a token-overlap requirement (LW-D14, 2026-08-28: the WSIB name and the matched entity name must share at least one non-generic token — both names are tokenized on whitespace, generic stopwords stripped (CONSTRUCTION/CONTRACTING/BUILDERS/HOMES/GROUP/INC/LTD/LIMITED/CO/COMPANY/CORP/ENTERPRISES/DEVELOPMENTS/SERVICES/ONTARIO/CANADA) along with purely-numeric tokens, and the two token sets must overlap — `similarity() > threshold` blocking alone let two differently-named companies match on a shared generic word) → 0.60 confidence, capped at 1,000 matches per invocation
...
**Edge Cases:** Generic names → may match wrong WSIB entry — this was the fan-in / magnet-entity concentration named in `docs/specs/01-pipeline/122_pipeline_step_optimization.md` C1 pilot 4; LW-D14's token-overlap requirement (above) is the fix, measured live pre-fix at only 10.49% (840/8,009) of Tier-3 links sharing a genuine non-generic token, the rest matching on a bare stopword or nothing. A declared check `tier3_token_overlap` (FAIL below `link_wsib_tier3_token_overlap_fail_pct`, default 50) makes any regression visible post-write. WSIB refresh (`load_wsib`, annual cadence) → newly-unlinked rows are matched on the next incremental run; an already-linked row is never re-evaluated except by the operator-invoked tier-3 repair (A-7).
