...
Schema as-built: M-1 = migration 167 (`ravines`), M-2 = 168 (`parcels` ravine cols), M-3 = 169 (`permits`/`coa_applications` ravine cols). Advisory locks: load-ravines 59, enrich-ravines 60, enrich-permits ravine step reuses 66 (L19). Spec 49 `assert-global-coverage` ravine rows DONE (#415, `938cf1a`). Open post-deploy items: `enrich_ravines` first-deploy runbook (#412), §8f admin-UI (future sibling spec). `enrich_ravines` full-table re-run perf: **measured 35.4 min** (local, 486,530 parcels, 2026-06-10 `pipeline_runs` id 1061) — the `~77min` figure this row previously carried predates the #418 incremental-skip guard and is CORRECTED here (batch-2 row 2.1, 2026-09-18); post-conversion (ENRICHER runner) measured **~61.8 min** for the same full recompute under session-concurrent load (an upper bound, not a clean apples-to-apples baseline — see `docs/reports/2026-09-18-batch2-p2-1-enrich-ravines-assessment.md`). A later CLEAN, unloaded forced-full run (O5, batch-2 row 2.1 post-cutover peel) measured **44.7 min** against the 35.4-min legacy baseline — a ~9-min delta, RULED CLOSE-AS-NOISE (2026-09-20, WF2 post-2.1 disposition; `scripts/steps/_schema/execution-budget-disposition.json` `declarations.step_timeout.pending_why`): the 35.4-min baseline is a pre-cutover PostGIS 15-3.3 Docker run (migration head ~175), the 44.7-min run is PG 17.6; `ENRICH_SQL` is byte-identical across both (re-verified against the legacy pre-conversion `UPDATE`, diff empty — no code moved); the `ravine_join` phase alone accounts for 2,678,339 ms of the 2,680,764 ms total, preceded 7 s earlier by a 486,530-row stamp-nulling UPDATE with autovacuum finishing 5 s before exit; a 1% sample measured 6.8-8.4 ms/row (24% run-to-run variance on this host) — variance of that size alone spans the delta between two Postgres major versions on an unchanged statement. `manifest.scripts.enrich_ravines.step_timeout_minutes` will be set from the batch-2 CLOUD PARTIAL RUN per Spec 124 R-AQ, not from either local figure.
