...
- **Skip reasons with counts.** 2026-08-07 had many steps skipped with **null `skip_reason`** `[READ]`.
...
① **SDK-only, plus the envelope** — export `verdictCascade`; `current_database()` in `createPool`; read `records_meta.skipped` in `run-chain.js`; set the two **inert** budget env vars in `chain-sources.yml`; raise the ceiling into the **150 minutes of unused headroom** (180 used of 360 available); fix `pipeline.js:64`'s empty-string `PIPELINE_STATEMENT_TIMEOUT_MS` crash. **And fix the three strand factories** — `assert_schema` (8 throws, **0 `finally`**), `assert_data_bounds` and `assert_engine_health` all hand-roll a `pipeline_runs` row and strand a `running` row on any throw `[READ]`. That matters for iteration speed before it matters for correctness: today a failed run can wedge the next one behind a stranded row, and one strand ran **39 days**. Zero step scripts touched; closes O6 across 27/27, E2 for 3 of 3, and the gate hole for all three ledger gates.
