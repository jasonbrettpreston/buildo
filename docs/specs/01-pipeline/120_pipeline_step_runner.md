...
**Self-skip clarification (WF3 VRD-SKIP, 2026-09-09, Spec 124 §2 Rule 10 rung b):** the *Audit table verdict* row keeps its three values — a `self_skipped` terminal folds to **WARN** (never `PASS`, never a 4th value); it is the **Run status** column (`self_skipped`) plus `records_meta.skipped`/`reason` that carry skip identity, not the verdict. `deriveVerdict`/`SEVERITY_RANK` (`scripts/lib/step/verdict.js`) are unchanged — this is a declared audit-row severity choice (`skipRecordsMeta`'s `status` row, `scripts/lib/step/index.js`), not a vocabulary change. Two related items remain **open, named not hidden**: `pipeline_runs.status` still carries **no DB CHECK constraint** (the ⚠️ item below is unbuilt); and **five un-converted scripts** (`compute-build-norms.js`, `compute-cost-estimates.js`, `compute-storey-norms.js`, `dispatch-notifications.js`, `quality/assert-network-health.js`) hardcode a `verdict:'SKIP'` on their own contention paths, and cloud carries live `verdict='UNKNOWN'` rows (`backup_db`) — both outside this table's three-value *Audit table verdict* vocabulary and outside Rule 10's `VERDICT_LIBRARY_CORPUS` boundary.
