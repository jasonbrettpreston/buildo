...
| `event_date` | DATE | nullable | **Real-world date the status change occurred** (sourced from CKAN: `permits.issued_date` / `completed_date` / `application_date` / `coa_applications.decision_date` / `hearing_date`, depending on writer + `to_status`). NULL when source provides no date (most permit non-milestone transitions; all classifier-derived rows). Inspector (Spec 76 §3.5) renders `COALESCE(event_date::timestamptz, transitioned_at)` with a 'detected' badge when NULL. **WF3 Pass-2.5 Finding C** — mig 160 (Phase 1, 2026-05-21) added the column; **Phase 2 (2026-05-21) populated it for 7 permit-side milestone statuses via `load-permits.js`** (see Spec 50 §3); **Phase 3 (2026-05-22) populated it for 8 CoA-side milestone statuses via `load-coa.js`** (see Spec 51 §3); **Phase 4 (2026-05-22, mig 161) added a CHECK constraint** `lifecycle_status_history_classifier_event_date_null` locking classifier-derived rows (`detected_by = 'classify-lifecycle-phase.js'`) to `event_date IS NULL` at the DB level — defense against future writer refactor drift. Phase 5 wires the Inspector. No historical backfill — pre-fix rows retain `event_date = NULL`. |
...
**Phase F.3 consumer wiring (DELIVERED 2026-05-17 commit `632e57d`):** `compute-opportunity-scores.js` consumes CoA-side `trade_forecasts` rows (F.1 outputs) and JOINs against CoA-side `cost_estimates` (R5.5 + mig 145) and `lead_analytics` (F.2 UNION). Branch discrimination via `lead_id LIKE 'coa:%'` with a `parseBranchFromLeadId` module-local helper (regex `^(coa|permit):` for ambiguity safety). NO separate UNION SOURCE_SQL — `trade_forecasts` is already-UNIONed substrate from F.1. Per-branch counter splits with retry-safe `updatedPermit/_coa` accumulation (HIGH-I post-`withTransaction`-resolve pattern) + 10 new audit rows + 19 records_meta entries. CRIT-A defensive integrity probe (EXISTS+sample form, capped at 50) catches upstream lead_key format drift symmetrically across both branches. `ce.lead_id` orphan discriminant (mig 145 NOT-NULL PK) eliminates F.2-style false-positive orphan classification.

**Phase F.2 consumer wiring (DELIVERED 2026-05-16 commit `66884af`):** `update-tracked-projects.js` consumes CoA-side fields from `coa_applications` (lifecycle_phase, lifecycle_stalled, lifecycle_group, status, decision, hearing_date, lifecycle_classified_at, last_seen_at) and CoA-side forecasts from `trade_forecasts.lead_id`. CoA branch routes on `tracked_projects.lead_id LIKE 'coa:%'`; mutually exclusive with permit branch via `tp.lead_id IS NULL OR NOT LIKE 'coa:%'` guard on Branch A. The 4 module-local pure helpers (`selectCoaStallThreshold` 3-tier per-status with null-guard, `isCoaInImminentWindow` hearing-date UTC-explicit parse, `isCoaDecisionTerminal`/`isCoaTerminalState` for auto-archive precedence) implement Spec 82 §4 CoA Lead Handling. `isCoaTerminalState` covers BOTH terminal decisions AND status IN ('Complete', 'Closed') — Spec 84 §3 row P20 mapping.

**Phase F.1 consumer wiring (DELIVERED 2026-05-16 commit `4d58444`):** `compute-trade-forecasts.js` reads `phase_stay_calibration` CoA-side rows (WHERE `permit_type IS NULL`) via a separate query from the legacy `phase_calibration` permit-side query. 5-tuple cohort lookup keys on **`from_seq`** matching the lead's current `lifecycle_seq` (the cohort row's `from_seq` represents the phase being EXITED in the LAG window — measures stay duration IN that phase). Multiple `to_seq` variants for the same `from_seq` are collapsed by keeping the row with maximum `sample_size`. 5-level fallback chain: exact → `(pt, __ALL__, fs)` → `(__ALL__, tc, fs)` → `(__ALL__, __ALL__, fs)` → default. Audit-verdict gate (Spec 42 §6.11 Phase F design constraint per follow-up #131) refuses to UNION CoA rows unless the most-recent `compute_phase_calibration` permits-chain run within `coa_gate_calibration_window_days` (default 7) shows `verdict='PASS'` AND `status='completed'`.
