...
| `event_date` | DATE | nullable | **Real-world date the status change occurred** (sourced from CKAN: `permits.issued_date` / `completed_date` / `application_date` / `coa_applications.decision_date` / `hearing_date`, depending on writer + `to_status`). NULL when source provides no date (most permit non-milestone transitions; all classifier-derived rows). Inspector (Spec 76 §3.5) renders event_date directly when set; falls back to date portion of `transitioned_at` with a 'detected' badge when NULL. **WF3 Pass-2.5 Finding C — FULLY SHIPPED** — Phase 1 (2026-05-21 mig 160) added the column; Phase 2 (2026-05-21) populated 7 permit milestone statuses via `load-permits.js` (Spec 50 §3); Phase 3 (2026-05-22) populated 8 CoA milestone statuses via `load-coa.js` (Spec 51 §3); Phase 4 (2026-05-22 mig 161) added CHECK constraint `lifecycle_status_history_classifier_event_date_null` locking classifier-derived rows to NULL; Phase 5 (2026-05-22) wired the Inspector cross-stream timeline rendering with TZ-deterministic ORDER BY + 'detected' badge fallback. No historical backfill — pre-fix rows retain `event_date = NULL`. |
