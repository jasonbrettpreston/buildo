...
- **Violation reads as:** a schema-validation throw at construction. *(Note: the declaration is enforced; the runtime mechanism that would make the declared posture actually TRUE — auto-detecting a crashed run and forcing FULL — does not exist yet. That is tracked as Spec 122 R-B, ⚠ GAP, lands pilot 4 — see the §5 register — not a fresh Spec 124 gap: this rule only requires the posture be truthfully declared, and it is.)*
...
| R-B | `recovery.interrupted` is a required, truthful declaration on any destructive-retraction write target; the runtime auto-detect-and-force-FULL mechanism is not yet built (⚠ GAP, lands pilot 4) | Rule 12 | Spec 122 R-B |
...
from a name or a comment. Cite the governing spec section. *(Spec 119 §1 stage 1–2; e.g. LM-D14 was found
only because a forced-FULL capture was inspected, not assumed — "found by EXECUTING, not by reading.")*
