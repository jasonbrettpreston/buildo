...
Twelve rules. Each is enforced today, or marked ⚠ GAP with the artifact that would close it.
...
- ⚠ **GAP G-1** — the *preference order itself* (descriptor > check > shape rule > new field) is applied by human adjudication in the Intent Ledger, not machine-checked. Nothing stops a future disposition from reaching for "new schema field" before trying "declared check."
