...
- ⚠ **GAP (R-T addendum, 2026-08-29) — Rule 13 needs a reworded boundary.** As built, `step:validate` conflates two different questions under one name: whether a step's *process* was followed (G0–G9, the policy coverage matrix) and whether a step's *data* is plausible (Spec 30 §5's bound doctrine, Spec 79's plausibility checks, Spec 120 §5's per-field invariants). Today only the process half is enforced. **The reworded rule: "the validator validates DATA (Spec 79/120/30 §5); the scorecard validates process, from structured artifacts."** The scorecard (this rule, as built) stays exactly as specified above. The validator half is ⚠ **GAP**, owned `wf: validator-v2` (`scripts/steps/_schema/programme-items.json` id `VAL-WF2`, gate `batching_prereq`) — it would add invariant bounds that are threshold-derived/reachable/last-measured-value-beside-the-bound (§9 below, citing Spec 30 §5), `checks[].plausibility[]`, a seam-validation pass over `outputs.invalidates` edges (closing VAL-3/LDG-2/LDG-7's concern), chain-end synthesis automation, and a structured (non-markdown-regex) artifact scorecard replacing `step-validate.mjs`'s current `section()`/`splitTableRow()` prose-scraping (programme item `REGEXSC`, marked `SUPERSEDED` by this same item). Filed, not built, this commit.
