...
- ⚠ **GAP G-1** — the *preference order itself* (descriptor > check > shape rule > new field) is applied by human adjudication in the Intent Ledger, not machine-checked. Nothing stops a future disposition from reaching for "new schema field" before trying "declared check." **R-R note (2026-08-29):** `step:validate`'s Rules-1–13 policy coverage matrix (§5 R-R) prints G-1 as PROSE-ONLY for every step, so the gap is at least VISIBLE per-step now rather than silently assumed closed; R-P's own gated-skip `checks[].when:"pre"` rung (b) requirement is tracked separately and has not yet folded into this rule's text.
...
- ⚠ **GAP G-4** — no check ties `on_invalid:"fail"` to "verdict- or write-affecting"; applied by author judgment, reviewed narratively.
...
- ⚠ **GAP G-2** — no test asserts that a `preserved-in-compute` disposition has a corresponding `checks[].why`. A future disposition could mark something `preserved-in-compute` and simply not write the rule down anywhere.
- **Archetype variance:** none.
...
- **Violation reads as:** a schema-validation throw at construction. *(Note, updated post pilot 4: the DECLARATION half is CLOSED — `recovery.interrupted` and the paired `recovery.before_image` (R-M) are both required, truthful, schema-enforced fields, and R-M's before-image mechanism is BUILT and LIVE-FIRED (`d07529af`, LG-17 — `link_massing`'s genuine forced-FULL wrote a 520,492-row before-image file before its retraction ran). The runtime mechanism that would make the CRASH-RECOVERY half of the posture actually TRUE — auto-detecting a `crashed`/stuck-`running` prior run and forcing FULL on the next invocation — remains ⚠ GAP, OPEN, and is carried forward as a pilot 5 item (Spec 122 R-B). This rule only requires the posture be truthfully declared, and it is; the reader that would make "declared" and "automatically enforced" the same thing does not exist yet.)*
...
| R-B | `recovery.interrupted` is a required, truthful declaration on any destructive-retraction write target. **CLOSED for the declaration + before-image (R-M) as of pilot 4** (`d07529af`, LG-17 — before-image live-fired on a genuine forced-FULL). **Still ⚠ OPEN: the staleness gate's crashed/stuck-`running` reader that would auto-detect an interrupted retraction and force FULL on the next run** — carried forward as a named pilot 5 item | Rule 12 | Spec 122 R-B |
