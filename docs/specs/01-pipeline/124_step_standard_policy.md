...
> fallback, planned — pilot 7 not yet implemented). **Enforced by:** a new ast-grep rule,
...
| R-W | **Compute MUST NOT branch on PostGIS availability** — `hasPostGIS`, `pg_extension`, `postgis_version` (identifier, property access, or a string/SQL fragment naming one), or a `try`/`catch` wrapped around a PostGIS-only call, are the same anti-pattern: a second, silently-selected algorithm hidden inside compute. `guards.requires: {kind: "extension", name: "postgis", on_missing: "fail"}` is the only legal form — the step refuses before compute runs. Three precedents: `link_massing` A-8 (grid/haversine fallback, retired), `compute_centroids` A-1(a) (arithmetic-mean centroid fallback, retired), `link_parcels` A-1 (JS nearest-parcel fallback, planned — pilot 7 not yet implemented). Filed at Fold B item 7 of the pilot 7 assessment, proposed for the Spec 122/123/124 owner's own fold, not self-authorizing until ratified here | Rule 2 (compute is just compute) addendum | pilot 7 assessment (`docs/reports/2026-08-30-pilot7-link-parcels-assessment.md`) §0.8 item 7; `scripts/ast-grep-rules/compute-shape.yml` `compute-no-postgis-branch`; `src/tests/step-conformance.infra.test.ts` `COMPUTE_RULE_IDS` |
