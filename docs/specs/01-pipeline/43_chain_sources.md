...
| 16 | `link_massing` | `link-massing.js` | Link parcels to building footprints via building-centroid-in-parcel PostGIS predicate. `--full` (via `manifest.scripts.link_massing.chain_args.sources`) now **permits** a full relink; the **WF2 P11-2 gate** does one only when the `building_footprints` count or `LINK_MASSING_CODE_VERSION` changed (Spec 56 §3), else incremental. **WF3 S0.3 (2026-09-21):** +1 plausibility row `borrowed_primary_links` (WARN) — the PRODUCING step's own visibility into a `match_type='nearest'` primary that is ALSO another parcel's primary (measured 101,799), the producer-side mirror of `assert_parcel_sanity`'s `existing_structure_borrowed_primary` | parcel_buildings |
...
| 23 | `compute_parcel_cost_estimates` | `compute-parcel-cost-estimates.js` | Top-down EXTERNAL-cost menu of the reno scenarios per parcel (`parcel_cost_menu` JSONB + `cost_fb_total`/`cost_coa_total` scalars) off the max-build envelope + neighbourhood build norms (Spec 88 P1). CORRECTED 2026-09-21 (batch-2 row 2.4 cutover): the script has no `--full` mode and no incremental mode at all (`cli()` parsed only `--dry-run`/`--limit=N`, both retired at conversion) — every non-skipped run full-streams every residential parcel; the "cascade" named below was really the (now-retired, CPCE-A1) run-ledger gate forcing a RUN, not a `--full`-triggered recompute. **CORRECTED 2026-09-21 (WF3 CPCE-D2 CLOSED):** the step also RETRACTS (NULLs) the 16 cost/FSI columns of any parcel a prior run priced that a subsequent re-zoning has since moved outside the residential population — `ctx.retract(1, [])`, called before the pricing pass, guarded and self-extinguishing (Spec 88 §2.1). **WF3 S0.2 (2026-09-21/22):** the pricing stream + its retraction + its `cost_by_zone` observability all now honour the FULL product-scope bound (Spec 88 §2.1) — a residential-zoned parcel out of the max-build model or with existing GFA over `product_scope_max_existing_gfa_sqm` (750 m²) is neither streamed nor priced; 18,851 parcels excluded on the converging run, $80.43B of `cost_gut_total` retracted. CONVERTED (Spec 122 §5.1) | `parcels` |
...
| 25 | `assert_parcel_sanity` | `quality/assert-parcel-sanity.js` | Value-CORRECTNESS gate (this spec §Core Logic item 11; coverage↔sanity boundary Spec 49 §2): zone-aware BOUNDS + cross-field INVARIANTS + per-zone DISTRIBUTION over all residential parcels; `gate:true` physical-impossibility/mislink checks FAIL the chain, known residuals WARN. Runs after enrich+cost so it reads FINAL values. **WF3 S0.3 (2026-09-21):** +3 checks (`existing_structure_shared_with_other_parcel`, `existing_structure_borrowed_primary`, WARN — the `parcel_buildings`/`match_type`/`is_primary` blind spot `review_followups.md:3123` named; `ravine_constrained_carries_priced_reno`, gate:true FAIL — the reno-line half of the withheld-envelope tripwire, Spec 59) + 1 `validate_only` invariant (`existing_structure_onlot_share_low`, zone-aware WARN, Spec 65 §5 ES-4). **WF3 S0.4 (2026-09-21/22):** `COST_ADDITION_GT50M_LEGIT` (the 42-id accept-list on `cost_addition_gt_50m`) is RETIRED WHOLE — dissolved by S0.2's product-scope bound, not curated (APS-D8, `defect-ledger.md`); the check's own bound is unchanged and now genuinely calibrated | pipeline_runs |
...
9. **Zoning + max-build + cost cascade** — `enrich_parcels` (**full re-enrich**, `chain_args.sources = --full`) writes the zoning feed + max-build envelope + existing/scenario GFAs + optimal-config + comparable-builds + neighbourhood_id (Specs 58/65/78); `compute_parcel_cost_estimates` then computes the per-parcel reno-scenario cost menu (Spec 88 P1). **CORRECTED 2026-09-21 (batch-2 row 2.4 cutover):** the cost step is not "cascaded" by the `--full` enrich — since CPCE-A1 retired its run-ledger gate it has no skip path and no incremental mode at all, so *every* scheduled run full-streams the whole `upper(zoning_class) LIKE 'R%'` population regardless of what enrich_parcels did; the 16-column `IS DISTINCT FROM` guard, not a gate, is what keeps a no-change run cheap (Spec 88 §2.1/§2.11).
10. **Phase B B3 — run-ledger gate (no-op-run avoidance):** `link_wsib`, `link_parcel_addresses`, and `compute_parcel_cost_estimates` have no dataset-version signal of their own (unlike the CKAN loaders), so each is gated by `scripts/lib/source-version.js#runLedgerGateDecision` — a consumer-side "has anything happened upstream since MY OWN last completed run that I haven't accounted for?" check, run inside the advisory lock before any real work. SKIP fires only when the step has completed at least once AND every pipeline_runs row for its upstream producer(s) since then is `status='completed'` with zero `records_new`/`records_updated` (any non-completed status — including `deferred_to_full` — forces a RUN, fail-safe). A SKIP still emits a COMPLETED `pipeline_runs` summary (DS4) so the next evaluation's own-last anchor advances. `link_wsib`'s upstream is `load_wsib` (wsib_registry) ∪ `builders` (entities.name_normalized); wsib_registry linkage is MONOTONE (`load-wsib.js`'s UPSERT never touches `linked_entity_id`), so an upstream reload can only add newly-unlinked rows, never silently un-link an already-matched one. `link_parcel_addresses`'s upstream is `address_points` ∪ `parcels`. **CORRECTED 2026-09-21 (batch-2 row 2.4 cutover, CPCE-A1 RULED, Spec 124 R-AR.1):** `compute_parcel_cost_estimates` is NO LONGER gated this way — `runEnrichPhase` (its runner shape) has no `ledgerGatedSkip` call and the step has no lineage-stamp column to hold a Layer-2 predicate (the `link_wsib`/`link_parcel_addresses` remedy, via `runCascadePhase`/`runMaterializePhase`, is structurally unavailable to an ENRICHER). Every scheduled run now streams the whole declared population; the 16-column `IS DISTINCT FROM` write guard keeps steady-state re-write cost low. The version-signal stamps (`rates_as_of`/`index_updated_at`, still diffed against `archetype_cost_rates`/`cost_escalation_index`'s live values) SURVIVE as `emits[]`-only observability, not a gate input. `--dry-run`/`--limit=N`/`COMPUTE_PARCEL_COST_FORCE_FULL` are retired along with the gate they existed to bypass.

    Separately, `enrich_ravines` and `enrich_heritage` each carried their OWN incremental skip (the #418 mechanism): a cheap pre-transaction `COUNT` of geom-bearing parcels whose lineage stamp is stale against the current source dataset version, scoped to the SAME eligibility their enrichment SQL uses (`geom IS NOT NULL AND NOT ST_IsEmpty(geom) AND ST_IsValid(geom)`) — a small population of invalid-geometry parcels is permanently excluded from both the enrichment UPDATE and this stale-count, so they can neither force a stale count forever (the "wedge-open trap") nor silently satisfy one. ⚠️ **CORRECTED 2026-09-20 — that Layer-1 mechanism is RETIRED in both steps** (batch-2 rows 2.1/2.2, `83b0cb98` / `c3c36315`): the Spec 122 ENRICHER runner has no gated-skip branch by design, so the identical eligibility predicate now lives **inside the join UPDATE's own scope** (Layer-2), which subsumes it — a zero-stale run is already a 0-row write, and the `parcels_ravine_enrich_skipped` / `parcels_heritage_enrich_skipped` audit rows are re-derived from the write count rather than from the retired branch. Both are declared `deviations[]` in their descriptors. One consequence is load-bearing and belongs to `enrich_heritage` only: because its lineage stamp is a content hash of the two source datasets, a levenshtein-threshold change or an address-only parcel edit no longer re-stales on its own, and **`ENRICH_HERITAGE_FORCE_FULL` is the declared remedy** (Spec 61 FOLD-I5). `enrich_ravines` declares no `force_full` override.
