...
## WF3 S0.2 (2026-09-21/22) — 26 differences measured, all population-narrowing consequences

Recapturing `sources.json`/`standalone.json` after S0.2's product-scope bound landed produced 26
genuine differences (13 fields × 2 files), none a data-drift regression — every one is the direct,
expected arithmetic consequence of `residential_parcels_examined` shrinking from 437,279 to
410,754 (18,851 parcels excluded: 18,529 out of the max-build model + 322 over the residual
`product_scope_max_existing_gfa_sqm` cut):

- `null_geom_basis_count` moved (fewer NULL-geom-basis parcels remain in the smaller population —
  the excluded parcels are no longer streamed at all, so they can no longer contribute to this
  count either way).
- `area_confidence.medium` moved (the `medium` confidence bucket is a straight count over the
  scanned population; a smaller population scans a smaller bucket).
- Every `line_coverage.<id>` entry moved — `max_build`, `coa_build`, `solar_max`, `solar_coa`,
  `garden_suite`, `laneway_suite`, `kitchen`, `bath`, `garage`, `basement_underpin`, `basement` —
  each is a COUNT of scanned parcels carrying that priced line; excluding 18,851 parcels from the
  stream necessarily lowers every one of the 11 (only `gut`/`addition` are unaffected in this
  specific measured run, since none of the 18,851 excluded parcels happened to carry those two
  lines in isolation from the others — a coincidence of this data snapshot, not a declared
  invariant).

All 26 are re-derivable directly from `residential_parcels_examined`'s own before/after values
(437,279 → 410,754) and are not independently suspicious — no value moved in a direction or
magnitude inconsistent with a straight population narrowing (every line-coverage count decreased,
...
## Addendum (2026-09-23, batch-2 row 2.5 E3/C4) — `parcel_cost_lines` read + `cost_lines_updated_at` emit, zero-diff otherwise

Row 2.5 (`.cursor/batch2_p2_5_pricing_admin_active_task.md`, Fold A2/A10, D4/D5) moved the EDITABLE
half of the 13-line cost catalogue (`archetype`, `base_confidence`, the fit-gated permission
vocabulary) out of `scripts/lib/parcel-cost.js`'s frozen `PARCEL_COST_LINES` array and into the new
`parcel_cost_lines` table (migration 248, row 2.5 C0). `readCostContract` now reads that table and
`buildParcelCostMenu` consumes the merged catalogue via the new required `opts.lines`
(`mergeCostLines`) — a pure structural-refactor with the same seed values as the old literals, so the
golden `--compare` between the pre-E3 tree and this tree (`docs/reports/golden/compute_parcel_cost_estimates/differential/pre-2.5-{sources,standalone}.json` vs the recaptured
`post/{sources,standalone}.json`) reports ONLY two additive keys, both declared by D4:

- `meta[0].reads.parcel_cost_lines` — the descriptor's new `inputs.reads.tables` entry for the table
  the step now reads (columns `id, archetype, base_confidence, fit_permitted_values, updated_at`).
- `summary.records_meta.cost_lines_updated_at` (and the equivalent `pipeline_runs[0].records_meta.cost_lines_updated_at` on the standalone/`none` invocation) — the descriptor's new `emits[]` entry,
  the freshest `parcel_cost_lines.updated_at` seen during the run.

No table-state, counter, or verdict difference — the zero-diff proof (Fold D5) holds. The pre-E3
baseline pair lives under `differential/pre-2.5-*.json` (not `pre/`, which this report's §"PRE
goldens" section already anchors to the legacy pre-conversion JS runtime baseline — a different
lineage `--compare` never binds by fingerprint); `post/{sources,standalone}.json` were overwritten
with the post-E3 recapture (C4, source_fingerprint recomputed over the changed
...
| 8 | compute_parcel_cost_estimates | PASS | G-4: 20 declared, 5 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
...
- compare ran: true · diffs found: 359 · unexplained: 0
...
| 3 | Tunables externalized | enforced-green | G-4: 20 declared, 5 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=52653B notes=0B checks=24 rows records_meta=6446B (newest post/ capture) |
