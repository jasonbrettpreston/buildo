...
**Score: 7/17** · G9 Reflection: PASS · G4d fence-lock coverage: PASS · G-shape: PASS · **Hard stop: YES (G8)**
...
| G7 | 3 | 3 | file=true fences=0 it-count=40 RED-evidence=true |
| G8 | 0 | 3 | missing-invocations=0 missing-pre-invocations=0 stale-fingerprints=2 unexplained-diffs=0 |
| G9 (binary) | PASS | — | heading=true low-confidence-table=true recurring-table=true |
| G4d (fence<=lock) | PASS | — | fences=0 lock-it-count=40 |
...
- stale fingerprints: sources.json, standalone.json
- compare ran: true · diffs found: 309 · unexplained: 0

### Test suite (item iii)
- 1361/1370 passed (suite success=false)
...
- failing (9):
  ...
  - src/tests/golden-fingerprint.infra.test.ts > golden-fingerprint — the golden capture is a LOCKFILE (ruling R-C, 2026-08-28) > scripts/enrich-parcels.js (slug "enrich_parcels") > every post/ capture carries source_fingerprint === the CURRENT fingerprint over (step, descriptor, notes, compute)
  - src/tests/golden-fingerprint.infra.test.ts > golden-fingerprint — the golden capture is a LOCKFILE (ruling R-C, 2026-08-28) > scripts/compute-parcel-cost-estimates.js (slug "compute_parcel_cost_estimates") > every post/ capture carries source_fingerprint === the CURRENT fingerprint over (step, descriptor, notes, compute)
  ...
  - src/tests/step-conformance.infra.test.ts > Rule 12 — truthful crash posture, static half (checkInterruptedPostureTruthful) > enrich_parcels: shape=enrich declares force_full_on_next_run and its runner (runEnrichPhase) is measured REACHABLE against the live scripts/lib/step/index.js
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=51510B notes=0B checks=24 rows records_meta=6335B (newest post/ capture) |
