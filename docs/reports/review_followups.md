...
| 431-FU | Spec 62 §11 WF3 (2026-06-09) — through-lot counts NAMED laneways as a second frontage | The abut-both fix landed `is_through_lot` at 11.3% (vs ~<5% expected). Sampling (`scripts/analysis/wf3-through-sample.js`) shows the residual is **real geometry** — genuine opposite-side named-street pairs in dense subdivisions — but some flags pair a street with a **named laneway** (e.g. `"Ln W Abraham Welsh S Isaac Devins"` at 3 m). The WF2 guard only excludes *unnamed* laneways. | **OPEN — fast follow-up WF3:** exclude named laneways from the through pair (gate on `toronto_centreline.feature_code` / `feature_code_desc` = Laneway, not just `linear_name IS NULL`). A street-to-street through lot should not count a lane as its second frontage; will push through toward <5% and slightly lower corner (lane-junction corners). Decide whether corner should likewise exclude lane junctions. User chose (2026-06-09) to ship the abut-both win now and queue this separately. |
