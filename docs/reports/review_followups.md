...
| 415 | WF2 Spec 59 §8e plan review (2026-06-03, 4-reviewer) — DEFER (Gemini/Observability/Integration, by design per DEC-H) | The §8e WF creates `permits`/`coa_applications`.{is_in_ravine_protection_area, ravine_distance_m} but does NOT add `assert-global-coverage.js` (Spec 49) rows for them — mirroring how #406 added the zoning-coverage rows as a SEPARATE WF3 after the zoning enrich WFs. | After §8e lands + a production run populates real ravine data, file a WF3 to add Spec 49 §4 rows for the permits/coa ravine columns (gate `ravine_distance_m` coverage over linked leads; `is_in_ravine_protection_area` is NOT NULL → INFO only). Calibrate any threshold from the first run (ravine affects a small geographic subset — likely INFO, not gated). |
