...
| 428 | Spec 61 §8e WF1 (2026-06-05) — DEFER: Spec 49 heritage completeness rows | The §8e WF creates `permits`/`coa_applications`.{is_heritage_designated, heritage_designation_type, heritage_designation_date} but does NOT add `assert-global-coverage.js` (Spec 49) rows for them — mirroring how #415 (ravine) + #406 (zoning) added the global-coverage rows as a SEPARATE WF after the enrich WFs. | After §8e lands, file a WF (the #415 pattern) adding INFO rows for the permits/coa heritage columns to assert-global-coverage.js under the `Step 9b — enrich_permits` / `CoA Step 4b — enrich_coa_zoning` labels (count-of-designated + per-type counts; INFO, not gated — heritage is a small geographic subset, like ravine). |
...
| 418 | Spec 59 full 3-chain validation run (2026-06-03, live sources→coa→permits) — DEFER PERF | `enrich_ravines` (enrich-ravines.js, lock 60) took **~77 min** on an idempotent re-run over 486K parcels even though it newly-enriched **0** rows — the set-based MATERIALIZED-centroid + LATERAL-KNN recomputes the full parcels×ravines join every cycle, regardless of whether the source `ravines` dataset_version changed. The §8d "~2 min" figure was a scoped smoke, not a full-table run; the real recurring cost on the sources chain is ~77 min. NOT a correctness issue (verdict PASS, 100% distance coverage, IS DISTINCT FROM guard correctly wrote 0 changes). | Add an incremental-skip guard: if every parcel's `ravine_dataset_version_when_enriched` already equals the current `ravines` `source_dataset_version` (no new/changed ravines since last run), short-circuit to a SKIP summary instead of recomputing the KNN. Mirrors the load-ravines version-pin pattern. Only the first run after a `ravines` refresh should pay the full KNN cost. |
