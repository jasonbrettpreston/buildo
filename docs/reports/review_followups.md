...
| 431 | Spec 62 §11 WF2 (2026-06-09) — containment→proximity FIXED; **corner/through OVER-DETECTION open (priority: is_corner_lot)** | §8d live validation: the spec's §11 `ST_Intersects(parcel, centreline)` join matched only **0.05%** of parcels — centerlines sit ~10 m off the lots. WF2 fixed it: `ST_DWithin(::geography, 20)` (geog GIST mig 175) + frontage P3 nearest-segment (not longest-intersection) + corner/through require both names NOT NULL (laneway guard). Re-validated: zero-intersection 99.95%→3.0%, **471,869 enriched (97%)**, frontage P1 91%, 8.1 min, PASS. Spec §11 corrected in-place (+ §11.0 Known Failure Modes). | **OPEN tuning follow-up (the user's KEY field):** proximity inflates the booleans — live `is_corner_lot` **24%** + `is_through_lot` **16.7%** vs typical ~13%/<5%; the 20 m radius catches non-fronting streets (cross-street a lot away → false corner; parallel street within radius → false through). Investigate + tighten: restrict corner/through to the parcel's genuine adjacent frontages (e.g. the 1-2 nearest per distinct bearing, or a tighter pair-radius than the 20 m frontage radius, or a shared-edge/azimuth-to-parcel constraint). `primary_frontage_street_name` is solid (P1 91%) — only the two flags need it. **PERF DEFER:** 8.1 min full recompute every run is acceptable now; a #418-style `centreline_dataset_version_when_enriched` stale-skip (+ the load-parcels DEC-FENCE2 entry per #430) is the optimization if the runtime becomes a problem. Spec L4b/§9 stale-text → #430. |
