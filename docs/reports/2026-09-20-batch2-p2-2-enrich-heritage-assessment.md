...
| 2a | **ADDED 2026-09-21 (§4.6 promotion feeder, for Spec 124 R-AR).** Retiring a legacy Layer-1 pre-transaction skip gate AS A MECHANISM and porting its eligibility predicate into the write's own `write_discipline.scope` — Layer-2 subsuming Layer-1, the observable re-derived from the write count — is the SAME disposition `enrich_ravines` (row 2.1, `83b0cb98`) made for the identical #418 mechanism one row earlier. **Named archetype match: ENRICHER.** It recurs next at `enrich_centreline` (row 2.3, relocated to Phase 3.10), which carries a legacy pre-transaction skip of the same shape, and — in its OTHER branch, where no lineage-stamp column exists to carry a Layer-2 predicate — at `compute_parcel_cost_estimates` (row 2.4), whose gate is knowingly retired instead (R-AR.1). |
| 2b | **ADDED 2026-09-21 (§4.6 promotion feeder, for Spec 124 R-AS).** Using a COMMITTED PERTURBATION COHORT rather than a forced FULL as the differential instrument, because the lineage stamp sits inside the write's own `IS DISTINCT FROM` guard and an unchanged-source re-run therefore writes 0 rows. **Named archetype match: ENRICHER** — and more precisely, every converted step whose `write_discipline.guard_columns` contains the stamp it writes: `enrich_ravines` and `enrich_heritage` today, `enrich_centreline` next. The forced-FULL instrument was REFUTED here at plan altitude, not discovered failing at output. |
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=41377B notes=0B checks=10 rows records_meta=4455B (newest post/ capture) |
