...
|---|---|---|---|---|---|
| L7c mass-delete terminal + "override never suppresses FAIL" (`:511-514`, `:553`) | fence `1ceebd17` feat(59) HIGH | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` + `preserved-in-validator` — check `ravine_mass_delete_pct` (`limit_from_config`, A-4) + `override.accept_anomaly[]` (A-5); `blocking:false` at commit 7 | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `checks[]` + `override` |
| Tier-2 content-hash gate + DS4 completed-row re-emit (`:354`, `:377-391`) | fence `0b230472` feat(43) HIGH | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` (`staleness.trigger[]` ×2) + `preserved-in-runner` (`buildSkipReEmitMeta`) + INFO check `ravine_load_skipped` | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `staleness` + library LG-3 |
| Streamed md5 hash-through-to-disk, never `arrayBuffer()` (`:209-215`) | fence `0b230472` feat(43) HIGH | agent pass, commit 2 (Claude) | PROPOSED `preserved-in-runner` (`ctx.acquire`, A-2) with the md5 pin as descriptor data; no-buffer rule → shape rule on `acquire.js` | **AWAITING OPERATOR ADJUDICATION §7.1** | library `scripts/lib/step/acquire.js` |
| Tier-1 validator-equality style, contentHash not in the no-validators bail (`:168-173`) | fence `0b230472` feat(43) HIGH | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` (`staleness` options) | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `staleness` |
| Temp-root cleanup `finally` `:372-373` (+ `:194` `:216` `:229`) | `1ceebd17` (no footer; load-bearing) | agent pass, commit 2 (Claude) | PROPOSED `preserved-in-runner` — acquire seam owns and cleans `tmpRoot` | **AWAITING OPERATOR ADJUDICATION §7.1** | library acquire seam |
| Six `ConfigSchema` defaults (`:46-53`) | `1ceebd17`; deferral #421 | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` — `config[]` ×6 seeded in `logic_variables` (P4; supersedes #421) | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `config` + seeds |
| `RAVINE_ACCEPT_FEATURE_COUNT_DRIFT` / `RAVINE_ACCEPT_MASS_DELETE` env reads (`:283-284`) + `*_present` WARN rows | `1ceebd17` | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` — `override.accept_anomaly[]` (A-5), rows generated by the library | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `override` |
| `String(runAt)` / sha1(last-modified) version fallbacks (`:459`) | `1ceebd17` — INTENT-UNKNOWN | agent pass, commit 2 (Claude) | PROPOSED `knowingly-retired` (unreachable; LR-D4) — **not retired by this pass** | **AWAITING OPERATOR ADJUDICATION §7.1** | LR-D4 |
| `dataset_source_license` INFO row + `LICENSE_URL` (`:41`, `:280`) | `1ceebd17` — INTENT-UNKNOWN (Spec 59 §9 row) | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` (declared INFO check, plan D-4) | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `checks[]` |
| Unit/API constants `365.25` `86400000` `4326` `3` `-4` `round3` | `1ceebd17` — structural | agent pass, commit 2 (Claude) | PROPOSED `preserved-in-compute` with values declared in `notes.json`/`limitations` (P5 satisfied); `4326` → `guards.srid` | **AWAITING OPERATOR ADJUDICATION §7.1** | compute + `notes.json` |
| `PIPELINE_NAME` #409 correction + `SPEC_VERSION '1.2'` exact pin (`:36`, `:40`) | `1ceebd17` | agent pass, commit 2 (Claude) | PROPOSED `encoded-as-descriptor-field` — `identity.name`, `identity.spec_version`; the pin is the consumer contract (plan C-9) | **AWAITING OPERATOR ADJUDICATION §7.1** | descriptor `identity` |
| Per-feature `ravine_geometry_skipped_source_id` rows (`:441`) | `1ceebd17` | agent pass, commit 2 (Claude) | PROPOSED `knowingly-retired` at peel 8b — ids move to `observation.detail` of one row (LR-D1); stays until then | **AWAITING OPERATOR ADJUDICATION §7.1** | LR-D1 closes at 8b |

No row is adjudicated by this pass. 12 rows, 0 `unknown`; 2 fence SHAs named.
...
|---|---|---|---|---|
| LR-D1 | `load-ravines.js:441` | unbounded per-feature `ravine_geometry_skipped_source_id` WARN rows | OPEN · PIN at commit 7 | **peel 8b** — one row per check, ids in `observation.detail` (plan D-5) |
| LR-D2 | `load-ravines.js:291-294` | prior-run read failure swallowed → all drift guards + both skip tiers degrade to "first run", no audit row | OPEN · PIN | **peel 8a** — staleness phase reports `prior_run_read_error` as a WARN/FAIL row (library `on_read_error`, Spec 122 §1.6) |
...
| LR-D6 | `load-ravines.js:556` | lock contention → bare `return;`, no step emit | OPEN · PIN | **commit 7** — library SKIP emit + `self_skipped` (declared diff D-10) |
...
| 8a | pending | peel: gating (two-tier staleness → library; LR-D2 read-error row) | `#154` + differential green on skip terminal |
| 8b | pending | peel: verdict/audit (metric rows → declared checks; LR-D1 retired) | `#154` + G4d F-1 lock stays green |
