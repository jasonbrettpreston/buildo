...
> **LEDGER NOTE — Fold C, operator ruling §7.1 2026-08-26 (`LR-D9`).** A THIRD load-bearing intent was found *after* the conversion landed, and it is not a commit-footer fence: Spec 59 states L7/L8 in IMPERATIVE form (§3.9 L8: "do not enter `pipeline.withTransaction`"; §3.10: "abort BEFORE entering withTransaction; **zero rows touched**"), and the pre-conversion loader honoured it by STATEMENT ORDER — `return { failed: true }` at `33786d1a:scripts/load-ravines.js:402-417` (L7) and `:443-454` (L8), both above `pool.query(VALIDATION_SQL)` / `withTransaction`. Lifting the loop into the INGESTOR archetype reordered the phases to acquire → validate → write → score, which silently retired the guarantee: the write was already committed when the FAIL row was built. This is registered as **`LR-D9` (introduced at commit 7, CLOSED · Fold C)** and closed by adding `checks[].when: "pre_write"` to the schema and the pre-write gate to `scripts/lib/step/index.js`. Disposition: **preserved-in-runner**. The generalizable lesson, and the reason it belongs in this section rather than only in the ledger: *the fence census reads commit footers and code, and neither carries a guarantee whose only expression is the ORDER of two statements — an archetype ruling that reorders phases must re-derive every "before X" guarantee stated in the step's spec.*
...
| LR-D4 | `load-ravines.js:459` | unreachable `etag`/`sha1`/`String(runAt)` version fallbacks; last is an unstable Date string | OPEN · INTENT-UNKNOWN · **DEFERRED past 8c** | no operator §7.1 ruling was given at the peel, and §7.1 is explicit that the agent that discovers may not retire. Carried forward |
...
| LR-D9 | `scripts/lib/step/index.js` `runIngestPhase` (introduced at commit 7); pre-conversion `load-ravines.js:402-417` / `:443-454` at `33786d1a` | **write-before-checks** — the archetype's acquire → validate → write → score order retired Spec 59 L7/L8's "abort BEFORE any DB write"; the FAIL row reported data that was already committed | **CLOSED · Fold C** | **operator §7.1 ruling 2026-08-26.** `checks[].when: "pre_write"` — the runner scores L7/L8 between the read-only geometry validation and the first write statement; an unaccepted FAIL skips the write entirely (no transaction, `ravines` untouched, remaining checks scored over a zeroed `written`, terminal `failed_<check_id>`). Acceptance moves the DECISION only (A-5); L8 has no override, so its abort is unconditional. `blocking:false` unchanged (D-8). Both directions locked in `violations.test.ts` + `step-library.logic.test.ts`; differential IDENTICAL on all three captures (D-20) |
...
| 8 | load_ravines | PASS | G-4: 6 declared, 4 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
| 20 | load_ravines | PASS | HB-1: execution.shape=null — HB-1 applies_when execution.shape=="enrich" only (RS-D-STA); not applicable, never a pass-by-omission |
