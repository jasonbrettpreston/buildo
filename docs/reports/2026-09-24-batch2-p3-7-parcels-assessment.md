...
## Commit ② residue — the `validateGeometries` TEXT-key CRITICAL (found, fixed, and re-verified between ② and ③)

The commit ③ cutover seat, on rebasing this worktree onto `fff52b7a` to recapture a fresh POST golden, discovered the committed ② golden (`15785c9b…`, `records_inserted: 0`, `records_updated: 0`) was a **false negative**: `scripts/lib/step/write.js:1027`'s `validateGeometries` built its lookup `Map` with `Number(r.source_key)` but looked it up with the RAW `f[keyColumn]` — for `parcels` (`key_sql_type: "TEXT"`, `coerceKey` returns a string) every one of the 495,495 shaped rows missed the join and was silently counted `skipped`, so the converted step had written **zero rows on every run since ② landed**, with `audit_table.verdict` still reading `PASS`. Filed CRITICAL in `review_followups.md` (2026-09-24, "commit ③ prep" entry) and fixed on the main tree via WF3 `.cursor/wf3_validate_geometries_text_key_active_task.md`: `6457e713` normalizes both sides of the join through one `String()` coercion (key-type-agnostic, no branch on `key_sql_type`) and makes a residual key miss THROW `ValidationKeyMissError` before any write, rather than silently degrading to `skipped`. `fff52b7a` corrected the diagnosis text (a `Number()`/`String()` type mismatch, not a `NaN` bug) and filed the WF3's own output-panel residue.

**Re-verification (WF3 Step 5, `.cursor/wf3_validate_geometries_text_key_active_task.md` "Step 5 RESULT"), MEASURED 2026-09-24:** worktree rebased onto the fix (① `8360fc32`, ② `1414cb73` re-landed on `fff52b7a`); `parcels` suite 60/60, `step-library.logic.test.ts` 308/308. A K=1000 forced-change perturbation (UPDATE-only, `lot_size_sqm`/`feature_type` guard columns, no `on_empty`, snapshotted first — DELETE/reinsert is unsafe over 4 FK tables) drove: converted-step run `records_updated=936`; legacy-loader run over the same perturbed rows `records_updated=936`; **both arms leave the SAME 64 residual sentinel `parcel_id`s** — parcels whose CSV row has departed the source, `retract:"none"` correctly keeping their last-seen values in both the converted and legacy loaders (not a conversion regression — see the descriptor `limitations[]` entry above, enriched with this measurement). A third run — the converted step over the DB state the LEGACY run just wrote — read `records_updated=0`, `content_hash` identical to the converted step's own prior write: **both arms write byte-identical state.** Perturbation fully reverted from backup; backup tables dropped.

**New trustworthy POST golden** (plain, unperturbed re-capture after the fix, uncommitted in the worktree, lands with this residue commit): `docs/reports/golden/parcels/post/sources.json` — `records_inserted: 0`, `records_updated: 0` (idempotent on unchanged live data, as expected for a plain capture), `rows_read: 498,479` (up from the pre-fix golden's undercounted `495,495` — the INGESTOR prerequisite 0o `rows_shaped`/`column_nulls` re-point, below, also landed in this same recapture), `table_state[0].content_hash: 72a91518…`, `row_count: 496,500`. `null_address_pct` now correctly reads `"100.0%"` / `WARN` (PR-D2's documented always-WARN legacy behaviour, genuinely restored) instead of the pre-fix golden's impossible `null`/`PASS` — the SAME `validateGeometries` miss that dropped every row also fed `null_address_pct`'s post-0o counters zero carried rows, which is why the CRITICAL surfaced during THIS recapture rather than at ②. The recapture also surfaces one declared-additive `records_meta` key with no PRE analogue: `warnings` (`["null_address_pct: 100.0%"]`), the runner's own row-derived list of every non-PASS check name — it did not exist in the pre-fix golden because a run with zero carried rows had nothing to warn about; it is not `parcels`-local, every converted step's `records_meta.warnings` array behaves identically, and `terminal` moves from `"loaded"` to `"loaded_with_warnings"` in lockstep with it. The prior golden (`15785c9b…`) is DISCARDED, not superseded-and-kept, per `tasks/lessons.md`'s new entry from this WF3: "a 0-write golden on a step that should write is not proof — demand a forced-change re-run with `records_inserted`/`records_updated` > 0."
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=39566B notes=12911B checks=7 rows records_meta=1461B (newest post/ capture) |
