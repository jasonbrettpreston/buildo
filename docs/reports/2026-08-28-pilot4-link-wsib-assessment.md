...
**Status:** Commits 1-6 landed, then commit 7 (`69de8a13`) + its out-of-sequence differential (commit 7b, §7) + peels 8a (`d44fb4ba`)/8b (`33ea3c0e`)/8c (`ce0d3f5f`). §0 (seed), Fold A/B/C (2026-08-28, folded into `.cursor/active_task.md`) remain below as history. **Scope RE-AUTHORIZED 2026-08-28 (orchestrator, within A-8): commit 8 (the one-off A-7 tier-3 repair FULL, budgeted/declared/operator-invoked) and commit 9 (cutover) are now in scope. R-L (chain_args `sources: ["--full"]`, closing the peel-8a-harvest ambiguity) is RULED and LANDED within this authorization.** **A-7's forced-FULL repair itself is NOT executed — BLOCKED.** Attempting it live found and closed a genuine SQL-generation defect (LW-D9), then surfaced a second, BLOCKING one (LW-D10: the T7 tier-3 convergence loop Fold B ruled required is declared in the descriptor/checks/notes but was never built in `scripts/lib/step/index.js`'s runtime `runCascadePhase`) — see §8b. Commit 9 (cutover — register `link-wsib.js` in `converted.json`, delete the `pending` entry, retire the old script text) remains **NOT STARTED**, now additionally gated on LW-D10's resolution (cutover needs a real `post/` capture, which needs a real repair run).
...
**Before/after table — NOT PRODUCED.** No repair ran; the pre-state table above is the only measured state. `docs/reports/golden/link_wsib/post-8-forced/sources-full-forced-1.json` records the THROWN attempt (exit_code 1, table hashes identical to `pre/`) as diagnostic evidence, not a successful capture — kept, not deleted, as the artifact `LW-D9` was found against.

**Defect ledger:** `LW-D9` CLOSED this commit · `LW-D10` OPEN, BLOCKING (see `docs/reports/defect-ledger.md`).
...
| 8 | Peel — one concern per commit (8a/8b/8c) → A-7 repair FULL | gating/staleness · verdict/audit · thresholds/checks; §8b R-L ruling + A-7 forced-FULL repair | full differential re-run after each; before/after repair table | **8a/8b/8c LANDED** (`d44fb4ba`/`33ea3c0e`/`ce0d3f5f`). **R-L LANDED this commit.** **A-7 forced-FULL repair NOT EXECUTED — BLOCKED on LW-D10** (§8b): LW-D9 (SQL-string bug) found live and CLOSED this commit; LW-D10 (T7 convergence loop unbuilt in `runCascadePhase`) is OPEN and BLOCKING, filed for a ruling rather than built unilaterally |
| 9 | Differential + cutover → G8, G4d, G-shape | `converted.json` (+1 → 4) | shape gate 4/62 enforced; differential green | NOT STARTED — **blocked behind commit 8's repair (LW-D10), which itself blocks the `post/` capture `#150` needs** |
