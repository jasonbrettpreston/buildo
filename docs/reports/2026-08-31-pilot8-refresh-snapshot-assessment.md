...
**Commit 9 rulings (operator, 2026-09-03):** **RS-D-STA** — STA-2/STA-3 (Spec 120 §6b's reset-SQL-generator/destructive-reset-guard promises) were authored before any RECORDER descriptor existed to check them against; the operator ruled their applicability itself becomes declared data rather than a hand-adjudicated exemption — `gate.applies_when: {"descriptor_path":"recovery.reset","equals":"generated"}` added to both items in `programme-items.json` (fast invariant #9 skips a blocked slug whose own descriptor's `recovery.reset` isn't `"generated"`; `refresh_snapshot`'s is prose, so the gate no longer blocks it — proven both directions in `src/tests/programme-backlog.infra.test.ts`). **RS-conformance-gap** — the §1.2a P4 tunable-externalization checker (`src/tests/step-conformance.infra.test.ts`) had a third blind spot alongside pilot 7's LP-D-conformance-gap: `config[BARE_CONST]` bracket reads via a plain single-const alias (`refresh-snapshot.js`'s `T1_VAR`/`T2_VAR`, distinct from `link-parcels.js`'s `CONFIG_KEYS.prop` object-literal form already handled) read as dead declarations for `snapshot_coa_conf_high`/`coa_match_conf_medium` despite being genuinely consumed. Widened the checker (`configSimpleConstMap`), not the compute — same precedent as pilot 7.
...
## WF3 I3a addendum (2026-09-14) — golden recapture, `pool_errors` G8 finding

`.cursor/wf3_i3a_infra_step_exemption_active_task.md` flips `refresh-snapshot.descriptor.json`'s
`identity.gate_exempt` `false → true` (§1.2), correcting it to match this slug's own always-true
`isInfraStep` verdict (the explicit `refresh_snapshot` name match) — independently supported by the
RECORDER contract itself ("record current state every run" should not stop on a zero-ingest day).
`source_fingerprint` hashes the descriptor file itself, so the flip staled all five `post/*.json`
golden captures (`coa.json`, `deep_scrapes.json`, `permits.json`, `sources.json`,
`standalone.json`) — Spec 122 §5.3 R-C's own rule applied. All five recaptured live (`node -r
dotenv/config scripts/analysis/capture-step-golden.js --step=scripts/refresh-snapshot.js
--chain=<chain> --invariants=docs/reports/golden/refresh_snapshot/invariants.json [--overwrite]`,
matching the original captures' explicit `--invariants=` flag — the descriptor's own derived
invariant set is narrower, 1 vs. this file's 2). `--compare` (old committed vs. new recapture)
found: `data_quality_snapshots` row_count `30 → 31` and its `content_hash` changing to match — this
IS the RECORDER doing its job (one more day's snapshot accumulated between the original capture and
this recapture, not a code change); `migrations=242→244` (more migrations landed on this branch
since the original capture); `chain_run_id: null→undefined` (a normalisation change from unrelated
already-landed work); and **`summary.records_meta.pool_errors: undefined → 0`** in all five
scenarios — a field `scripts/lib/step/index.js` (commit `6aa46613`, pilot 9 commit 8 P5, an
unrelated observability peel) started emitting on every run AFTER this step's original golden
captures were taken but BEFORE this WF3's recapture; it is `0` in every scenario (no pool errors
occurred), not something WF3 I3a's `gate_exempt` correction produced. Recorded here so
`step-validate.mjs`'s G8 "unexplained diffs" gate reads it as EXPLAINED (cited by name) rather than
silently passing or wrongly blaming this WF.

---

## WF3 `wf3_deep_scrapes_failures` Peel 2 addendum (2026-09-18) — golden recapture, `read_timings` G8 finding

Spec 118 §1 cause B (deep_scrapes, run `35009108061`, 2026-09-15): `refresh_snapshot`'s
`runRecorderPhase` read loop had no per-statement bound, no wall-clock phase deadline, and no
per-read trace — a single slow read was indistinguishable from a hang, and after the 15-minute
platform axe there was no record of which read was slow. This WF3 adds two new OPTIONAL
`execution.*_from_config` descriptor fields (`statement_timeout_minutes_from_config`,
`phase_deadline_minutes_from_config`, `refresh_snapshot_read_statement_timeout_minutes` = 5min,
`refresh_snapshot_phase_deadline_minutes` = 12min) and a new `records_meta.read_timings[]` array —
one `{key, elapsed_ms, row_count}` entry per one of the 8 main reads, surviving the process so a
slow read is diagnosable from the persisted `pipeline_runs` row after the fact. `source_fingerprint`
hashes the descriptor + compute files, so this change staled all five `post/*.json` golden captures
(`coa.json`, `deep_scrapes.json`, `permits.json`, `sources.json`, `standalone.json`) — Spec 122 §5.3
R-C's own rule applied. All five recaptured live (`node -r dotenv/config
scripts/analysis/capture-step-golden.js --step=scripts/refresh-snapshot.js --chain=<chain>
--invariants=docs/reports/golden/refresh_snapshot/invariants.json --overwrite`, after seeding the
two new logic variables locally via `node -r dotenv/config scripts/seeds/apply-logic-variables.js` —
a converted step's config never falls back to the seed file's default, Spec 122 §1.2a P4).
Diff-class prediction, made in the plan-panel fold BEFORE recapture
(`.cursor/wf3_deep_scrapes_failures_active_task.md`, Class A/B/C/D vocabulary per
`docs/reports/golden/enrich_parcels/phase010b-post-phase-seam-recapture.md`): **Class A non-empty
and EXPECTED** (the new `records_meta.read_timings[]` key on every capture; `config` key count
44→46 for the two new logic variables) — **measured: correct**, both predicted diffs present and
nothing else structural moved (no row added/removed/reordered, no counter moved, no threshold
moved). **Class B expected empty** (no live-count-drift mechanism this peel touches) — **measured:
correct**, the `data_quality_snapshots` row_count `31 → 32` and its `content_hash` moving is the
RECORDER doing its job (one more day accumulated between the WF3 I3a recapture and this one, not a
code change — the same reasoning I3a's own addendum gave for its `30 → 31`). **Class C expected
non-empty** (wall-clock duration, live counts) — **measured: correct**, `duration_ms` and the
per-read `elapsed_ms` values move on every capture by construction (declared in `nondeterminism[]`
before the first diff, per §5.3's own inventory rule) and are excluded from the must-match-exactly
comparison. **`read_timings` itself is the ONE genuinely new field** `--compare` flags as an
"unexplained" structural diff (G8) purely because the key never existed before this WF3 — recorded
here so the gate reads it as EXPLAINED (cited by name) rather than silently passing or wrongly
blaming a future WF.

### Output-panel fold R1 (2026-09-18) — the 6 optional reads gain the same bound + trace

The Peel-2 addendum above scoped `read_timings[]` to the 8 main reads only. Output-panel fold R1
extends the SAME per-statement bound and phase deadline (the SAME two logic variables — no new
config surface) to the 6 optional reads (massing, schemaColumnCounts, sla, inspections, costEst,
coaFunnel), which run on their own dedicated autocommit client outside the pinned REPEATABLE READ
snapshot (a pre-existing fence, git-blamed to `c19cf224^:scripts/refresh-snapshot.js:308-503`,
itself predating `fd14dc53` — preserved, not touched). Every `read_timings[]` entry now carries a
`phase` field (`"main"` or `"optional"`), and a bound-cancelled optional read additionally carries
`cancelled: true` + `cancel_kind` — three genuinely new leaf field names, all cited here so G8 reads
them as EXPLAINED. This is a THIRD recapture of all 5 `post/*.json` (same calendar day as the Peel-2
recapture — `data_quality_snapshots` row_count stayed `32`, confirming the same-day invariant
`src/tests/steps/refresh_snapshot/violations.test.ts` pins). No optional read was cancelled on any
of the 5 live recaptures (the DB is healthy) — `cancelled`/`cancel_kind` are proven only by
`src/tests/db/refresh-snapshot-recorder-bound.db.test.ts`'s real-Postgres RED-FIRST lock, not by
these captures.
...
> Generated by `node scripts/analysis/step-validate.mjs --step=refresh_snapshot --write` — Spec 123 §6, ruling R-R (2026-08-29).
...
| 8 | refresh_snapshot | PASS | G-4: 4 declared, 0 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
| 20 | refresh_snapshot | PASS | HB-1: execution.shape="recorder" — HB-1 applies_when execution.shape=="enrich" only (RS-D-STA); not applicable, never a pass-by-omission |
| 21 | refresh_snapshot | PASS | CEIL-1: execution.shape="recorder" — CEIL-1 applies_when execution.shape=="enrich" only (RS-D-STA); not applicable, never a pass-by-omission |
| 4 | (registry) | PASS | overlap: none |
| 5 | (registry) | PASS | clean (0 it.fails( call sites outside a declared pending slug) |
| 9 | (registry) | PASS | clean (0 converted slugs blocked by an unmet cutover_prereq item; blocks batching: 0) |
| 22 | (registry) | PASS | GOLD-PRE-FRESH: 62 PRE capture(s) across 16 converted step(s) all tracked + clean (git can restore every reference) |
...
| 25 | (registry) | PASS | ARCHETYPE-PARITY: 16 converted slug(s) — 8 compared against a retained census row (all agree), 8 with no retained row (census arm n/a, pre-R-AO cutovers); every archetype has a declared freeze profile |
| 26 | (registry) | PASS | COUNTER-ROOT: 35 declared counter source(s) across 12 descriptor(s) all root in their own shape's counterScope (+ records_meta) |

### Captures (item iv)
- missing invocations (POST): none
- missing invocations (PRE, GOLD-PRE): none
- stale fingerprints: none
- compare ran: true · diffs found: 747 · unexplained: 0

### Test suite (item iii)
- 1230/1249 passed (suite success=false)
- harvested: 21 file(s) from 3 FLEET-WIDE targets (src/tests/step-conformance.infra.test.ts, src/tests/golden-fingerprint.infra.test.ts, src/tests/steps/) — one spawn per run, so every step's report carries this same number, by design
...
- failing (19):
  ...
  - src/tests/step-conformance.infra.test.ts > R-R / Rule 13 — the generated scorecard block is not stale (vitest-independent sections) > scripts/enrich-ravines.js (slug "enrich_ravines") > report carries exactly one generated scorecard block
  ...
  - src/tests/step-conformance.infra.test.ts > R-R / Rule 13 — the generated scorecard block is not stale (vitest-independent sections) > scripts/enrich-ravines.js (slug "enrich_ravines") > the committed block also carries a Test-suite line and a 14-row Policy coverage matrix (presence only — content is `--all --write`'s job, not this lock's)
  - src/tests/steps/load_ravines/violations.test.ts > 55-A — the hard per-conversion gate (44, k=PER_STEP) > #203 Frozen `records_meta` producer/consumer blocks — ravine_load → enrich_ravines, all 7 consumed fields declared and read
...
| 3 | Tunables externalized | enforced-green | G-4: 4 declared, 0 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=29213B notes=3746B checks=10 rows records_meta=2859B (newest post/ capture) |
