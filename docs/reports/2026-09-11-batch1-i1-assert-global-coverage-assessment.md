...
| IL-4 | **DEC-2** — `Step 9b` / `CoA Step 4b` insert-after label convention, full renumber deferred as `#405` | same commit, `3ab4fa83` | (see IL-3) | `docs/specs/01-pipeline/49_data_completeness_profiling.md:170,194` confirms `#405` is a **separately-tracked cosmetic item**, not resolved by this or any later commit touching this file (re-confirmed live this session — no commit since `3ab4fa83` touches the label strings). | **RULED — ACCEPT: preserved-in-compute, labels copied VERBATIM** (Fold A item 3, binding) — the row-builder census below (§2.2) carries every `Step 9b`/`CoA Step 4b` label unmodified; #405's full renumber stays out of scope for this conversion. IL-4 ACCEPT: labels verbatim, #405 out of scope. |
...
*Report continues — commits 6-9 (red-first tests, descriptor+compute, peels, cutover) are out of scope for this WF2-C batch-1 I1 assessment session (commit 5 golden PRE captures + this non-determinism inventory + the PH-3 adjudication record, per the executing instruction).*
