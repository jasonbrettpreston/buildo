...
## §Addendum — chain_run_id re-capture note (2026-08-30, CC-D3 follow-on WF3)

`assert_schema`'s own `config.probe_presence`/`declared_logic_variables_present.expect` lists gained one
entry (`compute_centroids_full_recompute_batch_size`, CC-D3's new T3 tunable) when `compute_centroids`
declared it — a fleet-wide probe list, not an assert_schema-specific change. Re-capturing
`post/{coa,permits,sources,standalone}.json` to pick up that one-line descriptor edit also surfaced
`records_meta.chain_run_id` (R-U) in the diff against `pre/` for the first time — R-U landed on this branch
AFTER this report's own captures were last taken, unrelated to CC-D3 or the probe-list edit. Named here so
`step:validate`'s G8 diff-citation gate reads it as explained, not a silent regression.

---

## §Addendum — pilot 9 cutover re-capture note (2026-09-11, R-D three-way lock)

The enrich_parcels cutover (pilot 9 commit 9) added 44 declared logic variables to the converted fleet, so RULING R-D's three-way lock (checks[].expect ≡ config.probe_presence ≡ the LIVE collectDeclaredLogicVariableNames() derivation) required assert-schema.descriptor.json's two lists to be regenerated 42 → 86 names (the only descriptor edit; +88 lines, no other field touched). The descriptor is a fingerprint file (R-C), so all four POST captures (coa / permits / sources / standalone) were re-taken against the local stack at HEAD bc81ac84 — fingerprint 37025a60, exit 0, verdict PASS on every invocation. Versus the committed captures (b4ddfc24) each file differs in exactly three leaves, none behavioural: pool_errors (a new records_meta counter the shared library now emits on every run — EP-D16's pool-error accounting — value 0 on all four), chain_run_id (the key is now omitted rather than written as null under a chain — same CC-D3 addendum class as above), and stdout_lines[0] (the resolve-db target line reads migrations=244 instead of 242 — migrations 245–247 applied locally since). 4 differences in stdout_lines across the four files, all that one line. Pre-vs-post totals: 239 differences, of which the pre-existing conversion-era leaves (invariants [], meta.external, meta.reads "CKAN API" → logic_variables) are the §7 differential already explained above.

## §Addendum 2 — batch1 I1 cutover re-capture note (2026-09-12, R-D three-way lock)

Batch1 I1's own cutover (`assert_global_coverage`, commit 9, R-K) registered 20 more declared logic variables into the converted fleet (the step's 6 pre-existing + 14 newly-adjudicated tunables, report §2.4 of `docs/reports/2026-09-11-batch1-i1-assert-global-coverage-assessment.md`), so RULING R-D's three-way lock required `assert-schema.descriptor.json`'s two lists (`checks[0].expect` / `config.probe_presence`) to be regenerated **86 → 106** names (0 removed, all 20 additions are exactly `assert_global_coverage`'s declared set — verified programmatically against the live `collectDeclaredLogicVariableNames()` derivation, which now also walks `assert_global_coverage.descriptor.json` since it entered `converted.json.converted[]` this commit). No other field touched. The descriptor is a fingerprint file (R-C), so all four POST captures (coa / permits / sources / standalone) were re-taken against the local stack at HEAD `fa960c48` — fingerprint `2c7a2830de6eedfd446d28e67e807666f433f253f79cd30e457d1c9dec739313` on all four (was `37025a60…`), exit 0, verdict PASS on every invocation. Versus the pre-recapture committed captures, each file differs in exactly the declared-volatile leaves and nothing else (confirmed by direct `git diff`, not the generic `--compare`, which would also surface the unrelated pre-vs-post conversion diff): `git_head`, `pipeline_runs_max_id_before` (harness metadata, monotonic), `row:sys_duration_ms` (declared volatile, §5.1-class), the `stdout` blob's own embedded duration text, and `source_fingerprint` itself (the intended effect of editing a fingerprinted input) — plus, on `standalone.json` only, the single `pipeline_runs[0]` row's `id`/`started_at`/`completed_at`/`duration_ms`/`records_meta.chain_run_id` (declared volatile, unrelated to this edit). No check id, value, severity, or row shape changed on any chain; `declared_logic_variables_present`'s own row still reads `{"missing":[]}` PASS on all four, confirming all 106 names — including the 20 new ones — resolve against the live `logic_variables` table (seeded at batch1 I1 commit 7). `src/tests/golden-fingerprint.infra.test.ts` (39 tests, was 36) confirms every POST capture's `source_fingerprint` matches the current tree.
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=39129B notes=8992B checks=10 rows records_meta=1547B (newest post/ capture) |
