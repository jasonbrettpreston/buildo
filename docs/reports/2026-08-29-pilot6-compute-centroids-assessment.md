...
| `7c75e92e` (feat) | 2026-04-02 | PostGIS spatial offload — the ENTIRE current PostGIS fast path (`:98-115`) is introduced whole in this commit, unchanged since | ✓ `:98-115` is this commit's code verbatim, 0 subsequent edits to the PostGIS branch itself | **preserved-in-compute** — the fast-path SQL (`UPDATE parcels SET centroid_lat=ST_Y(ST_Centroid(geom)),...`) is verbatim-ported to `compute.js` at commit 7 per G2's "Spec 123 §7 step 7 requirement". **Grounded (CC-D4, 2026-09-03):** the RULE (PostGIS required, no second algorithm) is written down in `notes.json` `decisions[]` A-1(a) + descriptor `guards.requires` (postgis/fail) — Rule 2's R-W addendum is the standing policy this row instantiates. | `:98-115` current file, unchanged since `7c75e92e` |
...
| `90e3d0f8` (fix) | 2026-04-17 | `parseInt`/`parseFloat` → `safeParsePositiveInt`/`safeParseFloat` (B1 safe-math migration) | ✗ **PREMISE REFUTED (CC-D4, 2026-09-03):** 0 `safeParse` call sites in `scripts/lib/compute/compute-centroids.js`/`scripts/lib/step/*.js` (re-grepped this session); the old file's 3 call sites (`git show 90e3d0f8:scripts/compute-centroids.js` → `:19`,`:67`,`:198`,`:200`) were NOT ported | **knowingly-retired — re-disposed CC-D4 (operator ruling, 2026-09-03; supersedes Fold D's G3-vocabulary ruling at item 5, which predates commit 7's cutover)** — `:67`'s count parse is superseded by server-side `COUNT(*)::int` (`buildPreSql`/`buildPostSql`); `:198`/`:200`'s threshold parse is superseded by `config.logic_variables[]` min/max + `on_invalid:"fail"` (T1/T2) and the row-derived verdict. **Grounded (CC-D4):** purpose preserved via `checks[].why` of T1/T2 and the descriptor's `on_invalid:"fail"` field — `encoded-as-descriptor-field`, not literal code survival. | `git show 90e3d0f8:scripts/compute-centroids.js` (`:19,:67,:198,:200`); 0 hits in current compute module |
...
5. **G3 vocabulary.** ~~Safe-math hardening commits (`90e3d0f8`, `3c3e6f84`) → disposition **`preserved-in-compute`** (their helpers are live at `:67`/`:193`/`:198`).~~ **Amended 2026-09-03 (CC-D4, operator-ruled) — not silently:** this ruling for `90e3d0f8` predates commit 7's cutover and is REFUTED — 0 `safeParse` call sites survive in the converted compute module (`scripts/lib/compute/compute-centroids.js`); disposition corrected to **`knowingly-retired`** (purpose preserved via `COUNT(*)::int` + `config.logic_variables[]` min/max + `on_invalid:"fail"`; see §2 row `90e3d0f8` and §11). `3c3e6f84` (advisory lock retrofit, unrelated to safe-math) was mis-grouped here — its own §2 row correctly carries **`encoded-as-descriptor-field`** (`99` → `identity.lock`), unaffected by this correction. Commits leaving nothing observable in the current file → **`knowingly-retired`** with qualifier *"superseded by the 2026-08-29 rewrite, never itself load-bearing."* **`INCIDENTAL` never appears as a disposition** — every occurrence of the word as a G3/G6 disposition in this report is struck at its own location above.
...
## §11. CC-D4 — Rule 4/G-2 grounding (WF3, 2026-09-03)
...
```
`node -r dotenv/config scripts/analysis/step-validate.mjs --step=compute_centroids` → `compute_centroids: 14/17 hard-stop=false` (score measured this session; the plan's premise-verification cited 13/17 from an earlier run — both are pre-fix baselines, Rule 4 red in both).

**Which converted steps fail Rule 4 today (`--all --fast`):** 8 converted steps, 2 red before this commit — `link_wsib` (4/8, deliberately out of scope, one finding per WF3) and `compute_centroids` (3/3). `assert_schema` is green (6 rows, 0 ungrounded).

**Dispositions (this commit):**

- **Row A** (`e4765619`, §2 :101) — kept `preserved-in-compute`; grounded in-row: the accounting SHAPE is `encoded-as-descriptor-field`, declared in `checks[].why` of `parcels_processed`/`centroids_computed`; only the emission itself is in compute. No mechanism change.
- **Row B** (`7c75e92e`, §2 :109) — kept `preserved-in-compute`; grounded in-row: the RULE (PostGIS required, no second algorithm) is written down in `notes.json decisions[]` A-1(a) + descriptor `guards.requires` (postgis/fail) — Rule 2's R-W addendum is the standing policy this row instantiates.
- **Row C** (`90e3d0f8`, §2 :111) — **re-disposed `preserved-in-compute` → `knowingly-retired`** (operator ruling, §4.2 adjudication — this pass discovered the refutation, the operator adjudicated the disposition). Refutation evidence: `grep -rn safeParse scripts/lib/compute/compute-centroids.js scripts/lib/step/*.js` → **0 hits**; the pre-conversion file's 3 call sites confirmed via `git show 90e3d0f8:scripts/compute-centroids.js` (`:19,:67,:198,:200`). `:67`'s count parse is superseded by server-side `COUNT(*)::int` (`buildPreSql`/`buildPostSql`); `:198`/`:200`'s threshold parse is superseded by `config.logic_variables[]` min/max + `on_invalid:"fail"` (T1/T2) and the row-derived verdict. Since the row no longer reads `preserved-in-compute`, it drops out of G-2's `preservedRows` filter entirely — G-2 now scores 2 preserved-in-compute rows, both grounded.
- **Fold D's 2026-08-29 G3-vocabulary ruling** (item 5, pre-§2 amendment above) is corrected in place, dated, not silently — it ruled `90e3d0f8` `preserved-in-compute` before commit 7's cutover existed to check against.

**No new artefact needed — the rules were already declared.** None of the 3 rows is undeclared policy (a threshold, fence, or tunable needing a NEW rung): this step's only two thresholds (T1/T2) are already `encoded-as-descriptor-field` elsewhere in the ledger. Rows A and B are mechanism whose governing rule is already written down in shipped artefacts (`checks[].why`, `notes.json decisions[]`, `guards.requires`) — the defect was a citation gap in the Intent Ledger prose, not a missing declaration. Row C's mechanism was never ported at all, so `knowingly-retired` (purpose preserved by superseding artefacts) is the accurate word, also grounded in already-shipped fields. No descriptor or `notes.json` edit was made — `source_fingerprint` is unchanged, all `post/*.json` golden captures stay current (contingency in the plan's Risks §1 not triggered).

**GREEN, verbatim (after this commit, `--write`):** see the regenerated scorecard below — Rule 4 row and G-2 detail.

---

## WF3 I3a addendum (2026-09-14) — golden recapture, `pool_errors` G8 finding

`.cursor/wf3_i3a_infra_step_exemption_active_task.md` flips `compute-centroids.descriptor.json`'s
`identity.gate_exempt` `false → true` (§1.2), correcting it to match this slug's own always-true
`isInfraStep` verdict (the `compute_` prefix). `source_fingerprint` hashes the descriptor file
itself, so the flip staled all three `post/*.json` golden captures (`sources.json`,
`sources-full-forced-1.json`, `standalone.json`) — Spec 122 §5.3 R-C's own rule ("a descriptor edit
after capture requires a recapture with the named cause") applied, not a contingency this WF
skipped. All three recaptured live (`node -r dotenv/config scripts/analysis/capture-step-golden.js
--step=scripts/compute-centroids.js --chain=<sources|none> [--overwrite]`, `sources-full-forced-1`
additionally under `COMPUTE_CENTROIDS_FORCE_FULL=1`) against the SAME already-repaired corpus CC-D3
left behind — `table_state` hash unchanged (`parcels:486530/a12a1a54` in every scenario) and the
forced-full run again selected `recompute_rows_changed:0` (idempotent, matching CC-D3's own
second-run proof). `--compare` (old committed vs. new recapture) found only pre-existing,
already-explained drift: `migrations=242→244` (more migrations landed on this branch since the
original capture), `chain_run_id: null→undefined` (a normalisation change from unrelated already-
landed work), and **`summary.records_meta.pool_errors: undefined → 0`** — a field `scripts/lib/
step/index.js` (commit `6aa46613`, pilot 9 commit 8 P5, an unrelated observability peel) started
emitting on every run AFTER this step's original golden captures were taken but BEFORE this WF3's
recapture; it is `0` in every scenario (no pool errors occurred), not something WF3 I3a's
`gate_exempt` correction produced. Recorded here so `step-validate.mjs`'s G8 "unexplained diffs"
gate reads it as EXPLAINED (cited by name) rather than silently passing or wrongly blaming this WF.
...
| G1 | 1 | 1 | PH-3 section found=true sha-count=29 |
...
| 20 | compute_centroids | PASS | HB-1: execution.shape="backfill" — HB-1 applies_when execution.shape=="enrich" only (RS-D-STA); not applicable, never a pass-by-omission |
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=36278B notes=9463B checks=8 rows records_meta=3142B (newest post/ capture) |
