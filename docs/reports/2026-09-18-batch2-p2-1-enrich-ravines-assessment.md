...
> Generated by `node scripts/analysis/step-validate.mjs --step=enrich_ravines --write` — Spec 123 §6, ruling R-R (2026-08-29).
> Regenerate with the same command; a stale block is a conformance-lock finding (`step-conformance.infra.test.ts`).

**Score: 12/17** · G9 Reflection: PASS · G4d fence-lock coverage: PASS · G-shape: PASS · **Hard stop: no**

| Gate | Score | Max | Detail |
|---|---:|---:|---|
| G0 | 0 | 1 | boundary-section=true spec-line=false |
| G1 | 1 | 1 | PH-3 section found=true sha-count=12 |
| G2 | 1 | 1 | 122-churn-complexity.md quadrant=bottom-left window=39313d9 |
| G3 | 1 | 2 | table rows=11 vocab-hit rows=0 |
...
| # | Scope | Pass | Detail |
|---|---|---|---|
| 1 | enrich_ravines | PASS | min_migration=244 <= migrations count=244 |
...
| 8 | enrich_ravines | PASS | G-4: 8 declared, 1 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
| 20 | enrich_ravines | PASS | HB-1: runner=runEnrichPhase: runner-level token presence (MED-6, not a per-phase proof): source contains an onProgress seam token AND a startHeartbeatTicker( call token — the periodic ticker covers every phase uniformly by construction once present, independent of any single phase's own boundary, but ticker start/stop lifecycle is not independently verified here |
| 21 | enrich_ravines | PASS | CEIL-1: runner=runEnrichPhase: runner-level token presence (MED-6, not a per-phase proof): source contains a SET LOCAL statement_timeout/lock_timeout token pair AND a postClient-scoped SET statement_timeout token (EP-D16) — the per-phase claim itself is filed as its own followup |
...
| 3 | Tunables externalized | enforced-green | G-4: 8 declared, 1 verdict-affecting, 0 violate on_invalid:fail with no deviations[] cover |
...
| P3 | I/O cost adjudication (measured, not gated) | measured | descriptor=38227B notes=0B checks=7 rows records_meta=3343B (newest post/ capture) |
